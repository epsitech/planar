/**
  * \file PlnrdAppsrv.cpp
  * application server for Plnr daemon (implementation)
...
MHD_Daemon* PlnrdAppsrv::start(
			XchgPlnrd* xchg
		) {
//	return MHD_start_daemon(MHD_USE_SELECT_INTERNALLY, xchg->stgplnrd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_THREAD_POOL_SIZE, 16, MHD_OPTION_END);
	return MHD_start_daemon(MHD_USE_THREAD_PER_CONNECTION, xchg->stgplnrd.appsrvport, &MhdAccept, (void*) xchg, &MhdCallback, (void*) xchg, MHD_OPTION_NOTIFY_COMPLETED, &MhdComplete, (void*) xchg, MHD_OPTION_END);
};

void PlnrdAppsrv::stop(
			MHD_Daemon* d
		) {
	MHD_stop_daemon(d);
};

int PlnrdAppsrv::MhdAccept(
			void* cls
			, const sockaddr* addr
			, socklen_t addrlen
		) {
	...
	if (*con_cls == NULL) {
		...
		if (strcmp(method, "GET") == 0) {
			...
			if (s.find("/web/") != string::npos) {
				...
				// ex. "/web/CrdPlnrNav/CrdPlnrNav.html" ; note that eventual parameters such as "?scrJref=abcd1234efgh5678" are not included in url
			...
			*con_cls = req;
			...
			if (req->ixVBasetype == ReqPlnr::VecVBasetype::WEB) {
				...
				if (valid) {
					...
					if (dcol->dpchs.empty()) {
						if (dcol->hot) {
							...
							Mutex::lock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[1]");
							dcol->unlockAccess("PlnrdAppsrv", "MhdCallback[3]");
							Cond::timedwait(&(req->cReady), &(req->mcReady), &nexttime, "req->cReady", "PlnrdAppsrv", "MhdCallback");
							// cout << "got one after timedwait" << endl;
							Mutex::unlock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[1]");

							dcol = xchg->getDcolByJref(req->jref);
							if (dcol) {
								// note: the destructor of dcol is called after dcol is removed from the list from which it can be found by getDcolByJref()
								dcol->req = NULL;
								dcol->unlockAccess("PlnrdAppsrv", "MhdCallback[4]");
						...
						dcol->hot = true;
						dcol->unlockAccess("PlnrdAppsrv", "MhdCallback[5]");
				...
				if (valid) {
					...
					dcol->unlockAccess("PlnrdAppsrv", "MhdCallback[6]");
				...
				if (valid) {
					...
					// wait for reply
					Mutex::lock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[2]");
					Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "PlnrdAppsrv", "MhdCallback[1]");
					Mutex::unlock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[2]");
				...
				*upload_data_size = 0;
				...
				if (req->ixVBasetype == ReqPlnr::VecVBasetype::REGULAR) {
					...
					if (valid) {
						...
						// wait for reply
						Mutex::lock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[3]");
						if (req->ixVState != ReqPlnr::VecVState::REPLY) Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "PlnrdAppsrv", "MhdCallback[2]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[3]");
					...
					if (valid) {
						...
						Mutex::lock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[4]");
						Cond::wait(&(req->cReady), &(req->mcReady), "req->cReady", "PlnrdAppsrv", "MhdCallback[3]");
						Mutex::unlock(&(req->mcReady), "req->mcReady", "PlnrdAppsrv", "MhdCallback[4]");
...
#else
ssize_t PlnrdAppsrv::MhdFilesend(
			void* con_cls
			, uint64_t pos
			, char* buf
			, size_t max
...
bool PlnrdAppsrv::validateJrefNotify(
			XchgPlnrd* xchg
			, ReqPlnr* req
			, const bool update
		) {
	...
	if (dcol) {
		...
		dcol->unlockAccess("PlnrdAppsrv", "validateJrefNotify");
	};

	return retval;
};

uint PlnrdAppsrv::readDpchApp(
			XchgPlnrd* xchg
			, ReqPlnr* req
		) {
	xmlDoc* doc = NULL;

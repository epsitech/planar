/**
  * \file PnlPlnrDomMNCalc.js
  * web client functionality for panel PnlPlnrDomMNCalc
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDomMNCalc", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDomMNCalc", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("MNCalc_side").src = "./PnlPlnrDomMNCalc_aside.html";
		doc.getElementById("MNCalc_cont").src = "./PnlPlnrDomMNCalc_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDomMNCalc", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("MNCalc_side").src = "./PnlPlnrDomMNCalc_bside.html";
		doc.getElementById("MNCalc_cont").src = "./PnlPlnrDomMNCalc_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDomMNCalc", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDomMNCalc", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDomMNCalc", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDomMNCalc", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDomMNCalc", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDomMNCalc", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDomMNCalc", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDomMNCalc", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDomMNCalc", "TcoMrefWidth"));
		var TcoPrpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDomMNCalc", "TcoPrpWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDomMNCalc", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDomMNCalc", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDomMNCalc
		rownode = getRownode(srcdoc, "ListPlnrQDomMNCalc", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDomMNCalc", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDomMNCalc", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDomMNCalc", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDomMNCalc", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDomMNCalc", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDomMNCalc", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDomMNCalc", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDomMNCalcData", scrJref, "StgIacQryPlnrDomMNCalc");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDomMNCalc", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDomMNCalc", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDomMNCalc", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDomMNCalcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "ContInfPlnrDomMNCalc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StatAppPlnrDomMNCalc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StatShrPlnrDomMNCalc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StgIacPlnrDomMNCalc", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "TagPlnrDomMNCalc", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "ListPlnrQDomMNCalc", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StatAppQryPlnrDomMNCalc", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StatShrQryPlnrDomMNCalc", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDomMNCalcData", "StgIacQryPlnrDomMNCalc", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDomMNCalcData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDomMNCalcData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDomMNCalcData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

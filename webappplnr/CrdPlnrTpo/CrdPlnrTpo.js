/**
  * \file CrdPlnrTpo.js
  * web client functionality for card CrdPlnrTpo
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppPlnrTpo", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppPlnrTpo", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppPlnrTpo", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppPlnrTpo", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppPlnrTpo", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlPlnrTpoHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlPlnrTpoList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlPlnrTpoRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppPlnrTpo", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppPlnrTpo", "shortMenu", short);
	setSi(srcdoc, "StatAppPlnrTpo", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppPlnrTpo", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrTpo", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagPlnrTpo", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagPlnrTpo", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrPlnrTpo", "MspCrd1Avail") == "true");
	MitCrdUcsAvail = (retrieveSi(srcdoc, "StatShrPlnrTpo", "MitCrdUcsAvail") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrTpo", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdTpo"), retrieveCi(srcdoc, "ContInfPlnrTpo", "MtxCrdTpo"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdUcs"), retrieveTi(srcdoc, "TagPlnrTpo", "MitCrdUcs"));
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgPlnrTpoUsecase")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgusecase = retrieveSi(srcdoc, "StatShrPlnrTpo", "scrJrefDlgusecase");

	if (scrJrefDlgusecase != "") {
		if (scrJrefDlg != scrJrefDlgusecase) showDlg("DlgPlnrTpoUsecase", scrJrefDlgusecase);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfPlnrTpo", "MtxCrdTpo") + " - Planar 0.1.22";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrTpoDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrTpoDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, ctlsref) {
	window.open(retrieveCi(srcdoc, "ContInfPlnrTpo", ctlsref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrTpoData", "ContInfPlnrTpo", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrTpoData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngPlnrTpoData", "StatAppPlnrTpo", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrTpoData", "StatShrPlnrTpo", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrTpoData", "TagPlnrTpo", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrTpoData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrPlnrTpo", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrPlnrTpo", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrPlnrTpo", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrTpoData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrTpo", "srefIxPlnrVReqitmode");

	if (srefIxPlnrVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxPlnrVReqitmode = "notify";
		setSi(srcdoc, "StatAppPlnrTpo", "srefIxPlnrVReqitmode", srefIxPlnrVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrTpo", "srefIxPlnrVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppPlnrTpo", "latency"));

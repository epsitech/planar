/**
  * \file PnlPlnrTpoList.js
  * web client functionality for panel PnlPlnrTpoList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrTpoList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrTpoList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrTpoList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrTpoList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrTpoList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrTpoList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrTpoList", "srefIxPlnrVExpstate");
...
// IP refreshA.nopre --- BEGIN
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrTpoList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrTpoList", "ntot") + ")");
// IP refreshA.nopre --- END
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoTitWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoTypWidth"));
		var TcoMajWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoMajWidth"));
		var TcoSubWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoSubWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoSteWidth"));
		var TcoFliWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoList", "TcoFliWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrTpoList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFli"); doc.widths.push(TcoFliWidth); doc.fs.push("fli"); doc.tos.push(0); wtot += TcoFliWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrTpoList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrTpoList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQTpoList
		rownode = getRownode(srcdoc, "ListPlnrQTpoList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrTpoList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrTpoList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrTpoList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrTpoList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrTpoList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrTpoListData", scrJref, "StgIacQryPlnrTpoList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrTpoList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrTpoListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrTpoList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrTpoList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrTpoListData", scrJref, "ContIacPlnrTpoList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "ContIacPlnrTpoList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "ContInfPlnrTpoList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "StatShrPlnrTpoList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "StgIacPlnrTpoList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "TagPlnrTpoList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "ListPlnrQTpoList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "StatAppQryPlnrTpoList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "StatShrQryPlnrTpoList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrTpoListData", "StgIacQryPlnrTpoList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrTpoListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrTpoListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrTpoListData") {
				mask = mergeDpchEngData(dom);

/**
  * \file PnlPlnrTpoMNProject.js
  * web client functionality for panel PnlPlnrTpoMNProject
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrTpoMNProject", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrTpoMNProject", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("MNProject_side").src = "./PnlPlnrTpoMNProject_aside.html";
		doc.getElementById("MNProject_cont").src = "./PnlPlnrTpoMNProject_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrTpoMNProject", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("MNProject_side").src = "./PnlPlnrTpoMNProject_bside.html";
		doc.getElementById("MNProject_cont").src = "./PnlPlnrTpoMNProject_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrTpoMNProject", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrTpoMNProject", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrTpoMNProject", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrTpoMNProject", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoMNProject", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoMNProject", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoMNProject", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoMNProject", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrTpoMNProject", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrTpoMNProject", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrTpoMNProject", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQTpoMNProject
		rownode = getRownode(srcdoc, "ListPlnrQTpoMNProject", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrTpoMNProject", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrTpoMNProject", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrTpoMNProject", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoMNProject", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrTpoMNProject", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrTpoMNProject", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrTpoMNProject", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrTpoMNProjectData", scrJref, "StgIacQryPlnrTpoMNProject");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrTpoMNProject", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrTpoMNProject", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrTpoMNProject", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrTpoMNProjectDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "ContInfPlnrTpoMNProject", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StatAppPlnrTpoMNProject", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StatShrPlnrTpoMNProject", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StgIacPlnrTpoMNProject", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "TagPlnrTpoMNProject", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "ListPlnrQTpoMNProject", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StatAppQryPlnrTpoMNProject", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StatShrQryPlnrTpoMNProject", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrTpoMNProjectData", "StgIacQryPlnrTpoMNProject", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrTpoMNProjectData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrTpoMNProjectData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrTpoMNProjectData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrStkView.js
  * web client functionality for panel PnlPlnrStkView
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrStkView", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrStkView", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("View_side").src = "./PnlPlnrStkView_aside.html";
		doc.getElementById("View_cont").src = "./PnlPlnrStkView_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrStkView", "srefIxPlnrVExpstate", "regd");

		// change content (container heights in refreshB)
		doc.getElementById("View_side").src = "./PnlPlnrStkView_bside.html";
		doc.getElementById("View_cont").src = "./PnlPlnrStkView_b.html";
	};
};
...
function initA() {
	...
	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrStkView", "Cpt"));
...
// IP initB --- BEGIN
	initCpt(hdrdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrStkView", "Cpt"));
// IP initB --- END
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrStkViewData", "TagPlnrStkView", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrStkViewData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrStkViewData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrStkViewData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

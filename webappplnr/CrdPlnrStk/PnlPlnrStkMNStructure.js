/**
  * \file PnlPlnrStkMNStructure.js
  * web client functionality for panel PnlPlnrStkMNStructure
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrStkMNStructure", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrStkMNStructure", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("MNStructure", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("MNStructure_side").setAttribute("height", "30");
		doc.getElementById("MNStructure_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("MNStructure_side").src = "./PnlPlnrStkMNStructure_aside.html";
		doc.getElementById("MNStructure_cont").src = "./PnlPlnrStkMNStructure_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrStkMNStructure", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("MNStructure", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("MNStructure_side").setAttribute("height", "255");
		doc.getElementById("MNStructure_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("MNStructure_side").src = "./PnlPlnrStkMNStructure_bside.html";
		doc.getElementById("MNStructure_cont").src = "./PnlPlnrStkMNStructure_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNStructure_side").contentDocument;
	contdoc = doc.getElementById("MNStructure_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNStructure_side").contentDocument;
	contdoc = doc.getElementById("MNStructure_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNStructure_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNStructure_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNStructure_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrStkMNStructure", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrStkMNStructure", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrStkMNStructure", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrStkMNStructure", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrStkMNStructure", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrStkMNStructure", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrStkMNStructure", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrStkMNStructure", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrStkMNStructure", "TcoMrefWidth"));
		var TcoFrlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrStkMNStructure", "TcoFrlWidth"));
		var TcoTolWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrStkMNStructure", "TcoTolWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoFrl"); doc.widths.push(TcoFrlWidth); doc.fs.push("frl"); wtot += TcoFrlWidth;
			} else if (i == 3) {
				doc.cols.push("TcoTol"); doc.widths.push(TcoTolWidth); doc.fs.push("tol"); wtot += TcoTolWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrStkMNStructure", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrStkMNStructure", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQStkMNStructure
		rownode = getRownode(srcdoc, "ListPlnrQStkMNStructure", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrStkMNStructure", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrStkMNStructure", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrStkMNStructure", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNStructure", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStructure_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStructure_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNStructure_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNStructure_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrStkMNStructure", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrStkMNStructure", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrStkMNStructure", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrStkMNStructure", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrStkMNStructureData", scrJref, "StgIacQryPlnrStkMNStructure");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrStkMNStructure", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrStkMNStructure", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrStkMNStructure", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrStkMNStructureDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "ContInfPlnrStkMNStructure", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StatAppPlnrStkMNStructure", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StatShrPlnrStkMNStructure", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StgIacPlnrStkMNStructure", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "TagPlnrStkMNStructure", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "ListPlnrQStkMNStructure", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StatAppQryPlnrStkMNStructure", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StatShrQryPlnrStkMNStructure", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrStkMNStructureData", "StgIacQryPlnrStkMNStructure", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrStkMNStructureData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrStkMNStructureData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNStructure");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrStkMNStructureData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

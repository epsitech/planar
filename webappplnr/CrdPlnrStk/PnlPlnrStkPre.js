/**
  * \file PnlPlnrStkPre.js
  * web client functionality for panel PnlPlnrStkPre
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrStkPre", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrStkPre", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("Pre_side").src = "./PnlPlnrStkPre_aside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrStkPre_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrStkPre", "srefIxPlnrVExpstate", "regd");

		// change content (container heights in refreshB)
		doc.getElementById("Pre_side").src = "./PnlPlnrStkPre_bside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrStkPre_b.html";
	};
};
...
function initA() {
	...
	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrStkPre", "Cpt"));
...
// IP initB --- BEGIN
	initCpt(hdrdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrStkPre", "Cpt"));
// IP initB --- END
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrStkPreData", "TagPlnrStkPre", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrStkPreData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrStkPreData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrStkPreData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

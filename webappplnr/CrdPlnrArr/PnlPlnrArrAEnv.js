/**
  * \file PnlPlnrArrAEnv.js
  * web client functionality for panel PnlPlnrArrAEnv
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrArrAEnv", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrArrAEnv", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrArrAEnv_aside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrArrAEnv_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrArrAEnv", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrArrAEnv_bside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrArrAEnv_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrAEnv", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrArrAEnv", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrAEnv", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrArrAEnv", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrAEnv", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrAEnv", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrAEnv", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrAEnv", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrAEnv", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrAEnv", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrArrAEnv", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrArrAEnv", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQArrAEnv
		rownode = getRownode(srcdoc, "ListPlnrQArrAEnv", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrArrAEnv", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrArrAEnv", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrArrAEnv", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrAEnv", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrAEnv", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrArrAEnv", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrArrAEnv", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrArrAEnvData", scrJref, "StgIacQryPlnrArrAEnv");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrAEnv", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrArrAEnv", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrAEnv", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArrAEnvDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "ContInfPlnrArrAEnv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StatAppPlnrArrAEnv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StatShrPlnrArrAEnv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StgIacPlnrArrAEnv", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "TagPlnrArrAEnv", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "ListPlnrQArrAEnv", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StatAppQryPlnrArrAEnv", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StatShrQryPlnrArrAEnv", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrAEnvData", "StgIacQryPlnrArrAEnv", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrArrAEnvData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrArrAEnvData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrArrAEnvData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrArrKEnvKey.js
  * web client functionality for panel PnlPlnrArrKEnvKey
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrArrKEnvKey", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrArrKEnvKey", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("KEnvKey", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("KEnvKey_side").setAttribute("height", "30");
		doc.getElementById("KEnvKey_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("KEnvKey_side").src = "./PnlPlnrArrKEnvKey_aside.html";
		doc.getElementById("KEnvKey_cont").src = "./PnlPlnrArrKEnvKey_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrArrKEnvKey", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("KEnvKey", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("KEnvKey_side").setAttribute("height", "255");
		doc.getElementById("KEnvKey_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("KEnvKey_side").src = "./PnlPlnrArrKEnvKey_bside.html";
		doc.getElementById("KEnvKey_cont").src = "./PnlPlnrArrKEnvKey_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KEnvKey_side").contentDocument;
	contdoc = doc.getElementById("KEnvKey_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("KEnvKey_side").contentDocument;
	contdoc = doc.getElementById("KEnvKey_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KEnvKey_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KEnvKey_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KEnvKey_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrKEnvKey", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrArrKEnvKey", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrKEnvKey", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrArrKEnvKey", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrKEnvKey", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrKEnvKey", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrKEnvKey", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrKEnvKey", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrKEnvKey", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrArrKEnvKey", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQArrKEnvKey
		rownode = getRownode(srcdoc, "ListPlnrQArrKEnvKey", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrArrKEnvKey", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrArrKEnvKey", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrArrKEnvKey", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KEnvKey", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KEnvKey_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KEnvKey_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KEnvKey_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KEnvKey_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrKEnvKey", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrKEnvKey", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrArrKEnvKey", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrArrKEnvKey", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrArrKEnvKeyData", scrJref, "StgIacQryPlnrArrKEnvKey");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrArrKEnvKey", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrKEnvKey", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArrKEnvKeyDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "ContInfPlnrArrKEnvKey", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StatAppPlnrArrKEnvKey", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StatShrPlnrArrKEnvKey", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StgIacPlnrArrKEnvKey", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "TagPlnrArrKEnvKey", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "ListPlnrQArrKEnvKey", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StatAppQryPlnrArrKEnvKey", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StatShrQryPlnrArrKEnvKey", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrKEnvKeyData", "StgIacQryPlnrArrKEnvKey", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrArrKEnvKeyData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrArrKEnvKeyData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KEnvKey");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrArrKEnvKeyData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

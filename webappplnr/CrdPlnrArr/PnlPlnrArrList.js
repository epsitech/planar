/**
  * \file PnlPlnrArrList.js
  * web client functionality for panel PnlPlnrArrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrArrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrArrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrArrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrArrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrArrList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrArrList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrArrList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrArrList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrArrList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArrList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrArrList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrArrList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrArrList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrArrList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoSrfWidth"));
		var TcoTpoWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoTpoWidth"));
		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoDsnWidth"));
		var TcoIWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoIWidth"));
		var TcoJWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoJWidth"));
		var TcoKWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoKWidth"));
		var TcoLWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoLWidth"));
		var TcoMWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoMWidth"));
		var TcoNWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArrList", "TcoNWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrArrList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTpo"); doc.widths.push(TcoTpoWidth); doc.fs.push("tpo"); doc.tos.push(2); wtot += TcoTpoWidth;
			} else if (i == 3) {
				doc.cols.push("TcoDsn"); doc.widths.push(TcoDsnWidth); doc.fs.push("dsn"); doc.tos.push(3); wtot += TcoDsnWidth;
			} else if (i == 4) {
				doc.cols.push("TcoI"); doc.widths.push(TcoIWidth); doc.fs.push("i"); doc.tos.push(0); wtot += TcoIWidth;
			} else if (i == 5) {
				doc.cols.push("TcoJ"); doc.widths.push(TcoJWidth); doc.fs.push("j"); doc.tos.push(0); wtot += TcoJWidth;
			} else if (i == 6) {
				doc.cols.push("TcoK"); doc.widths.push(TcoKWidth); doc.fs.push("k"); doc.tos.push(0); wtot += TcoKWidth;
			} else if (i == 7) {
				doc.cols.push("TcoL"); doc.widths.push(TcoLWidth); doc.fs.push("l"); doc.tos.push(0); wtot += TcoLWidth;
			} else if (i == 8) {
				doc.cols.push("TcoM"); doc.widths.push(TcoMWidth); doc.fs.push("m"); doc.tos.push(0); wtot += TcoMWidth;
			} else if (i == 9) {
				doc.cols.push("TcoN"); doc.widths.push(TcoNWidth); doc.fs.push("n"); doc.tos.push(0); wtot += TcoNWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrArrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrArrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQArrList
		rownode = getRownode(srcdoc, "ListPlnrQArrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrArrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrArrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrArrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrArrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrArrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrArrListData", scrJref, "StgIacQryPlnrArrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArrList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrArrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrArrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrArrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrArrListData", scrJref, "ContIacPlnrArrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "ContIacPlnrArrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "ContInfPlnrArrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "StatShrPlnrArrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "StgIacPlnrArrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "TagPlnrArrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "ListPlnrQArrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "StatAppQryPlnrArrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "StatShrQryPlnrArrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrArrListData", "StgIacQryPlnrArrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrArrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrArrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrArrListData") {
				mask = mergeDpchEngData(dom);

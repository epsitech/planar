/**
  * \file PnlPlnrArr1NArrayitem.js
  * web client functionality for panel PnlPlnrArr1NArrayitem
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrArr1NArrayitem", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrArr1NArrayitem", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("1NArrayitem", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("1NArrayitem_side").setAttribute("height", "30");
		doc.getElementById("1NArrayitem_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("1NArrayitem_side").src = "./PnlPlnrArr1NArrayitem_aside.html";
		doc.getElementById("1NArrayitem_cont").src = "./PnlPlnrArr1NArrayitem_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrArr1NArrayitem", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("1NArrayitem", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("1NArrayitem_side").setAttribute("height", "255");
		doc.getElementById("1NArrayitem_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("1NArrayitem_side").src = "./PnlPlnrArr1NArrayitem_bside.html";
		doc.getElementById("1NArrayitem_cont").src = "./PnlPlnrArr1NArrayitem_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NArrayitem_side").contentDocument;
	contdoc = doc.getElementById("1NArrayitem_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NArrayitem_side").contentDocument;
	contdoc = doc.getElementById("1NArrayitem_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NArrayitem_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NArrayitem_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NArrayitem_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrArr1NArrayitem", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArr1NArrayitem", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArr1NArrayitem", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArr1NArrayitem", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArr1NArrayitem", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrArr1NArrayitem", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQArr1NArrayitem
		rownode = getRownode(srcdoc, "ListPlnrQArr1NArrayitem", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrArr1NArrayitem", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrArr1NArrayitem", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NArrayitem", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NArrayitem_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NArrayitem_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NArrayitem_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NArrayitem_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArr1NArrayitem", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrArr1NArrayitem", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrArr1NArrayitem", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrArr1NArrayitem", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrArr1NArrayitemData", scrJref, "StgIacQryPlnrArr1NArrayitem");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrArr1NArrayitem", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrArr1NArrayitem", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArr1NArrayitemDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "ContInfPlnrArr1NArrayitem", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StatAppPlnrArr1NArrayitem", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StatShrPlnrArr1NArrayitem", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StgIacPlnrArr1NArrayitem", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "TagPlnrArr1NArrayitem", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "ListPlnrQArr1NArrayitem", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StatAppQryPlnrArr1NArrayitem", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StatShrQryPlnrArr1NArrayitem", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrArr1NArrayitemData", "StgIacQryPlnrArr1NArrayitem", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrArr1NArrayitemData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrArr1NArrayitemData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NArrayitem");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrArr1NArrayitemData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

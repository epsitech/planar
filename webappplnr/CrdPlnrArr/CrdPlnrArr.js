/**
  * \file CrdPlnrArr.js
  * web client functionality for card CrdPlnrArr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppPlnrArr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppPlnrArr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppPlnrArr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppPlnrArr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppPlnrArr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlPlnrArrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlPlnrArrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlPlnrArrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppPlnrArr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppPlnrArr", "shortMenu", short);
	setSi(srcdoc, "StatAppPlnrArr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppPlnrArr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrArr", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagPlnrArr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagPlnrArr", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdArr"), retrieveCi(srcdoc, "ContInfPlnrArr", "MtxCrdArr"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfPlnrArr", "MtxCrdArr") + " - Planar 0.1.22";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArrDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrArrDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, ctlsref) {
	window.open(retrieveCi(srcdoc, "ContInfPlnrArr", ctlsref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrArrData", "ContInfPlnrArr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrArrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngPlnrArrData", "StatAppPlnrArr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrArrData", "StatShrPlnrArr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrArrData", "TagPlnrArr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrArrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrPlnrArr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrPlnrArr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrPlnrArr", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrArrData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrArr", "srefIxPlnrVReqitmode");

	if (srefIxPlnrVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxPlnrVReqitmode = "notify";
		setSi(srcdoc, "StatAppPlnrArr", "srefIxPlnrVReqitmode", srefIxPlnrVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrArr", "srefIxPlnrVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppPlnrArr", "latency"));

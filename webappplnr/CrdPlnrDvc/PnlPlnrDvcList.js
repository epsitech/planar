/**
  * \file PnlPlnrDvcList.js
  * web client functionality for panel PnlPlnrDvcList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrDvcList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrDvcList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrDvcList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrDvcList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDvcList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDvcList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrDvcList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrDvcList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrDvcList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDvcList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDvcList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrDvcList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrDvcList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrDvcList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoSrfWidth"));
		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoDsnWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoSupWidth"));
		var TcoTplWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoTplWidth"));
		var TcoMtyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoMtyWidth"));
		var TcoAtyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcList", "TcoAtyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrDvcList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDsn"); doc.widths.push(TcoDsnWidth); doc.fs.push("dsn"); doc.tos.push(2); wtot += TcoDsnWidth;
			} else if (i == 3) {
				doc.cols.push("TcoSup"); doc.widths.push(TcoSupWidth); doc.fs.push("sup"); doc.tos.push(3); wtot += TcoSupWidth;
			} else if (i == 4) {
				doc.cols.push("TcoTpl"); doc.widths.push(TcoTplWidth); doc.fs.push("tpl"); doc.tos.push(4); wtot += TcoTplWidth;
			} else if (i == 5) {
				doc.cols.push("TcoMty"); doc.widths.push(TcoMtyWidth); doc.fs.push("mty"); doc.tos.push(0); wtot += TcoMtyWidth;
			} else if (i == 6) {
				doc.cols.push("TcoAty"); doc.widths.push(TcoAtyWidth); doc.fs.push("aty"); doc.tos.push(0); wtot += TcoAtyWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrDvcList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDvcList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDvcList
		rownode = getRownode(srcdoc, "ListPlnrQDvcList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDvcList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDvcList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDvcList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDvcList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDvcList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDvcListData", scrJref, "StgIacQryPlnrDvcList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDvcList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDvcListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrDvcList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrDvcList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDvcListData", scrJref, "ContIacPlnrDvcList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "ContIacPlnrDvcList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "ContInfPlnrDvcList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "StatShrPlnrDvcList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "StgIacPlnrDvcList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "TagPlnrDvcList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "ListPlnrQDvcList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "StatAppQryPlnrDvcList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "StatShrQryPlnrDvcList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDvcListData", "StgIacQryPlnrDvcList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrDvcListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDvcListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDvcListData") {
				mask = mergeDpchEngData(dom);

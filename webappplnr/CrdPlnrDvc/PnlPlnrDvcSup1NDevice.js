/**
  * \file PnlPlnrDvcSup1NDevice.js
  * web client functionality for panel PnlPlnrDvcSup1NDevice
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDvcSup1NDevice", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDvcSup1NDevice", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("Sup1NDevice_side").src = "./PnlPlnrDvcSup1NDevice_aside.html";
		doc.getElementById("Sup1NDevice_cont").src = "./PnlPlnrDvcSup1NDevice_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDvcSup1NDevice", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("Sup1NDevice_side").src = "./PnlPlnrDvcSup1NDevice_bside.html";
		doc.getElementById("Sup1NDevice_cont").src = "./PnlPlnrDvcSup1NDevice_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDvcSup1NDevice", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcSup1NDevice", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcSup1NDevice", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcSup1NDevice", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcSup1NDevice", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDvcSup1NDevice", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDvcSup1NDevice
		rownode = getRownode(srcdoc, "ListPlnrQDvcSup1NDevice", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDvcSup1NDevice", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDvcSup1NDevice", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcSup1NDevice", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDvcSup1NDevice", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDvcSup1NDevice", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDvcSup1NDevice", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDvcSup1NDeviceData", scrJref, "StgIacQryPlnrDvcSup1NDevice");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDvcSup1NDevice", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDvcSup1NDevice", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDvcSup1NDeviceDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "ContInfPlnrDvcSup1NDevice", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StatAppPlnrDvcSup1NDevice", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StatShrPlnrDvcSup1NDevice", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StgIacPlnrDvcSup1NDevice", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "TagPlnrDvcSup1NDevice", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "ListPlnrQDvcSup1NDevice", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StatAppQryPlnrDvcSup1NDevice", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StatShrQryPlnrDvcSup1NDevice", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDvcSup1NDeviceData", "StgIacQryPlnrDvcSup1NDevice", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDvcSup1NDeviceData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDvcSup1NDeviceData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDvcSup1NDeviceData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

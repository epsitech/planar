/**
  * \file CrdPlnrDvc.js
  * web client functionality for card CrdPlnrDvc
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppPlnrDvc", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppPlnrDvc", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppPlnrDvc", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppPlnrDvc", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppPlnrDvc", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlPlnrDvcHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlPlnrDvcList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlPlnrDvcRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppPlnrDvc", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppPlnrDvc", "shortMenu", short);
	setSi(srcdoc, "StatAppPlnrDvc", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppPlnrDvc", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrDvc", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagPlnrDvc", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagPlnrDvc", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdDvc"), retrieveCi(srcdoc, "ContInfPlnrDvc", "MtxCrdDvc"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfPlnrDvc", "MtxCrdDvc") + " - Planar 0.1.22";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDvcDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDvcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, ctlsref) {
	window.open(retrieveCi(srcdoc, "ContInfPlnrDvc", ctlsref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDvcData", "ContInfPlnrDvc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDvcData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngPlnrDvcData", "StatAppPlnrDvc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDvcData", "StatShrPlnrDvc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDvcData", "TagPlnrDvc", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDvcData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrPlnrDvc", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrPlnrDvc", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrPlnrDvc", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDvcData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrDvc", "srefIxPlnrVReqitmode");

	if (srefIxPlnrVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxPlnrVReqitmode = "notify";
		setSi(srcdoc, "StatAppPlnrDvc", "srefIxPlnrVReqitmode", srefIxPlnrVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrDvc", "srefIxPlnrVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppPlnrDvc", "latency"));

/**
  * \file PnlPlnrDvcPre.js
  * web client functionality for panel PnlPlnrDvcPre
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDvcPre", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDvcPre", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("Pre_side").src = "./PnlPlnrDvcPre_aside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrDvcPre_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDvcPre", "srefIxPlnrVExpstate", "regd");

		// change content (container heights in refreshB)
		doc.getElementById("Pre_side").src = "./PnlPlnrDvcPre_bside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrDvcPre_b.html";
	};
};
...
function initA() {
	...
	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrDvcPre", "Cpt"));
...
// IP initB --- BEGIN
	initCpt(hdrdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrDvcPre", "Cpt"));
// IP initB --- END
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDvcPreData", "TagPlnrDvcPre", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDvcPreData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDvcPreData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDvcPreData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

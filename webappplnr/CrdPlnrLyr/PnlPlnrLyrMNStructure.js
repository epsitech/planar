/**
  * \file PnlPlnrLyrMNStructure.js
  * web client functionality for panel PnlPlnrLyrMNStructure
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrLyrMNStructure", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrLyrMNStructure", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("MNStructure_side").src = "./PnlPlnrLyrMNStructure_aside.html";
		doc.getElementById("MNStructure_cont").src = "./PnlPlnrLyrMNStructure_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrLyrMNStructure", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("MNStructure_side").src = "./PnlPlnrLyrMNStructure_bside.html";
		doc.getElementById("MNStructure_cont").src = "./PnlPlnrLyrMNStructure_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrLyrMNStructure", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrLyrMNStructure", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrLyrMNStructure", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrLyrMNStructure", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrMNStructure", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrMNStructure", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrMNStructure", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrMNStructure", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrMNStructure", "TcoMrefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrLyrMNStructure", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQLyrMNStructure
		rownode = getRownode(srcdoc, "ListPlnrQLyrMNStructure", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrLyrMNStructure", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrLyrMNStructure", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrLyrMNStructure", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrMNStructure", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrMNStructure", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrLyrMNStructure", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrLyrMNStructure", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrLyrMNStructureData", scrJref, "StgIacQryPlnrLyrMNStructure");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrLyrMNStructure", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrMNStructure", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrLyrMNStructureDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "ContInfPlnrLyrMNStructure", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StatAppPlnrLyrMNStructure", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StatShrPlnrLyrMNStructure", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StgIacPlnrLyrMNStructure", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "TagPlnrLyrMNStructure", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "ListPlnrQLyrMNStructure", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StatAppQryPlnrLyrMNStructure", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StatShrQryPlnrLyrMNStructure", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrLyrMNStructureData", "StgIacQryPlnrLyrMNStructure", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrLyrMNStructureData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrLyrMNStructureData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrLyrMNStructureData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

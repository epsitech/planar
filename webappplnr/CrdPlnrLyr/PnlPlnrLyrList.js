/**
  * \file PnlPlnrLyrList.js
  * web client functionality for panel PnlPlnrLyrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrLyrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrLyrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrLyrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrLyrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrLyrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrLyrList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrLyrList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrLyrList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrLyrList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrLyrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrLyrList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrLyrList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrLyrList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrLyrList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoSrfWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoTypWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoHkuWidth"));
		var TcoMatWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoMatWidth"));
		var TcoTtyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrLyrList", "TcoTtyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrLyrList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ"); doc.tos.push(2); wtot += TcoTypWidth;
			} else if (i == 3) {
				doc.cols.push("TcoHkt"); doc.widths.push(TcoHktWidth); doc.fs.push("hkt"); doc.tos.push(3); wtot += TcoHktWidth;
			} else if (i == 4) {
				doc.cols.push("TcoHku"); doc.widths.push(TcoHkuWidth); doc.fs.push("hku"); doc.tos.push(4); wtot += TcoHkuWidth;
			} else if (i == 5) {
				doc.cols.push("TcoMat"); doc.widths.push(TcoMatWidth); doc.fs.push("mat"); doc.tos.push(5); wtot += TcoMatWidth;
			} else if (i == 6) {
				doc.cols.push("TcoTty"); doc.widths.push(TcoTtyWidth); doc.fs.push("tty"); doc.tos.push(6); wtot += TcoTtyWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrLyrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrLyrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQLyrList
		rownode = getRownode(srcdoc, "ListPlnrQLyrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrLyrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrLyrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrLyrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrLyrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrLyrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrLyrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrLyrListData", scrJref, "StgIacQryPlnrLyrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrLyrList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrLyrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrLyrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrLyrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrLyrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrLyrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrLyrListData", scrJref, "ContIacPlnrLyrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "ContIacPlnrLyrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "ContInfPlnrLyrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "StatShrPlnrLyrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "StgIacPlnrLyrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "TagPlnrLyrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "ListPlnrQLyrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "StatAppQryPlnrLyrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "StatShrQryPlnrLyrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrLyrListData", "StgIacQryPlnrLyrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrLyrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrLyrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrLyrListData") {
				mask = mergeDpchEngData(dom);

/**
  * \file PnlPlnrDsn1NBlock.js
  * web client functionality for panel PnlPlnrDsn1NBlock
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsn1NBlock", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsn1NBlock", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("1NBlock_side").src = "./PnlPlnrDsn1NBlock_aside.html";
		doc.getElementById("1NBlock_cont").src = "./PnlPlnrDsn1NBlock_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsn1NBlock", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("1NBlock_side").src = "./PnlPlnrDsn1NBlock_bside.html";
		doc.getElementById("1NBlock_cont").src = "./PnlPlnrDsn1NBlock_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsn1NBlock", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsn1NBlock", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsn1NBlock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsn1NBlock", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsn1NBlock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsn1NBlock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsn1NBlock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsn1NBlock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsn1NBlock", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsn1NBlock", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsn1NBlock
		rownode = getRownode(srcdoc, "ListPlnrQDsn1NBlock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsn1NBlock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsn1NBlock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsn1NBlock", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsn1NBlock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsn1NBlock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsn1NBlock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsn1NBlock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsn1NBlockData", scrJref, "StgIacQryPlnrDsn1NBlock");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsn1NBlock", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsn1NBlock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsn1NBlockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "ContInfPlnrDsn1NBlock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StatAppPlnrDsn1NBlock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StatShrPlnrDsn1NBlock", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StgIacPlnrDsn1NBlock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "TagPlnrDsn1NBlock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "ListPlnrQDsn1NBlock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StatAppQryPlnrDsn1NBlock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StatShrQryPlnrDsn1NBlock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsn1NBlockData", "StgIacQryPlnrDsn1NBlock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsn1NBlockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsn1NBlockData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsn1NBlockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

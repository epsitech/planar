/**
  * \file PnlPlnrDsnHk1NLineguide.js
  * web client functionality for panel PnlPlnrDsnHk1NLineguide
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnHk1NLineguide", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnHk1NLineguide", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Hk1NLineguide", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Hk1NLineguide_side").setAttribute("height", "30");
		doc.getElementById("Hk1NLineguide_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Hk1NLineguide_side").src = "./PnlPlnrDsnHk1NLineguide_aside.html";
		doc.getElementById("Hk1NLineguide_cont").src = "./PnlPlnrDsnHk1NLineguide_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnHk1NLineguide", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Hk1NLineguide", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Hk1NLineguide_side").setAttribute("height", "255");
		doc.getElementById("Hk1NLineguide_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Hk1NLineguide_side").src = "./PnlPlnrDsnHk1NLineguide_bside.html";
		doc.getElementById("Hk1NLineguide_cont").src = "./PnlPlnrDsnHk1NLineguide_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Hk1NLineguide_side").contentDocument;
	contdoc = doc.getElementById("Hk1NLineguide_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Hk1NLineguide_side").contentDocument;
	contdoc = doc.getElementById("Hk1NLineguide_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Hk1NLineguide_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Hk1NLineguide_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Hk1NLineguide_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnHk1NLineguide", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnHk1NLineguide", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnHk1NLineguide", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnHk1NLineguide", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnHk1NLineguide", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnHk1NLineguide", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnHk1NLineguide
		rownode = getRownode(srcdoc, "ListPlnrQDsnHk1NLineguide", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnHk1NLineguide", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnHk1NLineguide", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Hk1NLineguide", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Hk1NLineguide_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Hk1NLineguide_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Hk1NLineguide_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Hk1NLineguide_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnHk1NLineguide", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnHk1NLineguide", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnHk1NLineguide", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnHk1NLineguide", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnHk1NLineguideData", scrJref, "StgIacQryPlnrDsnHk1NLineguide");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnHk1NLineguide", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnHk1NLineguide", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnHk1NLineguideDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "ContInfPlnrDsnHk1NLineguide", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StatAppPlnrDsnHk1NLineguide", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StatShrPlnrDsnHk1NLineguide", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StgIacPlnrDsnHk1NLineguide", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "TagPlnrDsnHk1NLineguide", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "ListPlnrQDsnHk1NLineguide", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StatAppQryPlnrDsnHk1NLineguide", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StatShrQryPlnrDsnHk1NLineguide", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnHk1NLineguideData", "StgIacQryPlnrDsnHk1NLineguide", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnHk1NLineguideData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnHk1NLineguideData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Hk1NLineguide");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnHk1NLineguideData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

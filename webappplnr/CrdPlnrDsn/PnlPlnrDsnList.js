/**
  * \file PnlPlnrDsnList.js
  * web client functionality for panel PnlPlnrDsnList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrDsnList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrDsnList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrDsnList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrDsnList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrDsnList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrDsnList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrDsnList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrDsnList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrDsnList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrDsnList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoTitWidth"));
		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoTypWidth"));
		var TcoDimWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoDimWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoReuWidth"));
		var TcoPrjWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoPrjWidth"));
		var TcoMtyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoMtyWidth"));
		var TcoModWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoModWidth"));
		var TcoDrvWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoDrvWidth"));
		var TcoMajWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoMajWidth"));
		var TcoSubWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnList", "TcoSubWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrDsnList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoTyp"); doc.widths.push(TcoTypWidth); doc.fs.push("typ"); doc.tos.push(1); wtot += TcoTypWidth;
			} else if (i == 3) {
				doc.cols.push("TcoDim"); doc.widths.push(TcoDimWidth); doc.fs.push("dim"); doc.tos.push(2); wtot += TcoDimWidth;
			} else if (i == 4) {
				doc.cols.push("TcoRet"); doc.widths.push(TcoRetWidth); doc.fs.push("ret"); doc.tos.push(3); wtot += TcoRetWidth;
			} else if (i == 5) {
				...
				doc.cols.push("TcoMty"); doc.widths.push(TcoMtyWidth); doc.fs.push("mty"); doc.tos.push(6); wtot += TcoMtyWidth;
			} else if (i == 8) {
				doc.cols.push("TcoMod"); doc.widths.push(TcoModWidth); doc.fs.push("mod"); doc.tos.push(7); wtot += TcoModWidth;
			} else if (i == 9) {
				doc.cols.push("TcoDrv"); doc.widths.push(TcoDrvWidth); doc.fs.push("drv"); doc.tos.push(8); wtot += TcoDrvWidth;
			} else if (i == 10) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrDsnList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnList
		rownode = getRownode(srcdoc, "ListPlnrQDsnList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnListData", scrJref, "StgIacQryPlnrDsnList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrDsnList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrDsnList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnListData", scrJref, "ContIacPlnrDsnList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "ContIacPlnrDsnList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "ContInfPlnrDsnList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "StatShrPlnrDsnList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "StgIacPlnrDsnList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "TagPlnrDsnList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "ListPlnrQDsnList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "StatAppQryPlnrDsnList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "StatShrQryPlnrDsnList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnListData", "StgIacQryPlnrDsnList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrDsnListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnListData") {
				mask = mergeDpchEngData(dom);

/**
  * \file PnlPlnrDsnDsn1NDevice.js
  * web client functionality for panel PnlPlnrDsnDsn1NDevice
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnDsn1NDevice", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NDevice", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Dsn1NDevice", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Dsn1NDevice_side").setAttribute("height", "30");
		doc.getElementById("Dsn1NDevice_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Dsn1NDevice_side").src = "./PnlPlnrDsnDsn1NDevice_aside.html";
		doc.getElementById("Dsn1NDevice_cont").src = "./PnlPlnrDsnDsn1NDevice_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NDevice", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Dsn1NDevice", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Dsn1NDevice_side").setAttribute("height", "255");
		doc.getElementById("Dsn1NDevice_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Dsn1NDevice_side").src = "./PnlPlnrDsnDsn1NDevice_bside.html";
		doc.getElementById("Dsn1NDevice_cont").src = "./PnlPlnrDsnDsn1NDevice_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Dsn1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NDevice_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Dsn1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NDevice_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Dsn1NDevice_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Dsn1NDevice_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Dsn1NDevice_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDevice", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDevice", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDevice", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDevice", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDevice", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnDsn1NDevice", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnDsn1NDevice
		rownode = getRownode(srcdoc, "ListPlnrQDsnDsn1NDevice", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnDsn1NDevice", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnDsn1NDevice", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Dsn1NDevice", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NDevice_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NDevice_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Dsn1NDevice_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Dsn1NDevice_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDevice", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDevice", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnDsn1NDevice", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnDsn1NDevice", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnDsn1NDeviceData", scrJref, "StgIacQryPlnrDsnDsn1NDevice");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnDsn1NDevice", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDevice", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnDsn1NDeviceDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "ContInfPlnrDsnDsn1NDevice", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StatAppPlnrDsnDsn1NDevice", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StatShrPlnrDsnDsn1NDevice", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StgIacPlnrDsnDsn1NDevice", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "TagPlnrDsnDsn1NDevice", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "ListPlnrQDsnDsn1NDevice", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StatAppQryPlnrDsnDsn1NDevice", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StatShrQryPlnrDsnDsn1NDevice", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDeviceData", "StgIacQryPlnrDsnDsn1NDevice", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnDsn1NDeviceData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnDsn1NDeviceData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Dsn1NDevice");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnDsn1NDeviceData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

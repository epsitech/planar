/**
  * \file PnlPlnrDsnMod1NDesign.js
  * web client functionality for panel PnlPlnrDsnMod1NDesign
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnMod1NDesign", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnMod1NDesign", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Mod1NDesign", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Mod1NDesign_side").setAttribute("height", "30");
		doc.getElementById("Mod1NDesign_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Mod1NDesign_side").src = "./PnlPlnrDsnMod1NDesign_aside.html";
		doc.getElementById("Mod1NDesign_cont").src = "./PnlPlnrDsnMod1NDesign_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnMod1NDesign", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Mod1NDesign", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Mod1NDesign_side").setAttribute("height", "255");
		doc.getElementById("Mod1NDesign_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Mod1NDesign_side").src = "./PnlPlnrDsnMod1NDesign_bside.html";
		doc.getElementById("Mod1NDesign_cont").src = "./PnlPlnrDsnMod1NDesign_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Mod1NDesign_side").contentDocument;
	contdoc = doc.getElementById("Mod1NDesign_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Mod1NDesign_side").contentDocument;
	contdoc = doc.getElementById("Mod1NDesign_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Mod1NDesign_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Mod1NDesign_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Mod1NDesign_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnMod1NDesign", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnMod1NDesign", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnMod1NDesign", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnMod1NDesign", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnMod1NDesign", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnMod1NDesign", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnMod1NDesign
		rownode = getRownode(srcdoc, "ListPlnrQDsnMod1NDesign", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnMod1NDesign", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnMod1NDesign", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Mod1NDesign", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Mod1NDesign_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Mod1NDesign_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Mod1NDesign_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Mod1NDesign_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnMod1NDesign", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnMod1NDesign", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnMod1NDesign", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnMod1NDesign", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnMod1NDesignData", scrJref, "StgIacQryPlnrDsnMod1NDesign");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnMod1NDesign", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnMod1NDesign", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnMod1NDesignDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "ContInfPlnrDsnMod1NDesign", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StatAppPlnrDsnMod1NDesign", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StatShrPlnrDsnMod1NDesign", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StgIacPlnrDsnMod1NDesign", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "TagPlnrDsnMod1NDesign", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "ListPlnrQDsnMod1NDesign", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StatAppQryPlnrDsnMod1NDesign", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StatShrQryPlnrDsnMod1NDesign", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnMod1NDesignData", "StgIacQryPlnrDsnMod1NDesign", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnMod1NDesignData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnMod1NDesignData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Mod1NDesign");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnMod1NDesignData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrDsnDsn1NStack.js
  * web client functionality for panel PnlPlnrDsnDsn1NStack
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnDsn1NStack", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NStack", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Dsn1NStack", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Dsn1NStack_side").setAttribute("height", "30");
		doc.getElementById("Dsn1NStack_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Dsn1NStack_side").src = "./PnlPlnrDsnDsn1NStack_aside.html";
		doc.getElementById("Dsn1NStack_cont").src = "./PnlPlnrDsnDsn1NStack_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NStack", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Dsn1NStack", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Dsn1NStack_side").setAttribute("height", "255");
		doc.getElementById("Dsn1NStack_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Dsn1NStack_side").src = "./PnlPlnrDsnDsn1NStack_bside.html";
		doc.getElementById("Dsn1NStack_cont").src = "./PnlPlnrDsnDsn1NStack_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Dsn1NStack_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NStack_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Dsn1NStack_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NStack_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Dsn1NStack_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Dsn1NStack_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Dsn1NStack_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NStack", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NStack", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NStack", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NStack", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NStack", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnDsn1NStack", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnDsn1NStack
		rownode = getRownode(srcdoc, "ListPlnrQDsnDsn1NStack", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnDsn1NStack", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnDsn1NStack", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Dsn1NStack", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NStack_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NStack_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Dsn1NStack_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Dsn1NStack_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NStack", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NStack", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnDsn1NStack", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnDsn1NStack", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnDsn1NStackData", scrJref, "StgIacQryPlnrDsnDsn1NStack");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnDsn1NStack", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NStack", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnDsn1NStackDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "ContInfPlnrDsnDsn1NStack", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StatAppPlnrDsnDsn1NStack", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StatShrPlnrDsnDsn1NStack", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StgIacPlnrDsnDsn1NStack", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "TagPlnrDsnDsn1NStack", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "ListPlnrQDsnDsn1NStack", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StatAppQryPlnrDsnDsn1NStack", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StatShrQryPlnrDsnDsn1NStack", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NStackData", "StgIacQryPlnrDsnDsn1NStack", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnDsn1NStackData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnDsn1NStackData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Dsn1NStack");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnDsn1NStackData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

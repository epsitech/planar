/**
  * \file PnlPlnrDsnDsn1NDomain.js
  * web client functionality for panel PnlPlnrDsnDsn1NDomain
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnDsn1NDomain", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NDomain", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Dsn1NDomain", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Dsn1NDomain_side").setAttribute("height", "30");
		doc.getElementById("Dsn1NDomain_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Dsn1NDomain_side").src = "./PnlPlnrDsnDsn1NDomain_aside.html";
		doc.getElementById("Dsn1NDomain_cont").src = "./PnlPlnrDsnDsn1NDomain_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnDsn1NDomain", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Dsn1NDomain", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Dsn1NDomain_side").setAttribute("height", "255");
		doc.getElementById("Dsn1NDomain_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Dsn1NDomain_side").src = "./PnlPlnrDsnDsn1NDomain_bside.html";
		doc.getElementById("Dsn1NDomain_cont").src = "./PnlPlnrDsnDsn1NDomain_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Dsn1NDomain_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NDomain_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Dsn1NDomain_side").contentDocument;
	contdoc = doc.getElementById("Dsn1NDomain_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Dsn1NDomain_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Dsn1NDomain_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Dsn1NDomain_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDomain", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDomain", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDomain", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDomain", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDomain", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnDsn1NDomain", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnDsn1NDomain
		rownode = getRownode(srcdoc, "ListPlnrQDsnDsn1NDomain", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnDsn1NDomain", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnDsn1NDomain", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Dsn1NDomain", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NDomain_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Dsn1NDomain_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Dsn1NDomain_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Dsn1NDomain_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDomain", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDsn1NDomain", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnDsn1NDomain", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnDsn1NDomain", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnDsn1NDomainData", scrJref, "StgIacQryPlnrDsnDsn1NDomain");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnDsn1NDomain", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDsn1NDomain", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnDsn1NDomainDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "ContInfPlnrDsnDsn1NDomain", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StatAppPlnrDsnDsn1NDomain", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StatShrPlnrDsnDsn1NDomain", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StgIacPlnrDsnDsn1NDomain", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "TagPlnrDsnDsn1NDomain", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "ListPlnrQDsnDsn1NDomain", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StatAppQryPlnrDsnDsn1NDomain", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StatShrQryPlnrDsnDsn1NDomain", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDsn1NDomainData", "StgIacQryPlnrDsnDsn1NDomain", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnDsn1NDomainData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnDsn1NDomainData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Dsn1NDomain");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnDsn1NDomainData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

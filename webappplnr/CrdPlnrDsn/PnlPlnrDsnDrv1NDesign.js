/**
  * \file PnlPlnrDsnDrv1NDesign.js
  * web client functionality for panel PnlPlnrDsnDrv1NDesign
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDsnDrv1NDesign", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDsnDrv1NDesign", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Drv1NDesign", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Drv1NDesign_side").setAttribute("height", "30");
		doc.getElementById("Drv1NDesign_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Drv1NDesign_side").src = "./PnlPlnrDsnDrv1NDesign_aside.html";
		doc.getElementById("Drv1NDesign_cont").src = "./PnlPlnrDsnDrv1NDesign_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDsnDrv1NDesign", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Drv1NDesign", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Drv1NDesign_side").setAttribute("height", "255");
		doc.getElementById("Drv1NDesign_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Drv1NDesign_side").src = "./PnlPlnrDsnDrv1NDesign_bside.html";
		doc.getElementById("Drv1NDesign_cont").src = "./PnlPlnrDsnDrv1NDesign_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Drv1NDesign_side").contentDocument;
	contdoc = doc.getElementById("Drv1NDesign_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Drv1NDesign_side").contentDocument;
	contdoc = doc.getElementById("Drv1NDesign_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Drv1NDesign_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Drv1NDesign_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Drv1NDesign_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDsnDrv1NDesign", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDrv1NDesign", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDrv1NDesign", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDrv1NDesign", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDrv1NDesign", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDsnDrv1NDesign", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDsnDrv1NDesign
		rownode = getRownode(srcdoc, "ListPlnrQDsnDrv1NDesign", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDsnDrv1NDesign", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDsnDrv1NDesign", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Drv1NDesign", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Drv1NDesign_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Drv1NDesign_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Drv1NDesign_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Drv1NDesign_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDrv1NDesign", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDsnDrv1NDesign", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDsnDrv1NDesign", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDsnDrv1NDesign", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDsnDrv1NDesignData", scrJref, "StgIacQryPlnrDsnDrv1NDesign");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDsnDrv1NDesign", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDsnDrv1NDesign", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDsnDrv1NDesignDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "ContInfPlnrDsnDrv1NDesign", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StatAppPlnrDsnDrv1NDesign", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StatShrPlnrDsnDrv1NDesign", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StgIacPlnrDsnDrv1NDesign", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "TagPlnrDsnDrv1NDesign", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "ListPlnrQDsnDrv1NDesign", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StatAppQryPlnrDsnDrv1NDesign", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StatShrQryPlnrDsnDrv1NDesign", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDsnDrv1NDesignData", "StgIacQryPlnrDsnDrv1NDesign", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDsnDrv1NDesignData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDsnDrv1NDesignData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Drv1NDesign");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDsnDrv1NDesignData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

...
function refreshLst(lstdoc, srcdoc, multsel, blk, numFirstdisp, numsSel) {
	...
	for (var i=1;i<=5;i++) {
		...
		res = srcdoc.evaluate("//plnr:" + blk + "/plnr:Fi[@num='" + num + "']", srcdoc, nsResolver, XPathResult.ANY_TYPE, null);
		...
		if (finode) {
			...
			txt = retrieveFeedTitle1ByNum(srcdoc, blk, num);

			if ((selclass == "sel") && (txt.length > Math.ceil(324*0.125))) {
				txt = txt.substring(0, Math.ceil(324*0.125) -3) + "...";
			} else if ((selclass != "sel") && (txt.length > Math.ceil(324*0.15))) {
				txt = txt.substring(0, Math.ceil(324*0.15) -3) + "...";
			};

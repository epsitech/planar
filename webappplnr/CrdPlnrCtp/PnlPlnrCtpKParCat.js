/**
  * \file PnlPlnrCtpKParCat.js
  * web client functionality for panel PnlPlnrCtpKParCat
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpKParCat", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpKParCat", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("KParCat", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("KParCat_side").setAttribute("height", "30");
		doc.getElementById("KParCat_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("KParCat_side").src = "./PnlPlnrCtpKParCat_aside.html";
		doc.getElementById("KParCat_cont").src = "./PnlPlnrCtpKParCat_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpKParCat", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("KParCat", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("KParCat_side").setAttribute("height", "255");
		doc.getElementById("KParCat_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("KParCat_side").src = "./PnlPlnrCtpKParCat_bside.html";
		doc.getElementById("KParCat_cont").src = "./PnlPlnrCtpKParCat_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KParCat_side").contentDocument;
	contdoc = doc.getElementById("KParCat_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("KParCat_side").contentDocument;
	contdoc = doc.getElementById("KParCat_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KParCat_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KParCat_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KParCat_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKParCat", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpKParCat", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKParCat", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpKParCat", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParCat", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParCat", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParCat", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParCat", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParCat", "TcoSrfWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParCat", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParCat", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpKParCat", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpKParCat", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpKParCat
		rownode = getRownode(srcdoc, "ListPlnrQCtpKParCat", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpKParCat", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpKParCat", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpKParCat", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KParCat", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KParCat_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KParCat_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KParCat_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KParCat_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParCat", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParCat", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpKParCat", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpKParCat", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpKParCatData", scrJref, "StgIacQryPlnrCtpKParCat");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParCat", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpKParCat", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParCat", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpKParCatDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "ContInfPlnrCtpKParCat", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StatAppPlnrCtpKParCat", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StatShrPlnrCtpKParCat", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StgIacPlnrCtpKParCat", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "TagPlnrCtpKParCat", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "ListPlnrQCtpKParCat", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StatAppQryPlnrCtpKParCat", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StatShrQryPlnrCtpKParCat", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParCatData", "StgIacQryPlnrCtpKParCat", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpKParCatData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpKParCatData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KParCat");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpKParCatData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrCtpAEnv.js
  * web client functionality for panel PnlPlnrCtpAEnv
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpAEnv", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpAEnv", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrCtpAEnv_aside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrCtpAEnv_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpAEnv", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrCtpAEnv_bside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrCtpAEnv_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpAEnv", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpAEnv", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpAEnv", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpAEnv", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAEnv", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAEnv", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAEnv", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAEnv", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpAEnv", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpAEnv", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpAEnv", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpAEnv", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpAEnv
		rownode = getRownode(srcdoc, "ListPlnrQCtpAEnv", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpAEnv", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpAEnv", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpAEnv", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAEnv", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAEnv", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpAEnv", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpAEnv", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpAEnvData", scrJref, "StgIacQryPlnrCtpAEnv");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAEnv", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpAEnv", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAEnv", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpAEnvDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "ContInfPlnrCtpAEnv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StatAppPlnrCtpAEnv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StatShrPlnrCtpAEnv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StgIacPlnrCtpAEnv", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "TagPlnrCtpAEnv", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "ListPlnrQCtpAEnv", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StatAppQryPlnrCtpAEnv", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StatShrQryPlnrCtpAEnv", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAEnvData", "StgIacQryPlnrCtpAEnv", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpAEnvData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpAEnvData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpAEnvData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

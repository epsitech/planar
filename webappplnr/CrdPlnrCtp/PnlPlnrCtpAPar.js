/**
  * \file PnlPlnrCtpAPar.js
  * web client functionality for panel PnlPlnrCtpAPar
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpAPar", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpAPar", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrCtpAPar_aside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrCtpAPar_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpAPar", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrCtpAPar_bside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrCtpAPar_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpAPar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpAPar", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpAPar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "ncol"));

		var TcoCatWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpAPar", "TcoCatWidth"));
		var TcoPrpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpAPar", "TcoPrpWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpAPar", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpAPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpAPar
		rownode = getRownode(srcdoc, "ListPlnrQCtpAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpAPar", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpAParData", scrJref, "StgIacQryPlnrCtpAPar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpAPar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "ContInfPlnrCtpAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StatAppPlnrCtpAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StatShrPlnrCtpAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StgIacPlnrCtpAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "TagPlnrCtpAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "ListPlnrQCtpAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StatAppQryPlnrCtpAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StatShrQryPlnrCtpAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpAParData", "StgIacQryPlnrCtpAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrCtpKParProp.js
  * web client functionality for panel PnlPlnrCtpKParProp
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpKParProp", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpKParProp", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("KParProp", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("KParProp_side").setAttribute("height", "30");
		doc.getElementById("KParProp_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("KParProp_side").src = "./PnlPlnrCtpKParProp_aside.html";
		doc.getElementById("KParProp_cont").src = "./PnlPlnrCtpKParProp_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpKParProp", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("KParProp", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("KParProp_side").setAttribute("height", "255");
		doc.getElementById("KParProp_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("KParProp_side").src = "./PnlPlnrCtpKParProp_bside.html";
		doc.getElementById("KParProp_cont").src = "./PnlPlnrCtpKParProp_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KParProp_side").contentDocument;
	contdoc = doc.getElementById("KParProp_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("KParProp_side").contentDocument;
	contdoc = doc.getElementById("KParProp_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KParProp_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KParProp_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KParProp_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKParProp", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpKParProp", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKParProp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpKParProp", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParProp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParProp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParProp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParProp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParProp", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParProp", "TcoAvlWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParProp", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKParProp", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpKParProp", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpKParProp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpKParProp
		rownode = getRownode(srcdoc, "ListPlnrQCtpKParProp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpKParProp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpKParProp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpKParProp", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KParProp", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KParProp_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KParProp_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KParProp_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KParProp_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParProp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKParProp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpKParProp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpKParProp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpKParPropData", scrJref, "StgIacQryPlnrCtpKParProp");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKParProp", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpKParProp", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKParProp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpKParPropDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "ContInfPlnrCtpKParProp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StatAppPlnrCtpKParProp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StatShrPlnrCtpKParProp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StgIacPlnrCtpKParProp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "TagPlnrCtpKParProp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "ListPlnrQCtpKParProp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StatAppQryPlnrCtpKParProp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StatShrQryPlnrCtpKParProp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKParPropData", "StgIacQryPlnrCtpKParProp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpKParPropData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpKParPropData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KParProp");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpKParPropData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

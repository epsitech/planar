/**
  * \file PnlPlnrCtpKCalcvar.js
  * web client functionality for panel PnlPlnrCtpKCalcvar
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpKCalcvar", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpKCalcvar", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("KCalcvar", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("KCalcvar_side").setAttribute("height", "30");
		doc.getElementById("KCalcvar_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("KCalcvar_side").src = "./PnlPlnrCtpKCalcvar_aside.html";
		doc.getElementById("KCalcvar_cont").src = "./PnlPlnrCtpKCalcvar_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpKCalcvar", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("KCalcvar", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("KCalcvar_side").setAttribute("height", "255");
		doc.getElementById("KCalcvar_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("KCalcvar_side").src = "./PnlPlnrCtpKCalcvar_bside.html";
		doc.getElementById("KCalcvar_cont").src = "./PnlPlnrCtpKCalcvar_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KCalcvar_side").contentDocument;
	contdoc = doc.getElementById("KCalcvar_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("KCalcvar_side").contentDocument;
	contdoc = doc.getElementById("KCalcvar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KCalcvar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KCalcvar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KCalcvar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcvar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcvar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcvar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcvar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcvar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcvar", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpKCalcvar
		rownode = getRownode(srcdoc, "ListPlnrQCtpKCalcvar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpKCalcvar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpKCalcvar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KCalcvar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KCalcvar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KCalcvar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KCalcvar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KCalcvar_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcvar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcvar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpKCalcvar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpKCalcvar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpKCalcvarData", scrJref, "StgIacQryPlnrCtpKCalcvar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpKCalcvar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcvar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpKCalcvarDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "ContInfPlnrCtpKCalcvar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StatAppPlnrCtpKCalcvar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StatShrPlnrCtpKCalcvar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StgIacPlnrCtpKCalcvar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "TagPlnrCtpKCalcvar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "ListPlnrQCtpKCalcvar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StatAppQryPlnrCtpKCalcvar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StatShrQryPlnrCtpKCalcvar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcvarData", "StgIacQryPlnrCtpKCalcvar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpKCalcvarData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpKCalcvarData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KCalcvar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpKCalcvarData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

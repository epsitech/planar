/**
  * \file PnlPlnrCtpKCalcdomprp.js
  * web client functionality for panel PnlPlnrCtpKCalcdomprp
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpKCalcdomprp", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpKCalcdomprp", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("KCalcdomprp", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("KCalcdomprp_side").setAttribute("height", "30");
		doc.getElementById("KCalcdomprp_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("KCalcdomprp_side").src = "./PnlPlnrCtpKCalcdomprp_aside.html";
		doc.getElementById("KCalcdomprp_cont").src = "./PnlPlnrCtpKCalcdomprp_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpKCalcdomprp", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("KCalcdomprp", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("KCalcdomprp_side").setAttribute("height", "255");
		doc.getElementById("KCalcdomprp_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("KCalcdomprp_side").src = "./PnlPlnrCtpKCalcdomprp_bside.html";
		doc.getElementById("KCalcdomprp_cont").src = "./PnlPlnrCtpKCalcdomprp_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("KCalcdomprp_side").contentDocument;
	contdoc = doc.getElementById("KCalcdomprp_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("KCalcdomprp_side").contentDocument;
	contdoc = doc.getElementById("KCalcdomprp_cont").contentDocument;
	hdrdoc = contdoc.getElementById("KCalcdomprp_hdr").contentDocument;
	tbldoc = contdoc.getElementById("KCalcdomprp_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("KCalcdomprp_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcdomprp", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcdomprp", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcdomprp", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcdomprp", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcdomprp", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "ncol"));

		var TcoFixWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoFixWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoSrfWidth"));
		var TcoAvlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoAvlWidth"));
		var TcoImpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoImpWidth"));
		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoTitWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpKCalcdomprp", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpKCalcdomprp
		rownode = getRownode(srcdoc, "ListPlnrQCtpKCalcdomprp", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpKCalcdomprp", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpKCalcdomprp", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("KCalcdomprp", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("KCalcdomprp_side").setAttribute("height", "" + (height+55));
	doc.getElementById("KCalcdomprp_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("KCalcdomprp_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("KCalcdomprp_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcdomprp", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpKCalcdomprp", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpKCalcdomprp", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpKCalcdomprp", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpKCalcdomprpData", scrJref, "StgIacQryPlnrCtpKCalcdomprp");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpKCalcdomprp", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpKCalcdomprp", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpKCalcdomprpDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "ContInfPlnrCtpKCalcdomprp", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StatAppPlnrCtpKCalcdomprp", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StatShrPlnrCtpKCalcdomprp", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StgIacPlnrCtpKCalcdomprp", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "TagPlnrCtpKCalcdomprp", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "ListPlnrQCtpKCalcdomprp", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StatAppQryPlnrCtpKCalcdomprp", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StatShrQryPlnrCtpKCalcdomprp", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpKCalcdomprpData", "StgIacQryPlnrCtpKCalcdomprp", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpKCalcdomprpData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpKCalcdomprpData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("KCalcdomprp");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpKCalcdomprpData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrCtpSup1NCalc.js
  * web client functionality for panel PnlPlnrCtpSup1NCalc
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCtpSup1NCalc", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCtpSup1NCalc", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Sup1NCalc", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Sup1NCalc_side").setAttribute("height", "30");
		doc.getElementById("Sup1NCalc_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Sup1NCalc_side").src = "./PnlPlnrCtpSup1NCalc_aside.html";
		doc.getElementById("Sup1NCalc_cont").src = "./PnlPlnrCtpSup1NCalc_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCtpSup1NCalc", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Sup1NCalc", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Sup1NCalc_side").setAttribute("height", "255");
		doc.getElementById("Sup1NCalc_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Sup1NCalc_side").src = "./PnlPlnrCtpSup1NCalc_bside.html";
		doc.getElementById("Sup1NCalc_cont").src = "./PnlPlnrCtpSup1NCalc_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NCalc_side").contentDocument;
	contdoc = doc.getElementById("Sup1NCalc_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Sup1NCalc_side").contentDocument;
	contdoc = doc.getElementById("Sup1NCalc_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NCalc_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NCalc_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NCalc_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCtpSup1NCalc", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpSup1NCalc", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpSup1NCalc", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpSup1NCalc", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpSup1NCalc", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCtpSup1NCalc", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCtpSup1NCalc
		rownode = getRownode(srcdoc, "ListPlnrQCtpSup1NCalc", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCtpSup1NCalc", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCtpSup1NCalc", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NCalc", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NCalc_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NCalc_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NCalc_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NCalc_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpSup1NCalc", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCtpSup1NCalc", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCtpSup1NCalc", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCtpSup1NCalc", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCtpSup1NCalcData", scrJref, "StgIacQryPlnrCtpSup1NCalc");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCtpSup1NCalc", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCtpSup1NCalc", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCtpSup1NCalcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "ContInfPlnrCtpSup1NCalc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StatAppPlnrCtpSup1NCalc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StatShrPlnrCtpSup1NCalc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StgIacPlnrCtpSup1NCalc", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "TagPlnrCtpSup1NCalc", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "ListPlnrQCtpSup1NCalc", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StatAppQryPlnrCtpSup1NCalc", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StatShrQryPlnrCtpSup1NCalc", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCtpSup1NCalcData", "StgIacQryPlnrCtpSup1NCalc", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCtpSup1NCalcData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCtpSup1NCalcData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NCalc");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCtpSup1NCalcData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

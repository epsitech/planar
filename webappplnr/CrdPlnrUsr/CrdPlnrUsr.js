/**
  * \file CrdPlnrUsr.js
  * web client functionality for card CrdPlnrUsr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppPlnrUsr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppPlnrUsr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppPlnrUsr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppPlnrUsr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppPlnrUsr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlPlnrUsrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlPlnrUsrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlPlnrUsrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppPlnrUsr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppPlnrUsr", "shortMenu", short);
	setSi(srcdoc, "StatAppPlnrUsr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppPlnrUsr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrUsr", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagPlnrUsr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagPlnrUsr", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsr"), retrieveCi(srcdoc, "ContInfPlnrUsr", "MtxCrdUsr"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfPlnrUsr", "MtxCrdUsr") + " - Planar 0.1.22";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrUsrDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrUsrDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, ctlsref) {
	window.open(retrieveCi(srcdoc, "ContInfPlnrUsr", ctlsref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrUsrData", "ContInfPlnrUsr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrUsrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngPlnrUsrData", "StatAppPlnrUsr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrUsrData", "StatShrPlnrUsr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrUsrData", "TagPlnrUsr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrUsrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrPlnrUsr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrPlnrUsr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrPlnrUsr", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrUsrData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrUsr", "srefIxPlnrVReqitmode");

	if (srefIxPlnrVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxPlnrVReqitmode = "notify";
		setSi(srcdoc, "StatAppPlnrUsr", "srefIxPlnrVReqitmode", srefIxPlnrVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrUsr", "srefIxPlnrVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppPlnrUsr", "latency"));

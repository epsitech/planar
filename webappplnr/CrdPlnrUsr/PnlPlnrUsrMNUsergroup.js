/**
  * \file PnlPlnrUsrMNUsergroup.js
  * web client functionality for panel PnlPlnrUsrMNUsergroup
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrUsrMNUsergroup", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrUsrMNUsergroup", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("MNUsergroup_side").src = "./PnlPlnrUsrMNUsergroup_aside.html";
		doc.getElementById("MNUsergroup_cont").src = "./PnlPlnrUsrMNUsergroup_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrUsrMNUsergroup", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("MNUsergroup_side").src = "./PnlPlnrUsrMNUsergroup_bside.html";
		doc.getElementById("MNUsergroup_cont").src = "./PnlPlnrUsrMNUsergroup_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrUsrMNUsergroup", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrUsrMNUsergroup", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrUsrMNUsergroup", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrUsrMNUsergroup", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrUsrMNUsergroup", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrUsrMNUsergroup", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrUsrMNUsergroup", "TcoUlvWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQUsrMNUsergroup
		rownode = getRownode(srcdoc, "ListPlnrQUsrMNUsergroup", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrUsrMNUsergroup", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrUsrMNUsergroup", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrUsrMNUsergroup", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrUsrMNUsergroup", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrUsrMNUsergroup", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrUsrMNUsergroup", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrUsrMNUsergroupData", scrJref, "StgIacQryPlnrUsrMNUsergroup");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrUsrMNUsergroup", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrUsrMNUsergroup", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrUsrMNUsergroupDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "ContInfPlnrUsrMNUsergroup", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StatAppPlnrUsrMNUsergroup", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StatShrPlnrUsrMNUsergroup", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StgIacPlnrUsrMNUsergroup", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "TagPlnrUsrMNUsergroup", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "ListPlnrQUsrMNUsergroup", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StatAppQryPlnrUsrMNUsergroup", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StatShrQryPlnrUsrMNUsergroup", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrUsrMNUsergroupData", "StgIacQryPlnrUsrMNUsergroup", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrUsrMNUsergroupData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrDtpTpl1NDevice.js
  * web client functionality for panel PnlPlnrDtpTpl1NDevice
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDtpTpl1NDevice", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDtpTpl1NDevice", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Tpl1NDevice", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Tpl1NDevice_side").setAttribute("height", "30");
		doc.getElementById("Tpl1NDevice_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Tpl1NDevice_side").src = "./PnlPlnrDtpTpl1NDevice_aside.html";
		doc.getElementById("Tpl1NDevice_cont").src = "./PnlPlnrDtpTpl1NDevice_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDtpTpl1NDevice", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Tpl1NDevice", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Tpl1NDevice_side").setAttribute("height", "255");
		doc.getElementById("Tpl1NDevice_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Tpl1NDevice_side").src = "./PnlPlnrDtpTpl1NDevice_bside.html";
		doc.getElementById("Tpl1NDevice_cont").src = "./PnlPlnrDtpTpl1NDevice_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Tpl1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NDevice_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Tpl1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Tpl1NDevice_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Tpl1NDevice_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Tpl1NDevice_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Tpl1NDevice_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDtpTpl1NDevice", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpTpl1NDevice", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpTpl1NDevice", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpTpl1NDevice", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpTpl1NDevice", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpTpl1NDevice", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDtpTpl1NDevice
		rownode = getRownode(srcdoc, "ListPlnrQDtpTpl1NDevice", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDtpTpl1NDevice", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDtpTpl1NDevice", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Tpl1NDevice", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NDevice_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Tpl1NDevice_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Tpl1NDevice_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Tpl1NDevice_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpTpl1NDevice", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpTpl1NDevice", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDtpTpl1NDevice", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDtpTpl1NDevice", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDtpTpl1NDeviceData", scrJref, "StgIacQryPlnrDtpTpl1NDevice");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDtpTpl1NDevice", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpTpl1NDevice", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDtpTpl1NDeviceDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "ContInfPlnrDtpTpl1NDevice", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StatAppPlnrDtpTpl1NDevice", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StatShrPlnrDtpTpl1NDevice", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StgIacPlnrDtpTpl1NDevice", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "TagPlnrDtpTpl1NDevice", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "ListPlnrQDtpTpl1NDevice", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StatAppQryPlnrDtpTpl1NDevice", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StatShrQryPlnrDtpTpl1NDevice", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpTpl1NDeviceData", "StgIacQryPlnrDtpTpl1NDevice", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDtpTpl1NDeviceData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDtpTpl1NDeviceData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Tpl1NDevice");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDtpTpl1NDeviceData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrDtpSup1NDevice.js
  * web client functionality for panel PnlPlnrDtpSup1NDevice
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDtpSup1NDevice", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDtpSup1NDevice", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Sup1NDevice", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Sup1NDevice_side").setAttribute("height", "30");
		doc.getElementById("Sup1NDevice_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Sup1NDevice_side").src = "./PnlPlnrDtpSup1NDevice_aside.html";
		doc.getElementById("Sup1NDevice_cont").src = "./PnlPlnrDtpSup1NDevice_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDtpSup1NDevice", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Sup1NDevice", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Sup1NDevice_side").setAttribute("height", "255");
		doc.getElementById("Sup1NDevice_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Sup1NDevice_side").src = "./PnlPlnrDtpSup1NDevice_bside.html";
		doc.getElementById("Sup1NDevice_cont").src = "./PnlPlnrDtpSup1NDevice_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Sup1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Sup1NDevice_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Sup1NDevice_side").contentDocument;
	contdoc = doc.getElementById("Sup1NDevice_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Sup1NDevice_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Sup1NDevice_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Sup1NDevice_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDtpSup1NDevice", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpSup1NDevice", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpSup1NDevice", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpSup1NDevice", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpSup1NDevice", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpSup1NDevice", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDtpSup1NDevice
		rownode = getRownode(srcdoc, "ListPlnrQDtpSup1NDevice", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDtpSup1NDevice", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDtpSup1NDevice", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Sup1NDevice", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NDevice_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Sup1NDevice_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Sup1NDevice_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Sup1NDevice_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpSup1NDevice", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpSup1NDevice", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDtpSup1NDevice", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDtpSup1NDevice", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDtpSup1NDeviceData", scrJref, "StgIacQryPlnrDtpSup1NDevice");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDtpSup1NDevice", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpSup1NDevice", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDtpSup1NDeviceDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "ContInfPlnrDtpSup1NDevice", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StatAppPlnrDtpSup1NDevice", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StatShrPlnrDtpSup1NDevice", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StgIacPlnrDtpSup1NDevice", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "TagPlnrDtpSup1NDevice", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "ListPlnrQDtpSup1NDevice", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StatAppQryPlnrDtpSup1NDevice", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StatShrQryPlnrDtpSup1NDevice", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpSup1NDeviceData", "StgIacQryPlnrDtpSup1NDevice", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDtpSup1NDeviceData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDtpSup1NDeviceData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Sup1NDevice");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDtpSup1NDeviceData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

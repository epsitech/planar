/**
  * \file PnlPlnrDtpHk1NStructure.js
  * web client functionality for panel PnlPlnrDtpHk1NStructure
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDtpHk1NStructure", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDtpHk1NStructure", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("Hk1NStructure_side").src = "./PnlPlnrDtpHk1NStructure_aside.html";
		doc.getElementById("Hk1NStructure_cont").src = "./PnlPlnrDtpHk1NStructure_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDtpHk1NStructure", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("Hk1NStructure_side").src = "./PnlPlnrDtpHk1NStructure_bside.html";
		doc.getElementById("Hk1NStructure_cont").src = "./PnlPlnrDtpHk1NStructure_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDtpHk1NStructure", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpHk1NStructure", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpHk1NStructure", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpHk1NStructure", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpHk1NStructure", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpHk1NStructure", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDtpHk1NStructure
		rownode = getRownode(srcdoc, "ListPlnrQDtpHk1NStructure", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDtpHk1NStructure", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDtpHk1NStructure", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpHk1NStructure", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpHk1NStructure", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDtpHk1NStructure", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDtpHk1NStructure", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDtpHk1NStructureData", scrJref, "StgIacQryPlnrDtpHk1NStructure");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDtpHk1NStructure", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpHk1NStructure", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDtpHk1NStructureDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "ContInfPlnrDtpHk1NStructure", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StatAppPlnrDtpHk1NStructure", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StatShrPlnrDtpHk1NStructure", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StgIacPlnrDtpHk1NStructure", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "TagPlnrDtpHk1NStructure", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "ListPlnrQDtpHk1NStructure", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StatAppQryPlnrDtpHk1NStructure", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StatShrQryPlnrDtpHk1NStructure", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpHk1NStructureData", "StgIacQryPlnrDtpHk1NStructure", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDtpHk1NStructureData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDtpHk1NStructureData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDtpHk1NStructureData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrDtpAPar.js
  * web client functionality for panel PnlPlnrDtpAPar
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrDtpAPar", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrDtpAPar", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrDtpAPar_aside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrDtpAPar_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrDtpAPar", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrDtpAPar_bside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrDtpAPar_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpAPar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrDtpAPar", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrDtpAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrDtpAPar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpAPar", "TcoKeyWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpAPar", "TcoFilWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrDtpAPar", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrDtpAPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrDtpAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQDtpAPar
		rownode = getRownode(srcdoc, "ListPlnrQDtpAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrDtpAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrDtpAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrDtpAPar", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrDtpAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrDtpAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrDtpAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrDtpAParData", scrJref, "StgIacQryPlnrDtpAPar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrDtpAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrDtpAPar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrDtpAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrDtpAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "ContInfPlnrDtpAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StatAppPlnrDtpAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StatShrPlnrDtpAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StgIacPlnrDtpAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "TagPlnrDtpAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "ListPlnrQDtpAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StatAppQryPlnrDtpAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StatShrQryPlnrDtpAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrDtpAParData", "StgIacQryPlnrDtpAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrDtpAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrDtpAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrDtpAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrFilMNCalc.js
  * web client functionality for panel PnlPlnrFilMNCalc
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrFilMNCalc", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrFilMNCalc", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("MNCalc", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("MNCalc_side").setAttribute("height", "30");
		doc.getElementById("MNCalc_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("MNCalc_side").src = "./PnlPlnrFilMNCalc_aside.html";
		doc.getElementById("MNCalc_cont").src = "./PnlPlnrFilMNCalc_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrFilMNCalc", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("MNCalc", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("MNCalc_side").setAttribute("height", "255");
		doc.getElementById("MNCalc_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("MNCalc_side").src = "./PnlPlnrFilMNCalc_bside.html";
		doc.getElementById("MNCalc_cont").src = "./PnlPlnrFilMNCalc_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNCalc_side").contentDocument;
	contdoc = doc.getElementById("MNCalc_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNCalc_side").contentDocument;
	contdoc = doc.getElementById("MNCalc_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNCalc_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNCalc_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNCalc_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrFilMNCalc", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrFilMNCalc", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrFilMNCalc", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrFilMNCalc", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrFilMNCalc", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrFilMNCalc", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrFilMNCalc", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrFilMNCalc", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrFilMNCalc", "TcoMrefWidth"));
		var TcoCliWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrFilMNCalc", "TcoCliWidth"));
		var TcoIoWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrFilMNCalc", "TcoIoWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoCli"); doc.widths.push(TcoCliWidth); doc.fs.push("cli"); wtot += TcoCliWidth;
			} else if (i == 3) {
				doc.cols.push("TcoIo"); doc.widths.push(TcoIoWidth); doc.fs.push("io"); wtot += TcoIoWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrFilMNCalc", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrFilMNCalc", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQFilMNCalc
		rownode = getRownode(srcdoc, "ListPlnrQFilMNCalc", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrFilMNCalc", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrFilMNCalc", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrFilMNCalc", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNCalc", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCalc_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNCalc_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNCalc_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNCalc_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrFilMNCalc", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrFilMNCalc", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrFilMNCalc", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrFilMNCalc", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrFilMNCalcData", scrJref, "StgIacQryPlnrFilMNCalc");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrFilMNCalc", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrFilMNCalc", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrFilMNCalc", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrFilMNCalcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "ContInfPlnrFilMNCalc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StatAppPlnrFilMNCalc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StatShrPlnrFilMNCalc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StgIacPlnrFilMNCalc", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "TagPlnrFilMNCalc", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "ListPlnrQFilMNCalc", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StatAppQryPlnrFilMNCalc", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StatShrQryPlnrFilMNCalc", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrFilMNCalcData", "StgIacQryPlnrFilMNCalc", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrFilMNCalcData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrFilMNCalcData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNCalc");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrFilMNCalcData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

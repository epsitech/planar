/**
  * \file PnlPlnrSru1NBlock.js
  * web client functionality for panel PnlPlnrSru1NBlock
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrSru1NBlock", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrSru1NBlock", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("1NBlock_side").src = "./PnlPlnrSru1NBlock_aside.html";
		doc.getElementById("1NBlock_cont").src = "./PnlPlnrSru1NBlock_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrSru1NBlock", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("1NBlock_side").src = "./PnlPlnrSru1NBlock_bside.html";
		doc.getElementById("1NBlock_cont").src = "./PnlPlnrSru1NBlock_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSru1NBlock", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrSru1NBlock", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSru1NBlock", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrSru1NBlock", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSru1NBlock", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSru1NBlock", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSru1NBlock", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSru1NBlock", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSru1NBlock", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrSru1NBlock", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrSru1NBlock", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQSru1NBlock
		rownode = getRownode(srcdoc, "ListPlnrQSru1NBlock", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrSru1NBlock", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrSru1NBlock", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrSru1NBlock", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSru1NBlock", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSru1NBlock", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrSru1NBlock", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrSru1NBlock", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrSru1NBlockData", scrJref, "StgIacQryPlnrSru1NBlock");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSru1NBlock", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrSru1NBlock", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSru1NBlock", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrSru1NBlockDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "ContInfPlnrSru1NBlock", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StatAppPlnrSru1NBlock", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StatShrPlnrSru1NBlock", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StgIacPlnrSru1NBlock", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "TagPlnrSru1NBlock", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "ListPlnrQSru1NBlock", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StatAppQryPlnrSru1NBlock", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StatShrQryPlnrSru1NBlock", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrSru1NBlockData", "StgIacQryPlnrSru1NBlock", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrSru1NBlockData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrSru1NBlockData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrSru1NBlockData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

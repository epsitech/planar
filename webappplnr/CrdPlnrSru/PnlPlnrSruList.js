/**
  * \file PnlPlnrSruList.js
  * web client functionality for panel PnlPlnrSruList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrSruList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrSruList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrSruList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrSruList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSruList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrSruList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrSruList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrSruList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrSruList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSruList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrSruList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSruList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrSruList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrSruList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrSruList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrSruList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruList", "TcoSrfWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruList", "TcoHkuWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrSruList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrSruList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrSruList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQSruList
		rownode = getRownode(srcdoc, "ListPlnrQSruList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrSruList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrSruList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrSruList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrSruList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrSruList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrSruListData", scrJref, "StgIacQryPlnrSruList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrSruList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrSruListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrSruList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrSruList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrSruListData", scrJref, "ContIacPlnrSruList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "ContIacPlnrSruList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "ContInfPlnrSruList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "StatShrPlnrSruList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "StgIacPlnrSruList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "TagPlnrSruList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "ListPlnrQSruList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "StatAppQryPlnrSruList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "StatShrQryPlnrSruList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrSruListData", "StgIacQryPlnrSruList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrSruListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrSruListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrSruListData") {
				mask = mergeDpchEngData(dom);

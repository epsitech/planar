/**
  * \file PnlPlnrSruMNStack.js
  * web client functionality for panel PnlPlnrSruMNStack
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrSruMNStack", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrSruMNStack", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("MNStack", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("MNStack_side").setAttribute("height", "30");
		doc.getElementById("MNStack_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("MNStack_side").src = "./PnlPlnrSruMNStack_aside.html";
		doc.getElementById("MNStack_cont").src = "./PnlPlnrSruMNStack_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrSruMNStack", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("MNStack", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("MNStack_side").setAttribute("height", "255");
		doc.getElementById("MNStack_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("MNStack_side").src = "./PnlPlnrSruMNStack_bside.html";
		doc.getElementById("MNStack_cont").src = "./PnlPlnrSruMNStack_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNStack_side").contentDocument;
	contdoc = doc.getElementById("MNStack_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNStack_side").contentDocument;
	contdoc = doc.getElementById("MNStack_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNStack_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNStack_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNStack_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSruMNStack", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrSruMNStack", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrSruMNStack", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrSruMNStack", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruMNStack", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruMNStack", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruMNStack", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruMNStack", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruMNStack", "TcoMrefWidth"));
		var TcoFrlWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruMNStack", "TcoFrlWidth"));
		var TcoTolWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrSruMNStack", "TcoTolWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrSruMNStack", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrSruMNStack", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQSruMNStack
		rownode = getRownode(srcdoc, "ListPlnrQSruMNStack", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrSruMNStack", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrSruMNStack", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrSruMNStack", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNStack", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStack_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNStack_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNStack_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNStack_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruMNStack", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrSruMNStack", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrSruMNStack", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrSruMNStack", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrSruMNStackData", scrJref, "StgIacQryPlnrSruMNStack");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrSruMNStack", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrSruMNStack", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrSruMNStack", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrSruMNStackDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "ContInfPlnrSruMNStack", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StatAppPlnrSruMNStack", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StatShrPlnrSruMNStack", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StgIacPlnrSruMNStack", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "TagPlnrSruMNStack", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "ListPlnrQSruMNStack", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StatAppQryPlnrSruMNStack", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StatShrQryPlnrSruMNStack", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrSruMNStackData", "StgIacQryPlnrSruMNStack", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrSruMNStackData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrSruMNStackData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNStack");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrSruMNStackData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

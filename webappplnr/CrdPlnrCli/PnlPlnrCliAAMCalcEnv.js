/**
  * \file PnlPlnrCliAAMCalcEnv.js
  * web client functionality for panel PnlPlnrCliAAMCalcEnv
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCliAAMCalcEnv", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCliAAMCalcEnv", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("AAMCalcEnv", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("AAMCalcEnv_side").setAttribute("height", "30");
		doc.getElementById("AAMCalcEnv_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("AAMCalcEnv_side").src = "./PnlPlnrCliAAMCalcEnv_aside.html";
		doc.getElementById("AAMCalcEnv_cont").src = "./PnlPlnrCliAAMCalcEnv_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCliAAMCalcEnv", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("AAMCalcEnv", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("AAMCalcEnv_side").setAttribute("height", "255");
		doc.getElementById("AAMCalcEnv_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("AAMCalcEnv_side").src = "./PnlPlnrCliAAMCalcEnv_bside.html";
		doc.getElementById("AAMCalcEnv_cont").src = "./PnlPlnrCliAAMCalcEnv_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AAMCalcEnv_side").contentDocument;
	contdoc = doc.getElementById("AAMCalcEnv_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("AAMCalcEnv_side").contentDocument;
	contdoc = doc.getElementById("AAMCalcEnv_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AAMCalcEnv_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AAMCalcEnv_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AAMCalcEnv_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcEnv", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcEnv", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcEnv", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcEnv", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcEnv", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCliAAMCalcEnv", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCliAAMCalcEnv", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCliAAMCalcEnv
		rownode = getRownode(srcdoc, "ListPlnrQCliAAMCalcEnv", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCliAAMCalcEnv", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCliAAMCalcEnv", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AAMCalcEnv", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AAMCalcEnv_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AAMCalcEnv_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AAMCalcEnv_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AAMCalcEnv_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcEnv", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcEnv", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCliAAMCalcEnv", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCliAAMCalcEnv", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCliAAMCalcEnvData", scrJref, "StgIacQryPlnrCliAAMCalcEnv");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCliAAMCalcEnv", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcEnv", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCliAAMCalcEnvDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "ContInfPlnrCliAAMCalcEnv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StatAppPlnrCliAAMCalcEnv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StatShrPlnrCliAAMCalcEnv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StgIacPlnrCliAAMCalcEnv", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "TagPlnrCliAAMCalcEnv", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "ListPlnrQCliAAMCalcEnv", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StatAppQryPlnrCliAAMCalcEnv", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StatShrQryPlnrCliAAMCalcEnv", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcEnvData", "StgIacQryPlnrCliAAMCalcEnv", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCliAAMCalcEnvData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCliAAMCalcEnvData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AAMCalcEnv");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCliAAMCalcEnvData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

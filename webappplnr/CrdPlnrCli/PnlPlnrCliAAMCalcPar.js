/**
  * \file PnlPlnrCliAAMCalcPar.js
  * web client functionality for panel PnlPlnrCliAAMCalcPar
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrCliAAMCalcPar", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrCliAAMCalcPar", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("AAMCalcPar", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("AAMCalcPar_side").setAttribute("height", "30");
		doc.getElementById("AAMCalcPar_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("AAMCalcPar_side").src = "./PnlPlnrCliAAMCalcPar_aside.html";
		doc.getElementById("AAMCalcPar_cont").src = "./PnlPlnrCliAAMCalcPar_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrCliAAMCalcPar", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("AAMCalcPar", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("AAMCalcPar_side").setAttribute("height", "255");
		doc.getElementById("AAMCalcPar_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("AAMCalcPar_side").src = "./PnlPlnrCliAAMCalcPar_bside.html";
		doc.getElementById("AAMCalcPar_cont").src = "./PnlPlnrCliAAMCalcPar_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AAMCalcPar_side").contentDocument;
	contdoc = doc.getElementById("AAMCalcPar_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("AAMCalcPar_side").contentDocument;
	contdoc = doc.getElementById("AAMCalcPar_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AAMCalcPar_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AAMCalcPar_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AAMCalcPar_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcPar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "ncol"));

		var TcoCatWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCliAAMCalcPar", "TcoCatWidth"));
		var TcoPrpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCliAAMCalcPar", "TcoPrpWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrCliAAMCalcPar", "TcoValWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoCat"); doc.widths.push(TcoCatWidth); doc.fs.push("cat"); wtot += TcoCatWidth;
			} else if (i == 2) {
				doc.cols.push("TcoPrp"); doc.widths.push(TcoPrpWidth); doc.fs.push("prp"); wtot += TcoPrpWidth;
			} else if (i == 3) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQCliAAMCalcPar
		rownode = getRownode(srcdoc, "ListPlnrQCliAAMCalcPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrCliAAMCalcPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrCliAAMCalcPar", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AAMCalcPar", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AAMCalcPar_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AAMCalcPar_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AAMCalcPar_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AAMCalcPar_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrCliAAMCalcPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrCliAAMCalcPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrCliAAMCalcPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrCliAAMCalcParData", scrJref, "StgIacQryPlnrCliAAMCalcPar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrCliAAMCalcPar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrCliAAMCalcPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrCliAAMCalcParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "ContInfPlnrCliAAMCalcPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StatAppPlnrCliAAMCalcPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StatShrPlnrCliAAMCalcPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StgIacPlnrCliAAMCalcPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "TagPlnrCliAAMCalcPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "ListPlnrQCliAAMCalcPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StatAppQryPlnrCliAAMCalcPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StatShrQryPlnrCliAAMCalcPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrCliAAMCalcParData", "StgIacQryPlnrCliAAMCalcPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrCliAAMCalcParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrCliAAMCalcParData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AAMCalcPar");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrCliAAMCalcParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

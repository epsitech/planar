/**
  * \file CrdPlnrClc.js
  * web client functionality for card CrdPlnrClc
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppPlnrClc", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppPlnrClc", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppPlnrClc", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppPlnrClc", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppPlnrClc", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlPlnrClcHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlPlnrClcList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlPlnrClcRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppPlnrClc", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppPlnrClc", "shortMenu", short);
	setSi(srcdoc, "StatAppPlnrClc", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppPlnrClc", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrClc", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagPlnrClc", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagPlnrClc", "MrlAppHlp"));
};
// IP initMenApp --- END
...
function initMenCrd() {
	...
	MspCrd1Avail = (retrieveSi(srcdoc, "StatShrPlnrClc", "MspCrd1Avail") == "true");
	MitCrdUcsAvail = (retrieveSi(srcdoc, "StatShrPlnrClc", "MitCrdUcsAvail") == "true");
	MitCrdGpvAvail = (retrieveSi(srcdoc, "StatShrPlnrClc", "MitCrdGpvAvail") == "true");
	MitCrdGpvActive = (retrieveSi(srcdoc, "StatShrPlnrClc", "MitCrdGpvActive") == "true");
	MitCrdEtxAvail = (retrieveSi(srcdoc, "StatShrPlnrClc", "MitCrdEtxAvail") == "true");
	MitCrdEtxActive = (retrieveSi(srcdoc, "StatShrPlnrClc", "MitCrdEtxActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppPlnrClc", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdClc"), retrieveCi(srcdoc, "ContInfPlnrClc", "MtxCrdClc"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdUcs"), retrieveTi(srcdoc, "TagPlnrClc", "MitCrdUcs"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdGpv"), retrieveTi(srcdoc, "TagPlnrClc", "MitCrdGpv"));
	setMitActive("MitCrdGpv", MitCrdGpvActive);
	setTextContent(mendoc, mendoc.getElementById("MitCrdEtx"), retrieveTi(srcdoc, "TagPlnrClc", "MitCrdEtx"));
	setMitActive("MitCrdEtx", MitCrdEtxActive);

	height -= setMitMspAvail("MspCrd1", MspCrd1Avail, 1);
	height -= setMitMspAvail("MitCrdUcs", MitCrdUcsAvail, 20);
	height -= setMitMspAvail("MitCrdGpv", MitCrdGpvAvail, 20);
	height -= setMitMspAvail("MitCrdEtx", MitCrdEtxAvail, 20);
...
function showDlg(sref, _scrJref)  {
	...
	if ((sref == "DlgPlnrClcUsecase")) myif.setAttribute("height", "585");
...
function refresh() {
	var scrJrefDlgexprtx = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefDlgexprtx");
	var scrJrefDlggenrpv = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefDlggenrpv");
	var scrJrefDlgusecase = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefDlgusecase");

	if (scrJrefDlgexprtx != "") {
		if (scrJrefDlg != scrJrefDlgexprtx) showDlg("DlgPlnrClcExprtx", scrJrefDlgexprtx);
	} else if (scrJrefDlggenrpv != "") {
		if (scrJrefDlg != scrJrefDlggenrpv) showDlg("DlgPlnrClcGenrpv", scrJrefDlggenrpv);
	} else if (scrJrefDlgusecase != "") {
		if (scrJrefDlg != scrJrefDlgusecase) showDlg("DlgPlnrClcUsecase", scrJrefDlgusecase);
	} else if (scrJrefDlg != "") hideDlg();

	doc.title = retrieveCi(srcdoc, "ContInfPlnrClc", "MtxCrdClc") + " - Planar 0.1.22";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, ctlsref) {
	window.open(retrieveCi(srcdoc, "ContInfPlnrClc", ctlsref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcData", "ContInfPlnrClc", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClcData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngPlnrClcData", "StatAppPlnrClc", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrClcData", "StatShrPlnrClc", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClcData", "TagPlnrClc", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrPlnrClc", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrClc", "srefIxPlnrVReqitmode");

	if (srefIxPlnrVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxPlnrVReqitmode = "notify";
		setSi(srcdoc, "StatAppPlnrClc", "srefIxPlnrVReqitmode", srefIxPlnrVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxPlnrVReqitmode = retrieveSi(srcdoc, "StatAppPlnrClc", "srefIxPlnrVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppPlnrClc", "latency"));

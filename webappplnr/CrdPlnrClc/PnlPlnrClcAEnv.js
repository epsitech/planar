/**
  * \file PnlPlnrClcAEnv.js
  * web client functionality for panel PnlPlnrClcAEnv
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClcAEnv", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClcAEnv", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("AEnv", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("AEnv_side").setAttribute("height", "30");
		doc.getElementById("AEnv_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrClcAEnv_aside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrClcAEnv_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClcAEnv", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("AEnv", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("AEnv_side").setAttribute("height", "255");
		doc.getElementById("AEnv_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("AEnv_side").src = "./PnlPlnrClcAEnv_bside.html";
		doc.getElementById("AEnv_cont").src = "./PnlPlnrClcAEnv_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AEnv_side").contentDocument;
	contdoc = doc.getElementById("AEnv_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("AEnv_side").contentDocument;
	contdoc = doc.getElementById("AEnv_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AEnv_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AEnv_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AEnv_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcAEnv", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrClcAEnv", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcAEnv", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClcAEnv", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcAEnv", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcAEnv", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcAEnv", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcAEnv", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcAEnv", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcAEnv", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrClcAEnv", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrClcAEnv", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQClcAEnv
		rownode = getRownode(srcdoc, "ListPlnrQClcAEnv", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrClcAEnv", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrClcAEnv", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrClcAEnv", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AEnv", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AEnv_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AEnv_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AEnv_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AEnv_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcAEnv", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcAEnv", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrClcAEnv", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrClcAEnv", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClcAEnvData", scrJref, "StgIacQryPlnrClcAEnv");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcAEnv", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrClcAEnv", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcAEnv", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcAEnvDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "ContInfPlnrClcAEnv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StatAppPlnrClcAEnv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StatShrPlnrClcAEnv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StgIacPlnrClcAEnv", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "TagPlnrClcAEnv", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "ListPlnrQClcAEnv", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StatAppQryPlnrClcAEnv", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StatShrQryPlnrClcAEnv", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcAEnvData", "StgIacQryPlnrClcAEnv", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClcAEnvData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcAEnvData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AEnv");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcAEnvData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

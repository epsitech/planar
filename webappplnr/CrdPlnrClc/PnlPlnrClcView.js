/**
  * \file PnlPlnrClcView.js
  * web client functionality for panel PnlPlnrClcView
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClcView", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClcView", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("View_side").src = "./PnlPlnrClcView_aside.html";
		doc.getElementById("View_cont").src = "./PnlPlnrClcView_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClcView", "srefIxPlnrVExpstate", "regd");

		// change content (container heights in refreshB)
		doc.getElementById("View_side").src = "./PnlPlnrClcView_bside.html";
		doc.getElementById("View_cont").src = "./PnlPlnrClcView_b.html";
	};
};
...
function initA() {
	...
	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrClcView", "Cpt"));
...
// IP initB --- BEGIN
	initCpt(hdrdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrClcView", "Cpt"));
// IP initB --- END
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcViewData", "TagPlnrClcView", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClcViewData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcViewData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcViewData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

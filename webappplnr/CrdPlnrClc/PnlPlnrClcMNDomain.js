/**
  * \file PnlPlnrClcMNDomain.js
  * web client functionality for panel PnlPlnrClcMNDomain
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClcMNDomain", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClcMNDomain", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("MNDomain", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("MNDomain_side").setAttribute("height", "30");
		doc.getElementById("MNDomain_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("MNDomain_side").src = "./PnlPlnrClcMNDomain_aside.html";
		doc.getElementById("MNDomain_cont").src = "./PnlPlnrClcMNDomain_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClcMNDomain", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("MNDomain", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("MNDomain_side").setAttribute("height", "255");
		doc.getElementById("MNDomain_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("MNDomain_side").src = "./PnlPlnrClcMNDomain_bside.html";
		doc.getElementById("MNDomain_cont").src = "./PnlPlnrClcMNDomain_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNDomain_side").contentDocument;
	contdoc = doc.getElementById("MNDomain_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNDomain_side").contentDocument;
	contdoc = doc.getElementById("MNDomain_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNDomain_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNDomain_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNDomain_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcMNDomain", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrClcMNDomain", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcMNDomain", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClcMNDomain", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNDomain", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNDomain", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNDomain", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNDomain", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcMNDomain", "TcoMrefWidth"));
		var TcoPrpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcMNDomain", "TcoPrpWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrClcMNDomain", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrClcMNDomain", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQClcMNDomain
		rownode = getRownode(srcdoc, "ListPlnrQClcMNDomain", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrClcMNDomain", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrClcMNDomain", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrClcMNDomain", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNDomain", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNDomain_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNDomain_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNDomain_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNDomain_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNDomain", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNDomain", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrClcMNDomain", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrClcMNDomain", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClcMNDomainData", scrJref, "StgIacQryPlnrClcMNDomain");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNDomain", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrClcMNDomain", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNDomain", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcMNDomainDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "ContInfPlnrClcMNDomain", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StatAppPlnrClcMNDomain", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StatShrPlnrClcMNDomain", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StgIacPlnrClcMNDomain", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "TagPlnrClcMNDomain", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "ListPlnrQClcMNDomain", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StatAppQryPlnrClcMNDomain", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StatShrQryPlnrClcMNDomain", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNDomainData", "StgIacQryPlnrClcMNDomain", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClcMNDomainData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcMNDomainData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNDomain");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcMNDomainData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrClc1NMaterial.js
  * web client functionality for panel PnlPlnrClc1NMaterial
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClc1NMaterial", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClc1NMaterial", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("1NMaterial", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("1NMaterial_side").setAttribute("height", "30");
		doc.getElementById("1NMaterial_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("1NMaterial_side").src = "./PnlPlnrClc1NMaterial_aside.html";
		doc.getElementById("1NMaterial_cont").src = "./PnlPlnrClc1NMaterial_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClc1NMaterial", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("1NMaterial", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("1NMaterial_side").setAttribute("height", "255");
		doc.getElementById("1NMaterial_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("1NMaterial_side").src = "./PnlPlnrClc1NMaterial_bside.html";
		doc.getElementById("1NMaterial_cont").src = "./PnlPlnrClc1NMaterial_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NMaterial_side").contentDocument;
	contdoc = doc.getElementById("1NMaterial_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NMaterial_side").contentDocument;
	contdoc = doc.getElementById("1NMaterial_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NMaterial_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NMaterial_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NMaterial_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClc1NMaterial", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrClc1NMaterial", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClc1NMaterial", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClc1NMaterial", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClc1NMaterial", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClc1NMaterial", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClc1NMaterial", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClc1NMaterial", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClc1NMaterial", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrClc1NMaterial", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQClc1NMaterial
		rownode = getRownode(srcdoc, "ListPlnrQClc1NMaterial", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrClc1NMaterial", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrClc1NMaterial", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrClc1NMaterial", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NMaterial", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMaterial_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NMaterial_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NMaterial_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NMaterial_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClc1NMaterial", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClc1NMaterial", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrClc1NMaterial", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrClc1NMaterial", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClc1NMaterialData", scrJref, "StgIacQryPlnrClc1NMaterial");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrClc1NMaterial", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClc1NMaterial", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClc1NMaterialDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "ContInfPlnrClc1NMaterial", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StatAppPlnrClc1NMaterial", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StatShrPlnrClc1NMaterial", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StgIacPlnrClc1NMaterial", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "TagPlnrClc1NMaterial", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "ListPlnrQClc1NMaterial", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StatAppQryPlnrClc1NMaterial", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StatShrQryPlnrClc1NMaterial", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrClc1NMaterialData", "StgIacQryPlnrClc1NMaterial", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClc1NMaterialData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClc1NMaterialData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NMaterial");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClc1NMaterialData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrClcMNFile.js
  * web client functionality for panel PnlPlnrClcMNFile
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClcMNFile", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClcMNFile", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("MNFile", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("MNFile_side").setAttribute("height", "30");
		doc.getElementById("MNFile_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("MNFile_side").src = "./PnlPlnrClcMNFile_aside.html";
		doc.getElementById("MNFile_cont").src = "./PnlPlnrClcMNFile_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClcMNFile", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("MNFile", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("MNFile_side").setAttribute("height", "255");
		doc.getElementById("MNFile_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("MNFile_side").src = "./PnlPlnrClcMNFile_bside.html";
		doc.getElementById("MNFile_cont").src = "./PnlPlnrClcMNFile_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("MNFile_side").contentDocument;
	contdoc = doc.getElementById("MNFile_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("MNFile_side").contentDocument;
	contdoc = doc.getElementById("MNFile_cont").contentDocument;
	hdrdoc = contdoc.getElementById("MNFile_hdr").contentDocument;
	tbldoc = contdoc.getElementById("MNFile_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("MNFile_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcMNFile", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrClcMNFile", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcMNFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClcMNFile", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcMNFile", "TcoMrefWidth"));
		var TcoCliWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcMNFile", "TcoCliWidth"));
		var TcoIoWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcMNFile", "TcoIoWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrClcMNFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrClcMNFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQClcMNFile
		rownode = getRownode(srcdoc, "ListPlnrQClcMNFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrClcMNFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrClcMNFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrClcMNFile", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("MNFile", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("MNFile_side").setAttribute("height", "" + (height+55));
	doc.getElementById("MNFile_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("MNFile_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("MNFile_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcMNFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrClcMNFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrClcMNFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClcMNFileData", scrJref, "StgIacQryPlnrClcMNFile");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcMNFile", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrClcMNFile", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcMNFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcMNFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "ContInfPlnrClcMNFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StatAppPlnrClcMNFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StatShrPlnrClcMNFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StgIacPlnrClcMNFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "TagPlnrClcMNFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "ListPlnrQClcMNFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StatAppQryPlnrClcMNFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StatShrQryPlnrClcMNFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcMNFileData", "StgIacQryPlnrClcMNFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClcMNFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcMNFileData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("MNFile");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcMNFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrClcList.js
  * web client functionality for panel PnlPlnrClcList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrClcList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrClcList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrClcList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrClcList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrClcList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrClcList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrClcList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrClcList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClcList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrClcList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrClcList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrClcList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrClcList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrClcList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "ncol"));

		var TcoTitWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoTitWidth"));
		var TcoPrjWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoPrjWidth"));
		var TcoDimWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoDimWidth"));
		var TcoTplWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoTplWidth"));
		var TcoSupWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoSupWidth"));
		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrClcList", "TcoDsnWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrClcList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoDim"); doc.widths.push(TcoDimWidth); doc.fs.push("dim"); doc.tos.push(3); wtot += TcoDimWidth;
			} else if (i == 4) {
				...
				doc.cols.push("TcoDsn"); doc.widths.push(TcoDsnWidth); doc.fs.push("dsn"); doc.tos.push(6); wtot += TcoDsnWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrClcList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrClcList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQClcList
		rownode = getRownode(srcdoc, "ListPlnrQClcList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrClcList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrClcList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrClcList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrClcList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrClcList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrClcList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClcListData", scrJref, "StgIacQryPlnrClcList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrClcList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrClcList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrClcList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrClcListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrClcList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrClcList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrClcListData", scrJref, "ContIacPlnrClcList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "ContIacPlnrClcList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "ContInfPlnrClcList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "StatShrPlnrClcList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "StgIacPlnrClcList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "TagPlnrClcList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "ListPlnrQClcList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "StatAppQryPlnrClcList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "StatShrQryPlnrClcList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrClcListData", "StgIacQryPlnrClcList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrClcListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcListData") {
				mask = mergeDpchEngData(dom);

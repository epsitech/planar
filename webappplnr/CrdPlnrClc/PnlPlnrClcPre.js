/**
  * \file PnlPlnrClcPre.js
  * web client functionality for panel PnlPlnrClcPre
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrClcPre", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrClcPre", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("Pre_side").src = "./PnlPlnrClcPre_aside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrClcPre_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrClcPre", "srefIxPlnrVExpstate", "regd");

		// change content (container heights in refreshB)
		doc.getElementById("Pre_side").src = "./PnlPlnrClcPre_bside.html";
		doc.getElementById("Pre_cont").src = "./PnlPlnrClcPre_b.html";
	};
};
...
function initA() {
	...
	initCpt(contdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrClcPre", "Cpt"));
...
// IP initB --- BEGIN
	initCpt(hdrdoc, "Cpt", retrieveTi(srcdoc, "TagPlnrClcPre", "Cpt"));
// IP initB --- END
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrClcPreData", "TagPlnrClcPre", srcdoc)) mask.push("tag");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrClcPreData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrClcPreData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrClcPreData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

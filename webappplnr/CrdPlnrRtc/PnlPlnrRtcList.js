/**
  * \file PnlPlnrRtcList.js
  * web client functionality for panel PnlPlnrRtcList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrRtcList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrRtcList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrRtcList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrRtcList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrRtcList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrRtcList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrRtcList", "srefIxPlnrVExpstate");
...
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrRtcList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrRtcList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrRtcList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrRtcList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrRtcList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrRtcList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrRtcList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrRtcList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrRtcList", "TcoSrfWidth"));
		var TcoHktWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrRtcList", "TcoHktWidth"));
		var TcoHkuWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrRtcList", "TcoHkuWidth"));
		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrRtcList", "TcoDsnWidth"));
		var TcoLyrWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrRtcList", "TcoLyrWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrRtcList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoHkt"); doc.widths.push(TcoHktWidth); doc.fs.push("hkt"); doc.tos.push(2); wtot += TcoHktWidth;
			} else if (i == 3) {
				doc.cols.push("TcoHku"); doc.widths.push(TcoHkuWidth); doc.fs.push("hku"); doc.tos.push(3); wtot += TcoHkuWidth;
			} else if (i == 4) {
				doc.cols.push("TcoDsn"); doc.widths.push(TcoDsnWidth); doc.fs.push("dsn"); doc.tos.push(4); wtot += TcoDsnWidth;
			} else if (i == 5) {
				doc.cols.push("TcoLyr"); doc.widths.push(TcoLyrWidth); doc.fs.push("lyr"); doc.tos.push(5); wtot += TcoLyrWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrRtcList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrRtcList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQRtcList
		rownode = getRownode(srcdoc, "ListPlnrQRtcList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrRtcList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrRtcList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrRtcList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrRtcList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrRtcList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrRtcList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrRtcList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrRtcListData", scrJref, "StgIacQryPlnrRtcList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrRtcList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrRtcList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrRtcList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrRtcListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrRtcList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrRtcList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrRtcListData", scrJref, "ContIacPlnrRtcList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "ContIacPlnrRtcList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "ContInfPlnrRtcList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "StatShrPlnrRtcList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "StgIacPlnrRtcList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "TagPlnrRtcList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "ListPlnrQRtcList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "StatAppQryPlnrRtcList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "StatShrQryPlnrRtcList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrRtcListData", "StgIacQryPlnrRtcList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrRtcListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrRtcListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrRtcListData") {
				mask = mergeDpchEngData(dom);

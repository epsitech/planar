/**
  * \file PnlPlnrAriList.js
  * web client functionality for panel PnlPlnrAriList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlPlnrAriList_aside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrAriList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlPlnrAriList_bside.html";
	doc.getElementById("List_cont").src = "./PnlPlnrAriList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrAriList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrAriList", "Trs"));
...
function init() {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatShrPlnrAriList", "srefIxPlnrVExpstate");
...
function refreshA() {
// IP refreshA.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrAriList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrAriList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrAriList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrAriList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrAriList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrAriList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrAriList", "ntot") + ")");
	};
// IP refreshA.pre --- END
};
...
function refreshB(chgcol) {
	...
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriList", "ntot"));
...
// IP refreshB.pre --- BEGIN
	var TxtFor = retrieveCi(srcdoc, "ContInfPlnrAriList", "TxtFor");

	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagPlnrAriList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfPlnrAriList", "TxtPre"));
// IP refreshB.pre --- END

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "ncol"));

		var TcoIWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoIWidth"));
		var TcoJWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoJWidth"));
		var TcoKWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoKWidth"));
		var TcoLWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoLWidth"));
		var TcoMWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoMWidth"));
		var TcoNWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoNWidth"));
		var TcoArrWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoArrWidth"));
		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoDsnWidth"));
		var TcoDxWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoDxWidth"));
		var TcoDyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriList", "TcoDyWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrAriList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoI"); doc.widths.push(TcoIWidth); doc.fs.push("i"); doc.tos.push(1); wtot += TcoIWidth;
			} else if (i == 2) {
				doc.cols.push("TcoJ"); doc.widths.push(TcoJWidth); doc.fs.push("j"); doc.tos.push(2); wtot += TcoJWidth;
			} else if (i == 3) {
				doc.cols.push("TcoK"); doc.widths.push(TcoKWidth); doc.fs.push("k"); doc.tos.push(3); wtot += TcoKWidth;
			} else if (i == 4) {
				doc.cols.push("TcoL"); doc.widths.push(TcoLWidth); doc.fs.push("l"); doc.tos.push(4); wtot += TcoLWidth;
			} else if (i == 5) {
				doc.cols.push("TcoM"); doc.widths.push(TcoMWidth); doc.fs.push("m"); doc.tos.push(5); wtot += TcoMWidth;
			} else if (i == 6) {
				doc.cols.push("TcoN"); doc.widths.push(TcoNWidth); doc.fs.push("n"); doc.tos.push(6); wtot += TcoNWidth;
			} else if (i == 7) {
				doc.cols.push("TcoArr"); doc.widths.push(TcoArrWidth); doc.fs.push("arr"); doc.tos.push(7); wtot += TcoArrWidth;
			} else if (i == 8) {
				doc.cols.push("TcoDsn"); doc.widths.push(TcoDsnWidth); doc.fs.push("dsn"); doc.tos.push(8); wtot += TcoDsnWidth;
			} else if (i == 9) {
				doc.cols.push("TcoDx"); doc.widths.push(TcoDxWidth); doc.fs.push("dx"); doc.tos.push(0); wtot += TcoDxWidth;
			} else if (i == 10) {
				doc.cols.push("TcoDy"); doc.widths.push(TcoDyWidth); doc.fs.push("dy"); doc.tos.push(0); wtot += TcoDyWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagPlnrAriList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrAriList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQAriList
		rownode = getRownode(srcdoc, "ListPlnrQAriList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrAriList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrAriList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrAriList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrAriList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrAriList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrAriListData", scrJref, "StgIacQryPlnrAriList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriList", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrAriList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrAriListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacPlnrAriList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacPlnrAriList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrAriListData", scrJref, "ContIacPlnrAriList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "ContIacPlnrAriList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "ContInfPlnrAriList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "StatShrPlnrAriList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "StgIacPlnrAriList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "TagPlnrAriList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "ListPlnrQAriList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "StatAppQryPlnrAriList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "StatShrQryPlnrAriList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrAriListData", "StgIacQryPlnrAriList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngPlnrAriListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrAriListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrAriListData") {
				mask = mergeDpchEngData(dom);

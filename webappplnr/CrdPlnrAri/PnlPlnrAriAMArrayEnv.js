/**
  * \file PnlPlnrAriAMArrayEnv.js
  * web client functionality for panel PnlPlnrAriAMArrayEnv
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrAriAMArrayEnv", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrAriAMArrayEnv", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("AMArrayEnv", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("AMArrayEnv_side").setAttribute("height", "30");
		doc.getElementById("AMArrayEnv_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("AMArrayEnv_side").src = "./PnlPlnrAriAMArrayEnv_aside.html";
		doc.getElementById("AMArrayEnv_cont").src = "./PnlPlnrAriAMArrayEnv_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrAriAMArrayEnv", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("AMArrayEnv", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("AMArrayEnv_side").setAttribute("height", "255");
		doc.getElementById("AMArrayEnv_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("AMArrayEnv_side").src = "./PnlPlnrAriAMArrayEnv_bside.html";
		doc.getElementById("AMArrayEnv_cont").src = "./PnlPlnrAriAMArrayEnv_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("AMArrayEnv_side").contentDocument;
	contdoc = doc.getElementById("AMArrayEnv_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("AMArrayEnv_side").contentDocument;
	contdoc = doc.getElementById("AMArrayEnv_cont").contentDocument;
	hdrdoc = contdoc.getElementById("AMArrayEnv_hdr").contentDocument;
	tbldoc = contdoc.getElementById("AMArrayEnv_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("AMArrayEnv_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrAriAMArrayEnv", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriAMArrayEnv", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriAMArrayEnv", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriAMArrayEnv", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriAMArrayEnv", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "ncol"));

		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriAMArrayEnv", "TcoKeyWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrAriAMArrayEnv", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQAriAMArrayEnv
		rownode = getRownode(srcdoc, "ListPlnrQAriAMArrayEnv", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrAriAMArrayEnv", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrAriAMArrayEnv", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("AMArrayEnv", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("AMArrayEnv_side").setAttribute("height", "" + (height+55));
	doc.getElementById("AMArrayEnv_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("AMArrayEnv_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("AMArrayEnv_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriAMArrayEnv", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrAriAMArrayEnv", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrAriAMArrayEnv", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrAriAMArrayEnv", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrAriAMArrayEnvData", scrJref, "StgIacQryPlnrAriAMArrayEnv");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrAriAMArrayEnv", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrAriAMArrayEnv", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrAriAMArrayEnvDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "ContInfPlnrAriAMArrayEnv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StatAppPlnrAriAMArrayEnv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StatShrPlnrAriAMArrayEnv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StgIacPlnrAriAMArrayEnv", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "TagPlnrAriAMArrayEnv", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "ListPlnrQAriAMArrayEnv", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StatAppQryPlnrAriAMArrayEnv", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StatShrQryPlnrAriAMArrayEnv", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrAriAMArrayEnvData", "StgIacQryPlnrAriAMArrayEnv", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrAriAMArrayEnvData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrAriAMArrayEnvData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("AMArrayEnv");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrAriAMArrayEnvData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

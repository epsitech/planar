/**
  * \file PnlPlnrMatMod1NMaterial.js
  * web client functionality for panel PnlPlnrMatMod1NMaterial
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrMatMod1NMaterial", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrMatMod1NMaterial", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("Mod1NMaterial", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("Mod1NMaterial_side").setAttribute("height", "30");
		doc.getElementById("Mod1NMaterial_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("Mod1NMaterial_side").src = "./PnlPlnrMatMod1NMaterial_aside.html";
		doc.getElementById("Mod1NMaterial_cont").src = "./PnlPlnrMatMod1NMaterial_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrMatMod1NMaterial", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("Mod1NMaterial", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("Mod1NMaterial_side").setAttribute("height", "255");
		doc.getElementById("Mod1NMaterial_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("Mod1NMaterial_side").src = "./PnlPlnrMatMod1NMaterial_bside.html";
		doc.getElementById("Mod1NMaterial_cont").src = "./PnlPlnrMatMod1NMaterial_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("Mod1NMaterial_side").contentDocument;
	contdoc = doc.getElementById("Mod1NMaterial_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("Mod1NMaterial_side").contentDocument;
	contdoc = doc.getElementById("Mod1NMaterial_cont").contentDocument;
	hdrdoc = contdoc.getElementById("Mod1NMaterial_hdr").contentDocument;
	tbldoc = contdoc.getElementById("Mod1NMaterial_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("Mod1NMaterial_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrMatMod1NMaterial", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatMod1NMaterial", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatMod1NMaterial", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatMod1NMaterial", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatMod1NMaterial", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatMod1NMaterial", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQMatMod1NMaterial
		rownode = getRownode(srcdoc, "ListPlnrQMatMod1NMaterial", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrMatMod1NMaterial", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrMatMod1NMaterial", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("Mod1NMaterial", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("Mod1NMaterial_side").setAttribute("height", "" + (height+55));
	doc.getElementById("Mod1NMaterial_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("Mod1NMaterial_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("Mod1NMaterial_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatMod1NMaterial", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatMod1NMaterial", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrMatMod1NMaterial", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrMatMod1NMaterial", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrMatMod1NMaterialData", scrJref, "StgIacQryPlnrMatMod1NMaterial");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrMatMod1NMaterial", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatMod1NMaterial", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrMatMod1NMaterialDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "ContInfPlnrMatMod1NMaterial", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StatAppPlnrMatMod1NMaterial", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StatShrPlnrMatMod1NMaterial", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StgIacPlnrMatMod1NMaterial", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "TagPlnrMatMod1NMaterial", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "ListPlnrQMatMod1NMaterial", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StatAppQryPlnrMatMod1NMaterial", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StatShrQryPlnrMatMod1NMaterial", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrMatMod1NMaterialData", "StgIacQryPlnrMatMod1NMaterial", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrMatMod1NMaterialData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrMatMod1NMaterialData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("Mod1NMaterial");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrMatMod1NMaterialData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

/**
  * \file PnlPlnrMatAPar.js
  * web client functionality for panel PnlPlnrMatAPar
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrMatAPar", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrMatAPar", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrMatAPar_aside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrMatAPar_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrMatAPar", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("APar_side").src = "./PnlPlnrMatAPar_bside.html";
		doc.getElementById("APar_cont").src = "./PnlPlnrMatAPar_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMatAPar", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrMatAPar", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMatAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrMatAPar", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "ncol"));

		var TcoDsnWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoDsnWidth"));
		var TcoCatWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoCatWidth"));
		var TcoPrpWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoPrpWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoFilWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoValWidth"));
		var TcoCmtWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMatAPar", "TcoCmtWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrMatAPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrMatAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQMatAPar
		rownode = getRownode(srcdoc, "ListPlnrQMatAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrMatAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrMatAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrMatAPar", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMatAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrMatAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrMatAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrMatAParData", scrJref, "StgIacQryPlnrMatAPar");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMatAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrMatAPar", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMatAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrMatAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "ContInfPlnrMatAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StatAppPlnrMatAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StatShrPlnrMatAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StgIacPlnrMatAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "TagPlnrMatAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "ListPlnrQMatAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StatAppQryPlnrMatAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StatShrQryPlnrMatAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrMatAParData", "StgIacQryPlnrMatAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrMatAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrMatAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrMatAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

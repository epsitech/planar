/**
  * \file PnlPlnrMat1NLayer.js
  * web client functionality for panel PnlPlnrMat1NLayer
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrMat1NLayer", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrMat1NLayer", "srefIxPlnrVExpstate", "mind");

		getRecwnd().changeHeight("1NLayer", 30, updh);
		doc.getElementById("tdSide").setAttribute("height", "30");
		doc.getElementById("1NLayer_side").setAttribute("height", "30");
		doc.getElementById("1NLayer_cont").setAttribute("height", "30");

		// change content
		doc.getElementById("1NLayer_side").src = "./PnlPlnrMat1NLayer_aside.html";
		doc.getElementById("1NLayer_cont").src = "./PnlPlnrMat1NLayer_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrMat1NLayer", "srefIxPlnrVExpstate", "regd");

		getRecwnd().changeHeight("1NLayer", 255, true);
		doc.getElementById("tdSide").setAttribute("height", "255");
		doc.getElementById("1NLayer_side").setAttribute("height", "255");
		doc.getElementById("1NLayer_cont").setAttribute("height", "255");

		// change content
		doc.getElementById("1NLayer_side").src = "./PnlPlnrMat1NLayer_bside.html";
		doc.getElementById("1NLayer_cont").src = "./PnlPlnrMat1NLayer_b.html";
	};
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NLayer_side").contentDocument;
	contdoc = doc.getElementById("1NLayer_cont").contentDocument;
...
function initB() {
	...
	sidedoc = doc.getElementById("1NLayer_side").contentDocument;
	contdoc = doc.getElementById("1NLayer_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NLayer_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NLayer_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NLayer_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMat1NLayer", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrMat1NLayer", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrMat1NLayer", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrMat1NLayer", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMat1NLayer", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMat1NLayer", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMat1NLayer", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMat1NLayer", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrMat1NLayer", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrMat1NLayer", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrMat1NLayer", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQMat1NLayer
		rownode = getRownode(srcdoc, "ListPlnrQMat1NLayer", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrMat1NLayer", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrMat1NLayer", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrMat1NLayer", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NLayer", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NLayer_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NLayer_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NLayer_tbl").setAttribute("height", "" + height);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NLayer_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMat1NLayer", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrMat1NLayer", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrMat1NLayer", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrMat1NLayer", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrMat1NLayerData", scrJref, "StgIacQryPlnrMat1NLayer");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrMat1NLayer", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrMat1NLayer", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrMat1NLayer", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrMat1NLayerDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "ContInfPlnrMat1NLayer", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StatAppPlnrMat1NLayer", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StatShrPlnrMat1NLayer", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StgIacPlnrMat1NLayer", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "TagPlnrMat1NLayer", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "ListPlnrQMat1NLayer", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StatAppQryPlnrMat1NLayer", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StatShrQryPlnrMat1NLayer", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrMat1NLayerData", "StgIacQryPlnrMat1NLayer", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrMat1NLayerData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrMat1NLayerData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NLayer");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrMat1NLayerData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

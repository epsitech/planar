/**
  * \file PnlPlnrPrsADetail.js
  * web client functionality for panel PnlPlnrPrsADetail
...
function minimize(updh) {
	var srefIxPlnrVExpstate = retrieveSi(srcdoc, "StatAppPlnrPrsADetail", "srefIxPlnrVExpstate");

	if (srefIxPlnrVExpstate != "mind") {
		setSi(srcdoc, "StatAppPlnrPrsADetail", "srefIxPlnrVExpstate", "mind");
		...
		// change content
		doc.getElementById("ADetail_side").src = "./PnlPlnrPrsADetail_aside.html";
		doc.getElementById("ADetail_cont").src = "./PnlPlnrPrsADetail_a.html";
	};
};
...
function regularize() {
	...
	if (srefIxPlnrVExpstate != "regd") {
		setSi(srcdoc, "StatAppPlnrPrsADetail", "srefIxPlnrVExpstate", "regd");
		...
		// change content
		doc.getElementById("ADetail_side").src = "./PnlPlnrPrsADetail_bside.html";
		doc.getElementById("ADetail_cont").src = "./PnlPlnrPrsADetail_b.html";
	};
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrPrsADetail", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagPlnrPrsADetail", "Trs"));
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagPlnrPrsADetail", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryPlnrPrsADetail", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrPrsADetail", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrPrsADetail", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrPrsADetail", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrPrsADetail", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "ncol"));

		var TcoTypWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrPrsADetail", "TcoTypWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacPlnrPrsADetail", "TcoValWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagPlnrPrsADetail", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryPlnrPrsADetail", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListPlnrQPrsADetail
		rownode = getRownode(srcdoc, "ListPlnrQPrsADetail", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagPlnrPrsADetail", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagPlnrPrsADetail", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfPlnrPrsADetail", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrPrsADetail", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryPlnrPrsADetail", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryPlnrPrsADetail", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryPlnrPrsADetail", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppPlnrPrsADetailData", scrJref, "StgIacQryPlnrPrsADetail");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryPlnrPrsADetail", "firstcol"));

	setSi(srcdoc, "StatAppQryPlnrPrsADetail", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryPlnrPrsADetail", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppPlnrPrsADetailDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "ContInfPlnrPrsADetail", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StatAppPlnrPrsADetail", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StatShrPlnrPrsADetail", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StgIacPlnrPrsADetail", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "TagPlnrPrsADetail", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "ListPlnrQPrsADetail", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StatAppQryPlnrPrsADetail", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StatShrQryPlnrPrsADetail", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngPlnrPrsADetailData", "StgIacQryPlnrPrsADetail", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngPlnrPrsADetailData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngPlnrPrsADetailData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngPlnrPrsADetailData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};

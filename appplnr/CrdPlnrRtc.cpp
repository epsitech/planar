/**
  * \file CrdPlnrRtc.cpp
  * app access code for job CrdPlnrRtc (implementation)
...
/******************************************************************************
 class CrdPlnrRtc::VecVDo
 ******************************************************************************/

uint CrdPlnrRtc::VecVDo::getIx(
			const string& sref
		) {
...
string CrdPlnrRtc::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrRtc::VecVSge
 ******************************************************************************/

uint CrdPlnrRtc::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CrdPlnrRtc::VecVSge::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrRtc::ContInf
 ******************************************************************************/

CrdPlnrRtc::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdRtc
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdRtc = MtxCrdRtc;

	mask = {NUMFSGE, MRLAPPHLP, MTXCRDRTC};
};

bool CrdPlnrRtc::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrRtc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrRtc";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdRtc", MtxCrdRtc)) add(MTXCRDRTC);
	};

	return basefound;
};

set<uint> CrdPlnrRtc::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdRtc.compare(comp->MtxCrdRtc) == 0) insert(items, MTXCRDRTC);

	return(items);
};

set<uint> CrdPlnrRtc::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDRTC};
...
/******************************************************************************
 class CrdPlnrRtc::StatApp
 ******************************************************************************/

CrdPlnrRtc::StatApp::StatApp(
...
bool CrdPlnrRtc::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrRtc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrRtc";
...
set<uint> CrdPlnrRtc::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrRtc::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdPlnrRtc::StatShr
 ******************************************************************************/

CrdPlnrRtc::StatShr::StatShr(
...
bool CrdPlnrRtc::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrRtc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrRtc";
...
set<uint> CrdPlnrRtc::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrRtc::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdPlnrRtc::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrRtc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrRtc";
...
/******************************************************************************
 class CrdPlnrRtc::DpchAppDo
 ******************************************************************************/

CrdPlnrRtc::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRRTCDO, scrJref) {
...
void CrdPlnrRtc::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrRtcDo");
...
/******************************************************************************
 class CrdPlnrRtc::DpchEngData
 ******************************************************************************/

CrdPlnrRtc::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRRTCDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrRtc::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrRtcData");
	else
		basefound = checkXPath(docctx, basexpath);

/**
  * \file PnlPlnrFilList.cpp
  * app access code for job PnlPlnrFilList (implementation)
...
/******************************************************************************
 class PnlPlnrFilList::VecVDo
 ******************************************************************************/

uint PnlPlnrFilList::VecVDo::getIx(
			const string& sref
		) {
...
string PnlPlnrFilList::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class PnlPlnrFilList::ContIac
 ******************************************************************************/

PnlPlnrFilList::ContIac::ContIac(
			const uint numFTos
		) : Block() {
...
bool PnlPlnrFilList::ContIac::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContIacPlnrFilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemIacPlnrFilList";
...
void PnlPlnrFilList::ContIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContIacPlnrFilList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemIacPlnrFilList";
...
set<uint> PnlPlnrFilList::ContIac::comm(
			const ContIac* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrFilList::ContIac::diff(
			const ContIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlPlnrFilList::ContInf
 ******************************************************************************/

PnlPlnrFilList::ContInf::ContInf(
...
bool PnlPlnrFilList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrFilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrFilList";
...
set<uint> PnlPlnrFilList::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrFilList::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlPlnrFilList::StatShr
 ******************************************************************************/

PnlPlnrFilList::StatShr::StatShr(
...
bool PnlPlnrFilList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrFilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrFilList";
...
set<uint> PnlPlnrFilList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrFilList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlPlnrFilList::StgIac
 ******************************************************************************/

PnlPlnrFilList::StgIac::StgIac(
...
bool PnlPlnrFilList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacPlnrFilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacPlnrFilList";
...
void PnlPlnrFilList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacPlnrFilList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacPlnrFilList";
...
set<uint> PnlPlnrFilList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrFilList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class PnlPlnrFilList::Tag
 ******************************************************************************/

PnlPlnrFilList::Tag::Tag(
			const string& Cpt
			, const string& Trs
	...
	mask = {CPT, TRS, TXTSHOWING1, TXTSHOWING2, TCOFNM, TCORET, TCOREU, TCOCNT, TCOMIM, TCOSIZ};
};

bool PnlPlnrFilList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrFilList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrFilList";
...
/******************************************************************************
 class PnlPlnrFilList::DpchAppData
 ******************************************************************************/

PnlPlnrFilList::DpchAppData::DpchAppData(
			const string& scrJref
			, ContIac* contiac
			, StgIac* stgiac
			, QryPlnrFilList::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRFILLISTDATA, scrJref) {
...
void PnlPlnrFilList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrFilListData");
...
/******************************************************************************
 class PnlPlnrFilList::DpchAppDo
 ******************************************************************************/

PnlPlnrFilList::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRFILLISTDO, scrJref) {
...
void PnlPlnrFilList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrFilListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
/******************************************************************************
 class PnlPlnrFilList::DpchEngData
 ******************************************************************************/

PnlPlnrFilList::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRFILLISTDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
	feedFTos.tag = "FeedFTos";
};

void PnlPlnrFilList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrFilListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrFilList::StatApp();
		statshrqry = QryPlnrFilList::StatShr();
		stgiacqry = QryPlnrFilList::StgIac();
	};
};

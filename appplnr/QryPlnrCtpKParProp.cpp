/**
  ...
  * app access code for job QryPlnrCtpKParProp (implementation)
...
/******************************************************************************
 class QryPlnrCtpKParProp::StatApp
 ******************************************************************************/

QryPlnrCtpKParProp::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpKParProp::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpKParProp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpKParProp";
...
set<uint> QryPlnrCtpKParProp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKParProp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpKParProp::StatShr
 ******************************************************************************/

QryPlnrCtpKParProp::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrCtpKParProp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpKParProp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpKParProp";
...
set<uint> QryPlnrCtpKParProp::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKParProp::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpKParProp::StgIac
 ******************************************************************************/

QryPlnrCtpKParProp::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrCtpKParProp::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrCtpKParProp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrCtpKParProp";
...
void QryPlnrCtpKParProp::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrCtpKParProp";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrCtpKParProp";
...
set<uint> QryPlnrCtpKParProp::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKParProp::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

/**
  * \file CrdPlnrStk.cpp
  * app access code for job CrdPlnrStk (implementation)
...
/******************************************************************************
 class CrdPlnrStk::VecVDo
 ******************************************************************************/

uint CrdPlnrStk::VecVDo::getIx(
			const string& sref
		) {
...
string CrdPlnrStk::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrStk::VecVSge
 ******************************************************************************/

uint CrdPlnrStk::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CrdPlnrStk::VecVSge::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrStk::ContInf
 ******************************************************************************/

CrdPlnrStk::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdStk
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdStk = MtxCrdStk;

	mask = {NUMFSGE, MRLAPPHLP, MTXCRDSTK};
};

bool CrdPlnrStk::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrStk");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrStk";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdStk", MtxCrdStk)) add(MTXCRDSTK);
	};

	return basefound;
};

set<uint> CrdPlnrStk::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdStk.compare(comp->MtxCrdStk) == 0) insert(items, MTXCRDSTK);

	return(items);
};

set<uint> CrdPlnrStk::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDSTK};
...
/******************************************************************************
 class CrdPlnrStk::StatApp
 ******************************************************************************/

CrdPlnrStk::StatApp::StatApp(
...
bool CrdPlnrStk::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrStk");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrStk";
...
set<uint> CrdPlnrStk::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrStk::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdPlnrStk::StatShr
 ******************************************************************************/

CrdPlnrStk::StatShr::StatShr(
...
bool CrdPlnrStk::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrStk");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrStk";
...
set<uint> CrdPlnrStk::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrStk::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdPlnrStk::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrStk");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrStk";
...
/******************************************************************************
 class CrdPlnrStk::DpchAppDo
 ******************************************************************************/

CrdPlnrStk::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSTKDO, scrJref) {
...
void CrdPlnrStk::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrStkDo");
...
/******************************************************************************
 class CrdPlnrStk::DpchEngData
 ******************************************************************************/

CrdPlnrStk::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRSTKDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrStk::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrStkData");
	else
		basefound = checkXPath(docctx, basexpath);

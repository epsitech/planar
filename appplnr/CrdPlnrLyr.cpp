/**
  * \file CrdPlnrLyr.cpp
  * app access code for job CrdPlnrLyr (implementation)
...
/******************************************************************************
 class CrdPlnrLyr::VecVDo
 ******************************************************************************/

uint CrdPlnrLyr::VecVDo::getIx(
			const string& sref
		) {
...
string CrdPlnrLyr::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrLyr::VecVSge
 ******************************************************************************/

uint CrdPlnrLyr::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CrdPlnrLyr::VecVSge::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrLyr::ContInf
 ******************************************************************************/

CrdPlnrLyr::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdLyr
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdLyr = MtxCrdLyr;

	mask = {NUMFSGE, MRLAPPHLP, MTXCRDLYR};
};

bool CrdPlnrLyr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrLyr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrLyr";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdLyr", MtxCrdLyr)) add(MTXCRDLYR);
	};

	return basefound;
};

set<uint> CrdPlnrLyr::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdLyr.compare(comp->MtxCrdLyr) == 0) insert(items, MTXCRDLYR);

	return(items);
};

set<uint> CrdPlnrLyr::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDLYR};
...
/******************************************************************************
 class CrdPlnrLyr::StatApp
 ******************************************************************************/

CrdPlnrLyr::StatApp::StatApp(
...
bool CrdPlnrLyr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrLyr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrLyr";
...
set<uint> CrdPlnrLyr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrLyr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdPlnrLyr::StatShr
 ******************************************************************************/

CrdPlnrLyr::StatShr::StatShr(
...
bool CrdPlnrLyr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrLyr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrLyr";
...
set<uint> CrdPlnrLyr::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrLyr::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdPlnrLyr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrLyr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrLyr";
...
/******************************************************************************
 class CrdPlnrLyr::DpchAppDo
 ******************************************************************************/

CrdPlnrLyr::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRLYRDO, scrJref) {
...
void CrdPlnrLyr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrLyrDo");
...
/******************************************************************************
 class CrdPlnrLyr::DpchEngData
 ******************************************************************************/

CrdPlnrLyr::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRLYRDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrLyr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrLyrData");
	else
		basefound = checkXPath(docctx, basexpath);

/**
  ...
  * app access code for job QryPlnrAriAMArrayEnv (implementation)
...
QryPlnrAriAMArrayEnv::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrAriAMArrayEnv::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrAriAMArrayEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrAriAMArrayEnv";
...
set<uint> QryPlnrAriAMArrayEnv::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrAriAMArrayEnv::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrAriAMArrayEnv::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrAriAMArrayEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrAriAMArrayEnv";

/**
  ...
  * app access code for job QryPlnrDsnHk1NVertex (implementation)
...
QryPlnrDsnHk1NVertex::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrDsnHk1NVertex::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrDsnHk1NVertex");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrDsnHk1NVertex";
...
set<uint> QryPlnrDsnHk1NVertex::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrDsnHk1NVertex::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrDsnHk1NVertex::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrDsnHk1NVertex");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrDsnHk1NVertex";

/**
  ...
  * app access code for job QryPlnrDtpSup1NDevice (implementation)
...
QryPlnrDtpSup1NDevice::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrDtpSup1NDevice::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrDtpSup1NDevice");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrDtpSup1NDevice";
...
set<uint> QryPlnrDtpSup1NDevice::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrDtpSup1NDevice::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrDtpSup1NDevice::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrDtpSup1NDevice");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrDtpSup1NDevice";

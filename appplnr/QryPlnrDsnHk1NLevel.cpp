/**
  ...
  * app access code for job QryPlnrDsnHk1NLevel (implementation)
...
QryPlnrDsnHk1NLevel::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrDsnHk1NLevel::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrDsnHk1NLevel");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrDsnHk1NLevel";
...
set<uint> QryPlnrDsnHk1NLevel::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrDsnHk1NLevel::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrDsnHk1NLevel::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrDsnHk1NLevel");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrDsnHk1NLevel";

/**
  * \file CrdPlnrMat.cpp
  * app access code for job CrdPlnrMat (implementation)
...
/******************************************************************************
 class CrdPlnrMat::VecVDo
 ******************************************************************************/

uint CrdPlnrMat::VecVDo::getIx(
			const string& sref
		) {
...
string CrdPlnrMat::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrMat::VecVSge
 ******************************************************************************/

uint CrdPlnrMat::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CrdPlnrMat::VecVSge::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrMat::ContInf
 ******************************************************************************/

CrdPlnrMat::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdMat
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdMat = MtxCrdMat;

	mask = {NUMFSGE, MRLAPPHLP, MTXCRDMAT};
};

bool CrdPlnrMat::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrMat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrMat";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdMat", MtxCrdMat)) add(MTXCRDMAT);
	};

	return basefound;
};

set<uint> CrdPlnrMat::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdMat.compare(comp->MtxCrdMat) == 0) insert(items, MTXCRDMAT);

	return(items);
};

set<uint> CrdPlnrMat::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDMAT};
...
/******************************************************************************
 class CrdPlnrMat::StatApp
 ******************************************************************************/

CrdPlnrMat::StatApp::StatApp(
...
bool CrdPlnrMat::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrMat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrMat";
...
set<uint> CrdPlnrMat::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrMat::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdPlnrMat::StatShr
 ******************************************************************************/

CrdPlnrMat::StatShr::StatShr(
...
bool CrdPlnrMat::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrMat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrMat";
...
set<uint> CrdPlnrMat::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
bool CrdPlnrMat::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrMat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrMat";
...
/******************************************************************************
 class CrdPlnrMat::DpchAppDo
 ******************************************************************************/

CrdPlnrMat::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRMATDO, scrJref) {
...
void CrdPlnrMat::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrMatDo");
...
/******************************************************************************
 class CrdPlnrMat::DpchEngData
 ******************************************************************************/

CrdPlnrMat::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRMATDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrMat::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrMatData");
	else
		basefound = checkXPath(docctx, basexpath);

/**
  * \file CrdPlnrDom.cpp
  * app access code for job CrdPlnrDom (implementation)
...
bool CrdPlnrDom::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrDom");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrDom";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdDom", MtxCrdDom)) add(MTXCRDDOM);
	};
...
set<uint> CrdPlnrDom::ContInf::comm(
	...
	if (MtxCrdDom.compare(comp->MtxCrdDom) == 0) insert(items, MTXCRDDOM);
...
bool CrdPlnrDom::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrDom");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrDom";
...
set<uint> CrdPlnrDom::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
bool CrdPlnrDom::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrDom");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrDom";
...
bool CrdPlnrDom::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrDom");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrDom";
...
CrdPlnrDom::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDOMDO, scrJref) {
...
void CrdPlnrDom::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrDomDo");
...
CrdPlnrDom::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDOMDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrDom::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrDomData");
	else
		basefound = checkXPath(docctx, basexpath);

/**
  ...
  * app access code for job QryPlnrCtpKEnvKey (implementation)
...
/******************************************************************************
 class QryPlnrCtpKEnvKey::StatApp
 ******************************************************************************/

QryPlnrCtpKEnvKey::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpKEnvKey::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpKEnvKey");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpKEnvKey";
...
set<uint> QryPlnrCtpKEnvKey::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKEnvKey::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpKEnvKey::StatShr
 ******************************************************************************/

QryPlnrCtpKEnvKey::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrCtpKEnvKey::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpKEnvKey");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpKEnvKey";
...
set<uint> QryPlnrCtpKEnvKey::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKEnvKey::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpKEnvKey::StgIac
 ******************************************************************************/

QryPlnrCtpKEnvKey::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrCtpKEnvKey::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrCtpKEnvKey");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrCtpKEnvKey";
...
void QryPlnrCtpKEnvKey::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrCtpKEnvKey";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrCtpKEnvKey";
...
set<uint> QryPlnrCtpKEnvKey::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKEnvKey::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

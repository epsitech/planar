/**
  ...
  * app access code for job QryPlnrCtpKCalcdomprp (implementation)
...
QryPlnrCtpKCalcdomprp::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpKCalcdomprp::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpKCalcdomprp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpKCalcdomprp";
...
set<uint> QryPlnrCtpKCalcdomprp::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpKCalcdomprp::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrCtpKCalcdomprp::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpKCalcdomprp");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpKCalcdomprp";

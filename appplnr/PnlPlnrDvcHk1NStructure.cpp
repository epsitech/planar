/**
  ...
  * app access code for job PnlPlnrDvcHk1NStructure (implementation)
...
bool PnlPlnrDvcHk1NStructure::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrDvcHk1NStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrDvcHk1NStructure";
...
PnlPlnrDvcHk1NStructure::StatApp::StatApp(
...
bool PnlPlnrDvcHk1NStructure::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrDvcHk1NStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrDvcHk1NStructure";
...
set<uint> PnlPlnrDvcHk1NStructure::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrDvcHk1NStructure::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrDvcHk1NStructure::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrDvcHk1NStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrDvcHk1NStructure";
...
PnlPlnrDvcHk1NStructure::Tag::Tag(
			const string& Cpt
			, const string& Trs
...
bool PnlPlnrDvcHk1NStructure::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrDvcHk1NStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrDvcHk1NStructure";
...
PnlPlnrDvcHk1NStructure::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrDvcHk1NStructure::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDVCHK1NSTRUCTUREDATA, scrJref) {
...
void PnlPlnrDvcHk1NStructure::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrDvcHk1NStructureData");
...
PnlPlnrDvcHk1NStructure::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDVCHK1NSTRUCTUREDO, scrJref) {
...
void PnlPlnrDvcHk1NStructure::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrDvcHk1NStructureDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrDvcHk1NStructure::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDVCHK1NSTRUCTUREDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrDvcHk1NStructure::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrDvcHk1NStructureData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrDvcHk1NStructure::StatApp();
		statshrqry = QryPlnrDvcHk1NStructure::StatShr();

/**
  * \file PnlPlnrUsrHeadbar.cpp
  * app access code for job PnlPlnrUsrHeadbar (implementation)
...
PnlPlnrUsrHeadbar::StgInf::StgInf(
...
bool PnlPlnrUsrHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfPlnrUsrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfPlnrUsrHeadbar";
...
set<uint> PnlPlnrUsrHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrUsrHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrUsrHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrUsrHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrUsrHeadbar";
...
PnlPlnrUsrHeadbar::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRUSRHEADBARDATA) {
};

void PnlPlnrUsrHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrUsrHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);

/**
  ...
  * app access code for job PnlPlnrPrjMNTapeout (implementation)
...
bool PnlPlnrPrjMNTapeout::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrPrjMNTapeout");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrPrjMNTapeout";
...
PnlPlnrPrjMNTapeout::StatApp::StatApp(
...
bool PnlPlnrPrjMNTapeout::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrPrjMNTapeout");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrPrjMNTapeout";
...
set<uint> PnlPlnrPrjMNTapeout::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrPrjMNTapeout::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrPrjMNTapeout::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrPrjMNTapeout");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrPrjMNTapeout";
...
bool PnlPlnrPrjMNTapeout::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrPrjMNTapeout");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrPrjMNTapeout";
...
PnlPlnrPrjMNTapeout::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrPrjMNTapeout::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRPRJMNTAPEOUTDATA, scrJref) {
...
void PnlPlnrPrjMNTapeout::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrPrjMNTapeoutData");
...
PnlPlnrPrjMNTapeout::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRPRJMNTAPEOUTDO, scrJref) {
...
void PnlPlnrPrjMNTapeout::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrPrjMNTapeoutDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrPrjMNTapeout::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRPRJMNTAPEOUTDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrPrjMNTapeout::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrPrjMNTapeoutData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrPrjMNTapeout::StatApp();
		statshrqry = QryPlnrPrjMNTapeout::StatShr();

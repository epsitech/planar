/**
  * \file CrdPlnrPrj.cpp
  * app access code for job CrdPlnrPrj (implementation)
...
bool CrdPlnrPrj::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrPrj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrPrj";
...
bool CrdPlnrPrj::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrPrj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrPrj";
...
set<uint> CrdPlnrPrj::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrPrj::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdPlnrPrj::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrPrj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrPrj";
...
bool CrdPlnrPrj::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrPrj");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrPrj";
...
CrdPlnrPrj::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRPRJDO, scrJref) {
...
void CrdPlnrPrj::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrPrjDo");
...
CrdPlnrPrj::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRPRJDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrPrj::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrPrjData");
	else
		basefound = checkXPath(docctx, basexpath);

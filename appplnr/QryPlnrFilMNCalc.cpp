/**
  ...
  * app access code for job QryPlnrFilMNCalc (implementation)
...
/******************************************************************************
 class QryPlnrFilMNCalc::StatApp
 ******************************************************************************/

QryPlnrFilMNCalc::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrFilMNCalc::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrFilMNCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrFilMNCalc";
...
set<uint> QryPlnrFilMNCalc::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrFilMNCalc::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrFilMNCalc::StatShr
 ******************************************************************************/

QryPlnrFilMNCalc::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrFilMNCalc::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrFilMNCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrFilMNCalc";
...
set<uint> QryPlnrFilMNCalc::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrFilMNCalc::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrFilMNCalc::StgIac
 ******************************************************************************/

QryPlnrFilMNCalc::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrFilMNCalc::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrFilMNCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrFilMNCalc";
...
void QryPlnrFilMNCalc::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrFilMNCalc";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrFilMNCalc";
...
set<uint> QryPlnrFilMNCalc::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrFilMNCalc::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

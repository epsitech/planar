/**
  ...
  * app access code for job QryPlnrCtpList (implementation)
...
/******************************************************************************
 class QryPlnrCtpList::VecVOrd
 ******************************************************************************/

uint QryPlnrCtpList::VecVOrd::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);

	if (s.compare("tit") == 0) return TIT;
	else if (s.compare("prj") == 0) return PRJ;
	else if (s.compare("dim") == 0) return DIM;
	else if (s.compare("tpl") == 0) return TPL;
	else if (s.compare("sup") == 0) return SUP;
	else if (s.compare("dsn") == 0) return DSN;

	return(0);
};

string QryPlnrCtpList::VecVOrd::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class QryPlnrCtpList::StatApp
 ******************************************************************************/

QryPlnrCtpList::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpList";
...
set<uint> QryPlnrCtpList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpList::StatShr
 ******************************************************************************/

QryPlnrCtpList::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrCtpList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpList";
...
set<uint> QryPlnrCtpList::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpList::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpList::StgIac
 ******************************************************************************/

QryPlnrCtpList::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrCtpList::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrCtpList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrCtpList";
...
void QryPlnrCtpList::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrCtpList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrCtpList";
...
set<uint> QryPlnrCtpList::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpList::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

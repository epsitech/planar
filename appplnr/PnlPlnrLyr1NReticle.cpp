/**
  * \file PnlPlnrLyr1NReticle.cpp
  * app access code for job PnlPlnrLyr1NReticle (implementation)
...
bool PnlPlnrLyr1NReticle::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrLyr1NReticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrLyr1NReticle";
...
PnlPlnrLyr1NReticle::StatApp::StatApp(
...
bool PnlPlnrLyr1NReticle::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrLyr1NReticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrLyr1NReticle";
...
set<uint> PnlPlnrLyr1NReticle::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrLyr1NReticle::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrLyr1NReticle::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrLyr1NReticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrLyr1NReticle";
...
bool PnlPlnrLyr1NReticle::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrLyr1NReticle");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrLyr1NReticle";
...
PnlPlnrLyr1NReticle::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrLyr1NReticle::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRLYR1NRETICLEDATA, scrJref) {
...
void PnlPlnrLyr1NReticle::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrLyr1NReticleData");
...
PnlPlnrLyr1NReticle::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRLYR1NRETICLEDO, scrJref) {
...
void PnlPlnrLyr1NReticle::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrLyr1NReticleDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrLyr1NReticle::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRLYR1NRETICLEDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrLyr1NReticle::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrLyr1NReticleData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrLyr1NReticle::StatApp();
		statshrqry = QryPlnrLyr1NReticle::StatShr();

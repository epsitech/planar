/**
  ...
  * app access code for job QryPlnrCtpTpl1NCalc (implementation)
...
/******************************************************************************
 class QryPlnrCtpTpl1NCalc::StatApp
 ******************************************************************************/

QryPlnrCtpTpl1NCalc::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpTpl1NCalc::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpTpl1NCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpTpl1NCalc";
...
set<uint> QryPlnrCtpTpl1NCalc::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpTpl1NCalc::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpTpl1NCalc::StatShr
 ******************************************************************************/

QryPlnrCtpTpl1NCalc::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrCtpTpl1NCalc::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpTpl1NCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpTpl1NCalc";
...
set<uint> QryPlnrCtpTpl1NCalc::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
/******************************************************************************
 class QryPlnrCtpTpl1NCalc::StgIac
 ******************************************************************************/

QryPlnrCtpTpl1NCalc::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrCtpTpl1NCalc::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrCtpTpl1NCalc");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrCtpTpl1NCalc";
...
void QryPlnrCtpTpl1NCalc::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrCtpTpl1NCalc";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrCtpTpl1NCalc";
...
set<uint> QryPlnrCtpTpl1NCalc::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpTpl1NCalc::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

/**
  ...
  * app access code for job QryPlnrClcAEnv (implementation)
...
/******************************************************************************
 class QryPlnrClcAEnv::StatApp
 ******************************************************************************/

QryPlnrClcAEnv::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrClcAEnv::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrClcAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrClcAEnv";
...
set<uint> QryPlnrClcAEnv::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrClcAEnv::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrClcAEnv::StatShr
 ******************************************************************************/

QryPlnrClcAEnv::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrClcAEnv::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrClcAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrClcAEnv";
...
set<uint> QryPlnrClcAEnv::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrClcAEnv::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrClcAEnv::StgIac
 ******************************************************************************/

QryPlnrClcAEnv::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrClcAEnv::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrClcAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrClcAEnv";
...
void QryPlnrClcAEnv::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrClcAEnv";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrClcAEnv";
...
set<uint> QryPlnrClcAEnv::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrClcAEnv::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

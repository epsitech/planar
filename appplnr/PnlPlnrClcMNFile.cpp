/**
  * \file PnlPlnrClcMNFile.cpp
  * app access code for job PnlPlnrClcMNFile (implementation)
...
bool PnlPlnrClcMNFile::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrClcMNFile");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrClcMNFile";
...
PnlPlnrClcMNFile::StatApp::StatApp(
...
bool PnlPlnrClcMNFile::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrClcMNFile");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrClcMNFile";
...
set<uint> PnlPlnrClcMNFile::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrClcMNFile::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrClcMNFile::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrClcMNFile");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrClcMNFile";
...
bool PnlPlnrClcMNFile::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrClcMNFile");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrClcMNFile";
...
PnlPlnrClcMNFile::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrClcMNFile::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRCLCMNFILEDATA, scrJref) {
...
void PnlPlnrClcMNFile::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrClcMNFileData");
...
PnlPlnrClcMNFile::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRCLCMNFILEDO, scrJref) {
...
void PnlPlnrClcMNFile::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrClcMNFileDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrClcMNFile::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRCLCMNFILEDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrClcMNFile::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrClcMNFileData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrClcMNFile::StatApp();
		statshrqry = QryPlnrClcMNFile::StatShr();

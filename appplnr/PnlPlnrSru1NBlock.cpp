/**
  ...
  * app access code for job PnlPlnrSru1NBlock (implementation)
...
bool PnlPlnrSru1NBlock::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrSru1NBlock");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrSru1NBlock";
...
PnlPlnrSru1NBlock::StatApp::StatApp(
...
bool PnlPlnrSru1NBlock::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrSru1NBlock");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrSru1NBlock";
...
set<uint> PnlPlnrSru1NBlock::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrSru1NBlock::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrSru1NBlock::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrSru1NBlock");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrSru1NBlock";
...
bool PnlPlnrSru1NBlock::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrSru1NBlock");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrSru1NBlock";
...
PnlPlnrSru1NBlock::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrSru1NBlock::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSRU1NBLOCKDATA, scrJref) {
...
void PnlPlnrSru1NBlock::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrSru1NBlockData");
...
PnlPlnrSru1NBlock::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSRU1NBLOCKDO, scrJref) {
...
void PnlPlnrSru1NBlock::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrSru1NBlockDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrSru1NBlock::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRSRU1NBLOCKDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrSru1NBlock::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrSru1NBlockData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrSru1NBlock::StatApp();
		statshrqry = QryPlnrSru1NBlock::StatShr();

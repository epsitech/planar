/**
  * \file CrdPlnrCli.cpp
  * app access code for job CrdPlnrCli (implementation)
...
/******************************************************************************
 class CrdPlnrCli::VecVDo
 ******************************************************************************/

uint CrdPlnrCli::VecVDo::getIx(
			const string& sref
		) {
...
string CrdPlnrCli::VecVDo::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrCli::VecVSge
 ******************************************************************************/

uint CrdPlnrCli::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string CrdPlnrCli::VecVSge::getSref(
			const uint ix
		) {
...
/******************************************************************************
 class CrdPlnrCli::ContInf
 ******************************************************************************/

CrdPlnrCli::ContInf::ContInf(
			const uint numFSge
			, const string& MrlAppHlp
			, const string& MtxCrdCli
		) : Block() {
	this->numFSge = numFSge;
	this->MrlAppHlp = MrlAppHlp;
	this->MtxCrdCli = MtxCrdCli;

	mask = {NUMFSGE, MRLAPPHLP, MTXCRDCLI};
};

bool CrdPlnrCli::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrCli");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrCli";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdCli", MtxCrdCli)) add(MTXCRDCLI);
	};

	return basefound;
};

set<uint> CrdPlnrCli::ContInf::comm(
			const ContInf* comp
		) {
	set<uint> items;
	...
	if (MtxCrdCli.compare(comp->MtxCrdCli) == 0) insert(items, MTXCRDCLI);

	return(items);
};

set<uint> CrdPlnrCli::ContInf::diff(
			const ContInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {NUMFSGE, MRLAPPHLP, MTXCRDCLI};
...
/******************************************************************************
 class CrdPlnrCli::StatApp
 ******************************************************************************/

CrdPlnrCli::StatApp::StatApp(
...
bool CrdPlnrCli::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrCli");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrCli";
...
set<uint> CrdPlnrCli::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrCli::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class CrdPlnrCli::StatShr
 ******************************************************************************/

CrdPlnrCli::StatShr::StatShr(
...
bool CrdPlnrCli::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrCli");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrCli";
...
set<uint> CrdPlnrCli::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> CrdPlnrCli::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdPlnrCli::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrCli");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrCli";
...
/******************************************************************************
 class CrdPlnrCli::DpchAppDo
 ******************************************************************************/

CrdPlnrCli::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRCLIDO, scrJref) {
...
void CrdPlnrCli::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrCliDo");
...
/******************************************************************************
 class CrdPlnrCli::DpchEngData
 ******************************************************************************/

CrdPlnrCli::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRCLIDATA) {
	feedFSge.tag = "FeedFSge";
};

void CrdPlnrCli::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrCliData");
	else
		basefound = checkXPath(docctx, basexpath);

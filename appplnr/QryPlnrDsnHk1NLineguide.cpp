/**
  ...
  * app access code for job QryPlnrDsnHk1NLineguide (implementation)
...
QryPlnrDsnHk1NLineguide::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrDsnHk1NLineguide::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrDsnHk1NLineguide");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrDsnHk1NLineguide";
...
set<uint> QryPlnrDsnHk1NLineguide::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrDsnHk1NLineguide::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrDsnHk1NLineguide::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrDsnHk1NLineguide");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrDsnHk1NLineguide";

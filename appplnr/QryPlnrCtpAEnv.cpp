/**
  ...
  * app access code for job QryPlnrCtpAEnv (implementation)
...
/******************************************************************************
 class QryPlnrCtpAEnv::StatApp
 ******************************************************************************/

QryPlnrCtpAEnv::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrCtpAEnv::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrCtpAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrCtpAEnv";
...
set<uint> QryPlnrCtpAEnv::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpAEnv::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpAEnv::StatShr
 ******************************************************************************/

QryPlnrCtpAEnv::StatShr::StatShr(
			const uint ntot
...
bool QryPlnrCtpAEnv::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrCtpAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrCtpAEnv";
...
set<uint> QryPlnrCtpAEnv::StatShr::comm(
			const StatShr* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpAEnv::StatShr::diff(
			const StatShr* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
/******************************************************************************
 class QryPlnrCtpAEnv::StgIac
 ******************************************************************************/

QryPlnrCtpAEnv::StgIac::StgIac(
			const uint jnum
			, const uint jnumFirstload
			, const uint nload
		) : Block() {
...
bool QryPlnrCtpAEnv::StgIac::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgIacQryPlnrCtpAEnv");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemIacQryPlnrCtpAEnv";
...
void QryPlnrCtpAEnv::StgIac::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgIacQryPlnrCtpAEnv";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemIacQryPlnrCtpAEnv";
...
set<uint> QryPlnrCtpAEnv::StgIac::comm(
			const StgIac* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrCtpAEnv::StgIac::diff(
			const StgIac* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;

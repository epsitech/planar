/**
  * \file PnlPlnrStkMNStructure.cpp
  * app access code for job PnlPlnrStkMNStructure (implementation)
...
bool PnlPlnrStkMNStructure::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfPlnrStkMNStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfPlnrStkMNStructure";
...
PnlPlnrStkMNStructure::StatApp::StatApp(
...
bool PnlPlnrStkMNStructure::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppPlnrStkMNStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppPlnrStkMNStructure";
...
set<uint> PnlPlnrStkMNStructure::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrStkMNStructure::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrStkMNStructure::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrPlnrStkMNStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrPlnrStkMNStructure";
...
PnlPlnrStkMNStructure::Tag::Tag(
	...
	mask = {CPT, TRS, TXTSHOWING1, TXTSHOWING2, TCOMREF, TCOFRL, TCOTOL};
};

bool PnlPlnrStkMNStructure::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrStkMNStructure");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrStkMNStructure";
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoFrl", TcoFrl)) add(TCOFRL);
...
PnlPlnrStkMNStructure::DpchAppData::DpchAppData(
			const string& scrJref
			, StgIac* stgiac
			, QryPlnrStkMNStructure::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSTKMNSTRUCTUREDATA, scrJref) {
...
void PnlPlnrStkMNStructure::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrStkMNStructureData");
...
PnlPlnrStkMNStructure::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSTKMNSTRUCTUREDO, scrJref) {
...
void PnlPlnrStkMNStructure::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppPlnrStkMNStructureDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlPlnrStkMNStructure::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRSTKMNSTRUCTUREDATA) {
	feedFCsiQst.tag = "FeedFCsiQst";
};

void PnlPlnrStkMNStructure::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrStkMNStructureData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryPlnrStkMNStructure::StatApp();
		statshrqry = QryPlnrStkMNStructure::StatShr();

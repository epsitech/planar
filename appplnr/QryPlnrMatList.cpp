/**
  ...
  * app access code for job QryPlnrMatList (implementation)
...
uint QryPlnrMatList::VecVOrd::getIx(
	...
	else if (s.compare("tit") == 0) return TIT;
...
QryPlnrMatList::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrMatList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrMatList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrMatList";
...
set<uint> QryPlnrMatList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrMatList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrMatList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrMatList");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrMatList";

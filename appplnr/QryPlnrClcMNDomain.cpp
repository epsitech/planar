/**
  ...
  * app access code for job QryPlnrClcMNDomain (implementation)
...
QryPlnrClcMNDomain::StatApp::StatApp(
			const uint firstcol
...
bool QryPlnrClcMNDomain::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryPlnrClcMNDomain");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppQryPlnrClcMNDomain";
...
set<uint> QryPlnrClcMNDomain::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryPlnrClcMNDomain::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryPlnrClcMNDomain::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryPlnrClcMNDomain");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrQryPlnrClcMNDomain";

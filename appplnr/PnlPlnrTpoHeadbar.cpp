/**
  * \file PnlPlnrTpoHeadbar.cpp
  * app access code for job PnlPlnrTpoHeadbar (implementation)
...
PnlPlnrTpoHeadbar::StgInf::StgInf(
...
bool PnlPlnrTpoHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfPlnrTpoHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfPlnrTpoHeadbar";
...
set<uint> PnlPlnrTpoHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlPlnrTpoHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlPlnrTpoHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagPlnrTpoHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemPlnrTpoHeadbar";
...
PnlPlnrTpoHeadbar::DpchEngData::DpchEngData() : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRTPOHEADBARDATA) {
};

void PnlPlnrTpoHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngPlnrTpoHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);

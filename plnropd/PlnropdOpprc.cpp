/**
  * \file PlnropdOpprc.cpp
  * operation processor for Plnr operation daemon plnropd (implementation)
...
void* PlnropdOpprc::run(
			void* arg
		) {
	XchgPlnropd* xchg = (XchgPlnropd*) arg;
	...
	while (true) {
		...
		while (req == NULL) {
			...
			Mutex::lock(&(xchg->mcOpprcs), "xchg->mcOpprcs", "PlnropdOpprc", "run");
			Cond::wait(&(xchg->cOpprcs), &(xchg->mcOpprcs), "xchg->cOpprcs", "PlnropdOpprc", "run");

			// note that the result here might be NULL, if another op processor strikes first
			req = xchg->pullFirstReq();

			Mutex::unlock(&(xchg->mcOpprcs), "xchg->mcOpprcs", "PlnropdOpprc", "run");
		};
		...
		if (req) {
			...
			Mutex::lock(&(req->mcReady), "req->mcReady", "PlnropdOpprc", "run");
			req->ixVState = ReqopPlnr::VecVState::REPLY;
			Cond::broadcast(&(req->cReady), "req->cReady", "PlnropdOpprc", "run");
			Mutex::unlock(&(req->mcReady), "req->mcReady", "PlnropdOpprc", "run");
		};
	};
...
void PlnropdOpprc::cleanup(
			void* arg
		) {
	XchgPlnropd* xchg = (XchgPlnropd*) arg;

	Mutex::unlock(&(xchg->mcOpprcs), "xchg->mcOpprcs", "PlnropdOpprc", "cleanup");
};

/**
  * \file PnlPlnrPrj1NCalc.cpp
  * job handler for job PnlPlnrPrj1NCalc (implementation)
...
#include "PnlPlnrPrj1NCalc_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrPrj1NCalc_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrPrj1NCalc
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrPrj1NCalc::PnlPlnrPrj1NCalc(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRPRJ1NCALC, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrPrj1NCalc(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrPrj1NCalc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrPrj1NCalc::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrPrj1NCalc::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRPRJ1NCALCDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRPRJ1NCALCDO) {
...
void PnlPlnrPrj1NCalc::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrPrj1NCalc::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrPrj1NCalc::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrPrj1NCalc::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrPrj1NCalc::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrPrj1NCalc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrPrj1NCalc::StgIac::JNUM)) {
...
void PnlPlnrPrj1NCalc::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrPrj1NCalc::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrPrj1NCalc::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrPrj1NCalc::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

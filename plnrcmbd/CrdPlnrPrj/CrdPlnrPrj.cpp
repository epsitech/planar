/**
  * \file CrdPlnrPrj.cpp
  * job handler for job CrdPlnrPrj (implementation)
...
CrdPlnrPrj::CrdPlnrPrj(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
			, const ubigint ref
		) : JobPlnr(xchg, VecPlnrVJob::CRDPLNRPRJ, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlPlnrPrjRec(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnlheadbar = new PnlPlnrPrjHeadbar(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnllist = new PnlPlnrPrjList(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* CrdPlnrPrj::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdPlnrPrj/" + VecPlnrVLocale::getSref(ixPlnrVLocale);
	continf.MtxCrdPrj = StubPlnr::getStubPrjStd(dbsplnr, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFPRJ, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdPlnrPrj::changeRef(
			DbsPlnr* dbsplnr
	...
	uint ixPlnrVAccess;
	xchg->triggerIxRefToIxCall(dbsplnr, VecPlnrVCall::CALLPLNRACCESS, jref, VecPlnrVMaintable::TBLPLNRMPROJECT, ref, ixPlnrVAccess);

	if (ixPlnrVAccess != VecPlnrVAccess::NONE) {
		xchg->addIxPreset(VecPlnrVPreset::PREPLNRIXACC, jref, ixPlnrVAccess);

		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsplnr, VecPlnrVCall::CALLPLNRLOG, jref, VecPlnrVPreset::VOID, 0, "CrdPlnrPrj", ref);
		xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFPRJ, jref, ref);

		refresh(dbsplnr, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFPRJ, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFPRJ, jrefTrig, notif);
	};
};

void CrdPlnrPrj::updatePreset(
			DbsPlnr* dbsplnr
...
void CrdPlnrPrj::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdPlnrPrj::enterSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
...
void CrdPlnrPrj::leaveSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeAlrplnrabt --- INSERT
};

uint CrdPlnrPrj::enterSgeClose(
			DbsPlnr* dbsplnr
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDCLOSE, jref, VecPlnrVCard::CRDPLNRPRJ);
	return retval;
};

void CrdPlnrPrj::leaveSgeClose(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeClose --- INSERT
};

string CrdPlnrPrj::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdPlnrPrj::handleRequest(
	...
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRPRJDO) {
...
void CrdPlnrPrj::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void CrdPlnrPrj::handleDpchAppDoClose(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	*dpcheng = new DpchEngPlnrConfirm(true, jref, "");
...
void CrdPlnrPrj::handleDpchAppDoMitAppAbtClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void CrdPlnrPrj::handleDpchAppPlnrAlert(
			DbsPlnr* dbsplnr
...
bool CrdPlnrPrj::handleCallPlnrDlgClose(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrPrj::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrPrj::handleCallPlnrRefPreSet(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecPlnrVPreset::PREPLNRREFPRJ) {

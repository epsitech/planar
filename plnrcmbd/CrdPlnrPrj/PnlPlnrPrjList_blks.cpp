/**
  * \file PnlPlnrPrjList_blks.cpp
  * job handler for job PnlPlnrPrjList (implementation of blocks)
...
void PnlPlnrPrjList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrPrjList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrPrjList";
...
void PnlPlnrPrjList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrPrjList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrPrjList";
...
void PnlPlnrPrjList::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrPrjList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrPrjList";
...
PnlPlnrPrjList::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRPRJLISTDATA) {
};

void PnlPlnrPrjList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrPrjListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrPrjList::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRPRJLISTDO) {
	ixVDo = 0;
};

void PnlPlnrPrjList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrPrjListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrPrjList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryPlnrPrjList::StatShr* statshrqry
			, QryPlnrPrjList::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRPRJLISTDATA, jref) {
...
void PnlPlnrPrjList::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrPrjList::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrPrjListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrPrjList::StatApp::writeXML(wr);

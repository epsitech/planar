/**
  * \file QryPlnrPrjList_blks.cpp
  * job handler for job QryPlnrPrjList (implementation of blocks)
...
void QryPlnrPrjList::VecVOrd::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void QryPlnrPrjList::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint firstcol
	...
	if (difftag.length() == 0) difftag = "StatAppQryPlnrPrjList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppQryPlnrPrjList";
...
void QryPlnrPrjList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrQryPlnrPrjList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrQryPlnrPrjList";

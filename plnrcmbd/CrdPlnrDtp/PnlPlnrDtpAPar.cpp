/**
  * \file PnlPlnrDtpAPar.cpp
  * job handler for job PnlPlnrDtpAPar (implementation)
...
#include "PnlPlnrDtpAPar_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDtpAPar_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDtpAPar
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDtpAPar::PnlPlnrDtpAPar(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDTPAPAR, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDtpAPar(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDtpAPar::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDtpAPar::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpAPar::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPAPARDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPAPARDO) {
...
void PnlPlnrDtpAPar::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpAPar::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpAPar::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDtpAPar::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDtpAPar::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDtpAPar::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDtpAPar::StgIac::JNUM)) {
...
void PnlPlnrDtpAPar::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpAPar::handleDpchAppDoButDuplicateClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpAPar::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpAPar::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDtpAPar::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDtpAPar::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

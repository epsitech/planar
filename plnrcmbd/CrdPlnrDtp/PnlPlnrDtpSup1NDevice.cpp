/**
  ...
  * job handler for job PnlPlnrDtpSup1NDevice (implementation)
...
#include "PnlPlnrDtpSup1NDevice_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDtpSup1NDevice_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDtpSup1NDevice
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDtpSup1NDevice::PnlPlnrDtpSup1NDevice(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDTPSUP1NDEVICE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDtpSup1NDevice(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDtpSup1NDevice::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDtpSup1NDevice::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDtpSup1NDevice::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpSup1NDevice::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPSUP1NDEVICEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPSUP1NDEVICEDO) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDtpSup1NDevice::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDtpSup1NDevice::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDtpSup1NDevice::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDtpSup1NDevice::StgIac::JNUM)) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDoButUpClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDoButDownClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpSup1NDevice::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDtpSup1NDevice::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDtpSup1NDevice::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

/**
  * \file PnlPlnrDtpDetail_blks.cpp
  * job handler for job PnlPlnrDtpDetail (implementation of blocks)
...
void PnlPlnrDtpDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDtpDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDtpDetail";
...
void PnlPlnrDtpDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppPlnrDtpDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrDtpDetail";
...
void PnlPlnrDtpDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrDtpDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDtpDetail";
...
void PnlPlnrDtpDetail::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrDtpDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrDtpDetail";
...
PnlPlnrDtpDetail::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDTPDETAILDATA) {
};

void PnlPlnrDtpDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDtpDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDtpDetail::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDTPDETAILDO) {
	ixVDo = 0;
};

void PnlPlnrDtpDetail::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDtpDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDtpDetail::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFPupAty
			...
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDTPDETAILDATA, jref) {
...
void PnlPlnrDtpDetail::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrDtpDetail::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDtpDetailData");

/**
  * \file PnlPlnrDtpTpl1NDevice.cpp
  * job handler for job PnlPlnrDtpTpl1NDevice (implementation)
...
#include "PnlPlnrDtpTpl1NDevice_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDtpTpl1NDevice_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDtpTpl1NDevice
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDtpTpl1NDevice::PnlPlnrDtpTpl1NDevice(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDTPTPL1NDEVICE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDtpTpl1NDevice(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDtpTpl1NDevice::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDtpTpl1NDevice::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDtpTpl1NDevice::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpTpl1NDevice::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPTPL1NDEVICEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDTPTPL1NDEVICEDO) {
...
void PnlPlnrDtpTpl1NDevice::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDtpTpl1NDevice::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpTpl1NDevice::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDtpTpl1NDevice::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDtpTpl1NDevice::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDtpTpl1NDevice::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDtpTpl1NDevice::StgIac::JNUM)) {
...
void PnlPlnrDtpTpl1NDevice::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDtpTpl1NDevice::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDtpTpl1NDevice::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDtpTpl1NDevice::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

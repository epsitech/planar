/**
  * \file CrdPlnrDtp_blks.cpp
  * job handler for job CrdPlnrDtp (implementation of blocks)
...
void CrdPlnrDtp::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
void CrdPlnrDtp::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDtp";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDtp";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		writeUintAttr(wr, itemtag, "sref", "numFSge", numFSge);
		writeStringAttr(wr, itemtag, "sref", "MrlAppHlp", MrlAppHlp);
		writeStringAttr(wr, itemtag, "sref", "MtxCrdDtp", MtxCrdDtp);
...
set<uint> CrdPlnrDtp::ContInf::comm(
	...
	if (MtxCrdDtp == comp->MtxCrdDtp) insert(items, MTXCRDDTP);
...
void CrdPlnrDtp::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppPlnrDtp";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrDtp";
...
void CrdPlnrDtp::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrPlnrDtp";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDtp";
...
CrdPlnrDtp::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDTPDO) {
	ixVDo = 0;
};

void CrdPlnrDtp::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDtpDo");
	else
...
CrdPlnrDtp::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFSge
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDTPDATA, jref) {
...
void CrdPlnrDtp::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void CrdPlnrDtp::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDtpData");

/**
  * \file PnlPlnrLyrMNStructure.cpp
  * job handler for job PnlPlnrLyrMNStructure (implementation)
...
#include "PnlPlnrLyrMNStructure_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrLyrMNStructure_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrLyrMNStructure
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrLyrMNStructure::PnlPlnrLyrMNStructure(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRLYRMNSTRUCTURE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrLyrMNStructure(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrLyrMNStructure::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrLyrMNStructure::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrLyrMNStructure::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrLyrMNStructure::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRLYRMNSTRUCTUREDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRLYRMNSTRUCTUREDO) {
...
void PnlPlnrLyrMNStructure::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrLyrMNStructure::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrMNStructure::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrLyrMNStructure::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrLyrMNStructure::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrLyrMNStructure::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrLyrMNStructure::StgIac::JNUM)) {
...
void PnlPlnrLyrMNStructure::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQLyrMNStructure WHERE ref = " + to_string(refSel), refSel)) {
...
void PnlPlnrLyrMNStructure::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrMNStructure::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrMNStructure::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrLyrMNStructure::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrLyrMNStructure::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

/**
  * \file CrdPlnrLyr.cpp
  * job handler for job CrdPlnrLyr (implementation)
...
// IP cust --- INSERT
DpchEngPlnr* CrdPlnrLyr::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdPlnrLyr/" + VecPlnrVLocale::getSref(ixPlnrVLocale);
	continf.MtxCrdLyr = StubPlnr::getStubLyrStd(dbsplnr, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFLYR, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdPlnrLyr::changeRef(
			DbsPlnr* dbsplnr
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsplnr, VecPlnrVCall::CALLPLNRLOG, jref, ixPlnrVPreset, xchg->getRefPreset(ixPlnrVPreset, jref), "CrdPlnrLyr", ref);
	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFLYR, jref, ref);

	refresh(dbsplnr, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));

	if (pnllist) pnllist->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFLYR, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFLYR, jrefTrig, notif);
};

void CrdPlnrLyr::updatePreset(
			DbsPlnr* dbsplnr
			, const uint ixPlnrVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdPlnrLyr::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdPlnrLyr::enterSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
...
void CrdPlnrLyr::leaveSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeAlrplnrabt --- INSERT
};

uint CrdPlnrLyr::enterSgeClose(
			DbsPlnr* dbsplnr
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDCLOSE, jref, VecPlnrVCard::CRDPLNRLYR);
	return retval;
};

void CrdPlnrLyr::leaveSgeClose(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeClose --- INSERT
};

string CrdPlnrLyr::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdPlnrLyr::handleRequest(
	...
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRLYRDO) {
...
void CrdPlnrLyr::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void CrdPlnrLyr::handleDpchAppDoClose(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	*dpcheng = new DpchEngPlnrConfirm(true, jref, "");
...
void CrdPlnrLyr::handleDpchAppDoMitAppAbtClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void CrdPlnrLyr::handleDpchAppPlnrAlert(
			DbsPlnr* dbsplnr
...
bool CrdPlnrLyr::handleCallPlnrDlgClose(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrLyr::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrLyr::handleCallPlnrRefPreSet(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecPlnrVPreset::PREPLNRREFLYR) {

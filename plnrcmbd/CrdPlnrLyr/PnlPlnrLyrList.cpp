/**
  ...
  * job handler for job PnlPlnrLyrList (implementation)
...
#include "PnlPlnrLyrList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrLyrList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrLyrList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrLyrList::PnlPlnrLyrList(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRLYRLIST, jrefSup, ixPlnrVLocale) {
...
// IP constructor.addJob --- END
	...
	QryPlnrLyrList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFSEL, jref, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFLYR, jref));
	xchg->addIxPreset(VecPlnrVPreset::PREPLNRIXORD, jref, QryPlnrLyrList::VecVOrd::SRF);

	muteRefresh = false;

	qry = new QryPlnrLyrList(xchg, dbsplnr, jref, ixPlnrVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixPlnrVExpstate = ((xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFLYR, jref)) ? VecPlnrVExpstate::MIND : VecPlnrVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrLyrList::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	...
	if ((ixPre != 0) && (ixPre != VecPlnrVPreset::VOID)) {
		...
		if (ixPre == VecPlnrVPreset::PREPLNRREFSTK) continf.TxtPre = StubPlnr::getStubStkStd(dbsplnr, xchg->getRefPreset(ixPre, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);
...
void PnlPlnrLyrList::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrLyrList::regularize(
			DbsPlnr* dbsplnr
			, const bool notif
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRLYRLISTDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRLYRLISTDO) {
...
void PnlPlnrLyrList::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrLyrList::handleDpchAppDataContiac(
			DbsPlnr* dbsplnr
			, ContIac* _contiac
			, DpchEngPlnr** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryPlnrLyrList::VecVOrd::SRF) && (_contiac->numFTos <= QryPlnrLyrList::VecVOrd::TTY)) {
...
void PnlPlnrLyrList::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrLyrList::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrLyrList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrLyrList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			refresh(dbsplnr, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrLyrList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsplnr, VecPlnrVCall::CALLPLNRREFPRESET, jref, VecPlnrVPreset::PREPLNRREFLYR, refSelNew);
		};
	};
...
void PnlPlnrLyrList::handleDpchAppDoButMinimizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDoButRegularizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDoButFilterClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrLyrList::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrLyrList::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrLyrList::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

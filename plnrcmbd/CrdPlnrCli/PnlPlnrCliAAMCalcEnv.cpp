/**
  ...
  * job handler for job PnlPlnrCliAAMCalcEnv (implementation)
...
#include "PnlPlnrCliAAMCalcEnv_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrCliAAMCalcEnv_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrCliAAMCalcEnv
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrCliAAMCalcEnv::PnlPlnrCliAAMCalcEnv(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRCLIAAMCALCENV, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrCliAAMCalcEnv(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrCliAAMCalcEnv::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrCliAAMCalcEnv::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrCliAAMCalcEnv::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrCliAAMCalcEnv::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLIAAMCALCENVDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLIAAMCALCENVDO) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrCliAAMCalcEnv::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrCliAAMCalcEnv::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrCliAAMCalcEnv::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrCliAAMCalcEnv::StgIac::JNUM)) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDoButDuplicateClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCliAAMCalcEnv::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrCliAAMCalcEnv::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrCliAAMCalcEnv::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

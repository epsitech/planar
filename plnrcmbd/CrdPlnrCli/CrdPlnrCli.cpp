/**
  * \file CrdPlnrCli.cpp
  * job handler for job CrdPlnrCli (implementation)
...
CrdPlnrCli::CrdPlnrCli(
		...
		) : JobPlnr(xchg, VecPlnrVJob::CRDPLNRCLI, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlPlnrCliRec(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnlheadbar = new PnlPlnrCliHeadbar(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnllist = new PnlPlnrCliList(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* CrdPlnrCli::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdPlnrCli/" + VecPlnrVLocale::getSref(ixPlnrVLocale);
	continf.MtxCrdCli = StubPlnr::getStubCliStd(dbsplnr, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFCLI, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdPlnrCli::changeRef(
			DbsPlnr* dbsplnr
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsplnr, VecPlnrVCall::CALLPLNRLOG, jref, ixPlnrVPreset, xchg->getRefPreset(ixPlnrVPreset, jref), "CrdPlnrCli", ref);
	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFCLI, jref, ref);

	refresh(dbsplnr, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));

	if (pnllist) pnllist->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFCLI, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFCLI, jrefTrig, notif);
};

void CrdPlnrCli::updatePreset(
			DbsPlnr* dbsplnr
			, const uint ixPlnrVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdPlnrCli::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdPlnrCli::enterSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
...
void CrdPlnrCli::leaveSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeAlrplnrabt --- INSERT
};

uint CrdPlnrCli::enterSgeClose(
			DbsPlnr* dbsplnr
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDCLOSE, jref, VecPlnrVCard::CRDPLNRCLI);
	return retval;
};

void CrdPlnrCli::leaveSgeClose(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeClose --- INSERT
};

string CrdPlnrCli::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdPlnrCli::handleRequest(
	...
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLIDO) {
...
void CrdPlnrCli::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void CrdPlnrCli::handleDpchAppDoClose(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	*dpcheng = new DpchEngPlnrConfirm(true, jref, "");
...
void CrdPlnrCli::handleDpchAppDoMitAppAbtClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void CrdPlnrCli::handleDpchAppPlnrAlert(
			DbsPlnr* dbsplnr
...
bool CrdPlnrCli::handleCallPlnrDlgClose(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrCli::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrCli::handleCallPlnrRefPreSet(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecPlnrVPreset::PREPLNRREFCLI) {

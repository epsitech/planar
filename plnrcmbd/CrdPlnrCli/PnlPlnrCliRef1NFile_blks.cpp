/**
  * \file PnlPlnrCliRef1NFile_blks.cpp
  * job handler for job PnlPlnrCliRef1NFile (implementation of blocks)
...
void PnlPlnrCliRef1NFile::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrCliRef1NFile";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrCliRef1NFile";
...
void PnlPlnrCliRef1NFile::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrCliRef1NFile";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrCliRef1NFile";
...
void PnlPlnrCliRef1NFile::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrCliRef1NFile";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrCliRef1NFile";
...
void PnlPlnrCliRef1NFile::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrCliRef1NFile";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrCliRef1NFile";
...
PnlPlnrCliRef1NFile::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRCLIREF1NFILEDATA) {
};

void PnlPlnrCliRef1NFile::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrCliRef1NFileData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrCliRef1NFile::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRCLIREF1NFILEDO) {
	ixVDo = 0;
};

void PnlPlnrCliRef1NFile::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrCliRef1NFileDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrCliRef1NFile::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRCLIREF1NFILEDATA, jref) {
...
void PnlPlnrCliRef1NFile::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrCliRef1NFile::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrCliRef1NFileData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrCliRef1NFile::StatApp::writeXML(wr);

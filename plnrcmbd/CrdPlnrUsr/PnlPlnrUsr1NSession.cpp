/**
  * \file PnlPlnrUsr1NSession.cpp
  * job handler for job PnlPlnrUsr1NSession (implementation)
...
#include "PnlPlnrUsr1NSession_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrUsr1NSession_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrUsr1NSession
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrUsr1NSession::PnlPlnrUsr1NSession(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRUSR1NSESSION, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrUsr1NSession(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrUsr1NSession::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrUsr1NSession::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrUsr1NSession::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrUsr1NSession::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRUSR1NSESSIONDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRUSR1NSESSIONDO) {
...
void PnlPlnrUsr1NSession::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrUsr1NSession::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrUsr1NSession::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrUsr1NSession::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrUsr1NSession::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrUsr1NSession::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrUsr1NSession::StgIac::JNUM)) {
...
void PnlPlnrUsr1NSession::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrUsr1NSession::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrUsr1NSession::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrUsr1NSession::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

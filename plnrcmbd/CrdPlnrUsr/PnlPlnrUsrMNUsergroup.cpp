/**
  * \file PnlPlnrUsrMNUsergroup.cpp
  * job handler for job PnlPlnrUsrMNUsergroup (implementation)
...
#include "PnlPlnrUsrMNUsergroup_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrUsrMNUsergroup_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrUsrMNUsergroup
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrUsrMNUsergroup::PnlPlnrUsrMNUsergroup(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRUSRMNUSERGROUP, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrUsrMNUsergroup(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrUsrMNUsergroup::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrUsrMNUsergroup::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrUsrMNUsergroup::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrUsrMNUsergroup::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRUSRMNUSERGROUPDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRUSRMNUSERGROUPDO) {
...
void PnlPlnrUsrMNUsergroup::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrUsrMNUsergroup::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrUsrMNUsergroup::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrUsrMNUsergroup::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrUsrMNUsergroup::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrUsrMNUsergroup::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrUsrMNUsergroup::StgIac::JNUM)) {
...
void PnlPlnrUsrMNUsergroup::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQUsrMNUsergroup WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, 0, 0, "CrdPlnrUsg", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngPlnrConfirm(false, 0, "");
	else *dpcheng = new DpchEngPlnrConfirm(true, jrefNew, "CrdPlnrUsg");
};

void PnlPlnrUsrMNUsergroup::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrUsrMNUsergroup::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrUsrMNUsergroup::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrUsrMNUsergroup::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrUsrMNUsergroup::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

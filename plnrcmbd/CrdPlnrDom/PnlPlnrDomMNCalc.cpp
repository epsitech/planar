/**
  ...
  * job handler for job PnlPlnrDomMNCalc (implementation)
...
#include "PnlPlnrDomMNCalc_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDomMNCalc_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDomMNCalc
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDomMNCalc::PnlPlnrDomMNCalc(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDOMMNCALC, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDomMNCalc(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDomMNCalc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDomMNCalc::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDomMNCalc::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDOMMNCALCDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDOMMNCALCDO) {
...
void PnlPlnrDomMNCalc::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDomMNCalc::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDomMNCalc::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDomMNCalc::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDomMNCalc::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDomMNCalc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDomMNCalc::StgIac::JNUM)) {
...
void PnlPlnrDomMNCalc::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDomMNCalc::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDomMNCalc::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDomMNCalc::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDomMNCalc::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

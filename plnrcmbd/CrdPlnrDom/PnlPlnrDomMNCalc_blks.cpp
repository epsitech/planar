/**
  * \file PnlPlnrDomMNCalc_blks.cpp
  * job handler for job PnlPlnrDomMNCalc (implementation of blocks)
...
void PnlPlnrDomMNCalc::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDomMNCalc";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDomMNCalc";
...
void PnlPlnrDomMNCalc::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrDomMNCalc";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrDomMNCalc";
...
void PnlPlnrDomMNCalc::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrDomMNCalc";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDomMNCalc";
...
void PnlPlnrDomMNCalc::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrDomMNCalc";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrDomMNCalc";
...
PnlPlnrDomMNCalc::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDOMMNCALCDATA) {
};

void PnlPlnrDomMNCalc::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDomMNCalcData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDomMNCalc::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDOMMNCALCDO) {
	ixVDo = 0;
};

void PnlPlnrDomMNCalc::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDomMNCalcDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDomMNCalc::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDOMMNCALCDATA, jref) {
...
void PnlPlnrDomMNCalc::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrDomMNCalc::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDomMNCalcData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrDomMNCalc::StatApp::writeXML(wr);

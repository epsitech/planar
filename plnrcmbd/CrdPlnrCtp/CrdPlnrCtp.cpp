/**
  * \file CrdPlnrCtp.cpp
  * job handler for job CrdPlnrCtp (implementation)
...
CrdPlnrCtp::CrdPlnrCtp(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
			, const ubigint ref
		) : JobPlnr(xchg, VecPlnrVJob::CRDPLNRCTP, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlPlnrCtpRec(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnlheadbar = new PnlPlnrCtpHeadbar(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnllist = new PnlPlnrCtpList(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* CrdPlnrCtp::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdPlnrCtp/" + VecPlnrVLocale::getSref(ixPlnrVLocale);
	continf.MtxCrdCtp = StubPlnr::getStubCtpStd(dbsplnr, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFCTP, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdPlnrCtp::changeRef(
			DbsPlnr* dbsplnr
	...
	if (ixPlnrVAccess != VecPlnrVAccess::NONE) {
		...
		if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsplnr, VecPlnrVCall::CALLPLNRLOG, jref, VecPlnrVPreset::VOID, 0, "CrdPlnrCtp", ref);
		xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFCTP, jref, ref);

		refresh(dbsplnr, moditems);
		if (notif) xchg->submitDpch(getNewDpchEng(moditems));

		if (pnllist) pnllist->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFCTP, jrefTrig, notif);
		if (pnlrec) pnlrec->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFCTP, jrefTrig, notif);
	};
};

void CrdPlnrCtp::updatePreset(
			DbsPlnr* dbsplnr
...
void CrdPlnrCtp::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdPlnrCtp::enterSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
...
void CrdPlnrCtp::leaveSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeAlrplnrabt --- INSERT
};

uint CrdPlnrCtp::enterSgeClose(
			DbsPlnr* dbsplnr
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDCLOSE, jref, VecPlnrVCard::CRDPLNRCTP);
	return retval;
};

void CrdPlnrCtp::leaveSgeClose(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeClose --- INSERT
};

string CrdPlnrCtp::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdPlnrCtp::handleRequest(
	...
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCTPDO) {
...
void CrdPlnrCtp::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
bool CrdPlnrCtp::handleCallPlnrDlgClose(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrCtp::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrCtp::handleCallPlnrRefPreSet(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecPlnrVPreset::PREPLNRREFCTP) {

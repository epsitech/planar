/**
  ...
  * job handler for job PnlPlnrCtpKCalcdomprp (implementation)
...
#include "PnlPlnrCtpKCalcdomprp_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrCtpKCalcdomprp_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrCtpKCalcdomprp
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrCtpKCalcdomprp::PnlPlnrCtpKCalcdomprp(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRCTPKCALCDOMPRP, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrCtpKCalcdomprp(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrCtpKCalcdomprp::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrCtpKCalcdomprp::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrCtpKCalcdomprp::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrCtpKCalcdomprp::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCTPKCALCDOMPRPDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCTPKCALCDOMPRPDO) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButUpClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButDownClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButDuplicateClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpKCalcdomprp::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrCtpKCalcdomprp::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrCtpKCalcdomprp::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

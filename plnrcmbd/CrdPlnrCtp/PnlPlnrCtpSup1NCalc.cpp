/**
  * \file PnlPlnrCtpSup1NCalc.cpp
  * job handler for job PnlPlnrCtpSup1NCalc (implementation)
...
#include "PnlPlnrCtpSup1NCalc_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrCtpSup1NCalc_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrCtpSup1NCalc
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrCtpSup1NCalc::PnlPlnrCtpSup1NCalc(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRCTPSUP1NCALC, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrCtpSup1NCalc(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrCtpSup1NCalc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrCtpSup1NCalc::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrCtpSup1NCalc::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrCtpSup1NCalc::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCTPSUP1NCALCDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCTPSUP1NCALCDO) {
...
void PnlPlnrCtpSup1NCalc::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrCtpSup1NCalc::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpSup1NCalc::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrCtpSup1NCalc::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrCtpSup1NCalc::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrCtpSup1NCalc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrCtpSup1NCalc::StgIac::JNUM)) {
...
void PnlPlnrCtpSup1NCalc::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrCtpSup1NCalc::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrCtpSup1NCalc::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrCtpSup1NCalc::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

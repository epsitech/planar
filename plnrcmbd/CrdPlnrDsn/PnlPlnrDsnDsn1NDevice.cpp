/**
  * \file PnlPlnrDsnDsn1NDevice.cpp
  * job handler for job PnlPlnrDsnDsn1NDevice (implementation)
...
#include "PnlPlnrDsnDsn1NDevice_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDsnDsn1NDevice_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDsnDsn1NDevice
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDsnDsn1NDevice::PnlPlnrDsnDsn1NDevice(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDSNDSN1NDEVICE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDsnDsn1NDevice(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDsnDsn1NDevice::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDsnDsn1NDevice::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDsnDsn1NDevice::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnDsn1NDevice::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDEVICEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDEVICEDO) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDsnDsn1NDevice::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDsnDsn1NDevice::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDsnDsn1NDevice::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDsnDsn1NDevice::StgIac::JNUM)) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDoButUpClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDoButDownClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDevice::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDsnDsn1NDevice::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDsnDsn1NDevice::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

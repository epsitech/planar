/**
  * \file PnlPlnrDsnHk1NArcguide_blks.cpp
  * job handler for job PnlPlnrDsnHk1NArcguide (implementation of blocks)
...
void PnlPlnrDsnHk1NArcguide::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDsnHk1NArcguide";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDsnHk1NArcguide";
...
void PnlPlnrDsnHk1NArcguide::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrDsnHk1NArcguide";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrDsnHk1NArcguide";
...
void PnlPlnrDsnHk1NArcguide::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrDsnHk1NArcguide";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDsnHk1NArcguide";
...
void PnlPlnrDsnHk1NArcguide::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrDsnHk1NArcguide";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrDsnHk1NArcguide";
...
PnlPlnrDsnHk1NArcguide::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDSNHK1NARCGUIDEDATA) {
};

void PnlPlnrDsnHk1NArcguide::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDsnHk1NArcguideData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDsnHk1NArcguide::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDSNHK1NARCGUIDEDO) {
	ixVDo = 0;
};

void PnlPlnrDsnHk1NArcguide::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDsnHk1NArcguideDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDsnHk1NArcguide::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDSNHK1NARCGUIDEDATA, jref) {
...
void PnlPlnrDsnHk1NArcguide::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrDsnHk1NArcguide::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDsnHk1NArcguideData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrDsnHk1NArcguide::StatApp::writeXML(wr);

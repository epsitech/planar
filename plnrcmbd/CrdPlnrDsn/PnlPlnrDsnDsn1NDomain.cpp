/**
  ...
  * job handler for job PnlPlnrDsnDsn1NDomain (implementation)
...
#include "PnlPlnrDsnDsn1NDomain_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDsnDsn1NDomain_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDsnDsn1NDomain
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDsnDsn1NDomain::PnlPlnrDsnDsn1NDomain(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDSNDSN1NDOMAIN, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDsnDsn1NDomain(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDsnDsn1NDomain::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDsnDsn1NDomain::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDsnDsn1NDomain::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnDsn1NDomain::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDOMAINDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDOMAINDO) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDsnDsn1NDomain::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDsnDsn1NDomain::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDsnDsn1NDomain::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDsnDsn1NDomain::StgIac::JNUM)) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButUpClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButDownClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (refSel != 0)  xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, VecPlnrVPreset::PREPLNRREFDSN, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFDSN, jref), "CrdPlnrDom", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngPlnrConfirm(false, 0, "");
	else *dpcheng = new DpchEngPlnrConfirm(true, jrefNew, "CrdPlnrDom");
};

void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint jrefNew = 0;

	xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, VecPlnrVPreset::PREPLNRREFDSN, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFDSN, jref), "CrdPlnrDom", -1, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngPlnrConfirm(false, 0, "");
	else *dpcheng = new DpchEngPlnrConfirm(true, jrefNew, "CrdPlnrDom");
};

void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnDsn1NDomain::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDsnDsn1NDomain::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDsnDsn1NDomain::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

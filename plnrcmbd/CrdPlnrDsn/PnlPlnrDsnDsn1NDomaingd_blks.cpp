/**
  * \file PnlPlnrDsnDsn1NDomaingd_blks.cpp
  * job handler for job PnlPlnrDsnDsn1NDomaingd (implementation of blocks)
...
void PnlPlnrDsnDsn1NDomaingd::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDsnDsn1NDomaingd";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDsnDsn1NDomaingd";
...
void PnlPlnrDsnDsn1NDomaingd::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrDsnDsn1NDomaingd";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrDsnDsn1NDomaingd";
...
void PnlPlnrDsnDsn1NDomaingd::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrDsnDsn1NDomaingd";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDsnDsn1NDomaingd";
...
void PnlPlnrDsnDsn1NDomaingd::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrDsnDsn1NDomaingd";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrDsnDsn1NDomaingd";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Domain guides");
			writeStringAttr(wr, itemtag, "sref", "TcoRef", "Domain guide");
		};
...
PnlPlnrDsnDsn1NDomaingd::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDOMAINGDDATA) {
};

void PnlPlnrDsnDsn1NDomaingd::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDsnDsn1NDomaingdData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDsnDsn1NDomaingd::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDSNDSN1NDOMAINGDDO) {
	ixVDo = 0;
};

void PnlPlnrDsnDsn1NDomaingd::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDsnDsn1NDomaingdDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDsnDsn1NDomaingd::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDSNDSN1NDOMAINGDDATA, jref) {
...
void PnlPlnrDsnDsn1NDomaingd::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrDsnDsn1NDomaingd::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDsnDsn1NDomaingdData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrDsnDsn1NDomaingd::StatApp::writeXML(wr);

/**
  * \file PnlPlnrDsnMod1NDesign.cpp
  * job handler for job PnlPlnrDsnMod1NDesign (implementation)
...
#include "PnlPlnrDsnMod1NDesign_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDsnMod1NDesign_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDsnMod1NDesign
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDsnMod1NDesign::PnlPlnrDsnMod1NDesign(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDSNMOD1NDESIGN, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDsnMod1NDesign(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDsnMod1NDesign::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDsnMod1NDesign::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDsnMod1NDesign::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnMod1NDesign::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNMOD1NDESIGNDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDSNMOD1NDESIGNDO) {
...
void PnlPlnrDsnMod1NDesign::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDsnMod1NDesign::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDsnMod1NDesign::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDsnMod1NDesign::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDsnMod1NDesign::StgIac::JNUM)) {
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	uint ixPre = xchg->getIxPreset(VecPlnrVPreset::PREPLNRIXPRE, jref);
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDsnMod1NDesign::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDsnMod1NDesign::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDsnMod1NDesign::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

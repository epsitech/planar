/**
  ...
  * job handler for job PnlPlnrStkMNStructure (implementation)
...
#include "PnlPlnrStkMNStructure_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrStkMNStructure_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrStkMNStructure
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrStkMNStructure::PnlPlnrStkMNStructure(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRSTKMNSTRUCTURE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrStkMNStructure(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrStkMNStructure::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrStkMNStructure::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrStkMNStructure::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrStkMNStructure::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSTKMNSTRUCTUREDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSTKMNSTRUCTUREDO) {
...
void PnlPlnrStkMNStructure::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrStkMNStructure::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrStkMNStructure::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrStkMNStructure::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrStkMNStructure::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrStkMNStructure::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrStkMNStructure::StgIac::JNUM)) {
...
void PnlPlnrStkMNStructure::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQStkMNStructure WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecPlnrVPreset::PREPLNRREFDSN) xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdPlnrSru", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, 0, 0, "CrdPlnrSru", refSel, jrefNew);
	};
...
void PnlPlnrStkMNStructure::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrStkMNStructure::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrStkMNStructure::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrStkMNStructure::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrStkMNStructure::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

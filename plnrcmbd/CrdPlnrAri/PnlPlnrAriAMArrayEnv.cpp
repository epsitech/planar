/**
  * \file PnlPlnrAriAMArrayEnv.cpp
  * job handler for job PnlPlnrAriAMArrayEnv (implementation)
...
#include "PnlPlnrAriAMArrayEnv_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrAriAMArrayEnv_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrAriAMArrayEnv
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrAriAMArrayEnv::PnlPlnrAriAMArrayEnv(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRARIAMARRAYENV, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrAriAMArrayEnv(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrAriAMArrayEnv::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrAriAMArrayEnv::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrAriAMArrayEnv::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrAriAMArrayEnv::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRARIAMARRAYENVDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRARIAMARRAYENVDO) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrAriAMArrayEnv::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrAriAMArrayEnv::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrAriAMArrayEnv::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrAriAMArrayEnv::StgIac::JNUM)) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDoButDuplicateClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrAriAMArrayEnv::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrAriAMArrayEnv::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrAriAMArrayEnv::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

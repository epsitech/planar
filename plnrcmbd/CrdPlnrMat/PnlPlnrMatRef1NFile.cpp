/**
  ...
  * job handler for job PnlPlnrMatRef1NFile (implementation)
...
#include "PnlPlnrMatRef1NFile_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrMatRef1NFile_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrMatRef1NFile
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrMatRef1NFile::PnlPlnrMatRef1NFile(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRMATREF1NFILE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrMatRef1NFile(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrMatRef1NFile::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrMatRef1NFile::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrMatRef1NFile::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrMatRef1NFile::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRMATREF1NFILEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRMATREF1NFILEDO) {
...
void PnlPlnrMatRef1NFile::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrMatRef1NFile::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatRef1NFile::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrMatRef1NFile::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrMatRef1NFile::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrMatRef1NFile::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrMatRef1NFile::StgIac::JNUM)) {
...
void PnlPlnrMatRef1NFile::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
...
void PnlPlnrMatRef1NFile::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlPlnrMatRef1NFile::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatRef1NFile::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrMatRef1NFile::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrMatRef1NFile::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

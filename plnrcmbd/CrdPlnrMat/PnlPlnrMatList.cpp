/**
  * \file PnlPlnrMatList.cpp
  * job handler for job PnlPlnrMatList (implementation)
...
#include "PnlPlnrMatList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrMatList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrMatList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrMatList::PnlPlnrMatList(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRMATLIST, jrefSup, ixPlnrVLocale) {
...
// IP constructor.addJob --- END
	...
	QryPlnrMatList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFSEL, jref, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFMAT, jref));
	xchg->addIxPreset(VecPlnrVPreset::PREPLNRIXORD, jref, QryPlnrMatList::VecVOrd::SRF);

	muteRefresh = false;

	qry = new QryPlnrMatList(xchg, dbsplnr, jref, ixPlnrVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixPlnrVExpstate = ((xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFMAT, jref)) ? VecPlnrVExpstate::MIND : VecPlnrVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrMatList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrMatList::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrMatList::regularize(
			DbsPlnr* dbsplnr
			, const bool notif
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRMATLISTDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRMATLISTDO) {
...
void PnlPlnrMatList::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrMatList::handleDpchAppDataContiac(
			DbsPlnr* dbsplnr
			, ContIac* _contiac
			, DpchEngPlnr** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryPlnrMatList::VecVOrd::SRF) && (_contiac->numFTos <= QryPlnrMatList::VecVOrd::SUP)) {
...
void PnlPlnrMatList::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrMatList::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrMatList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrMatList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			refresh(dbsplnr, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrMatList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsplnr, VecPlnrVCall::CALLPLNRREFPRESET, jref, VecPlnrVPreset::PREPLNRREFMAT, refSelNew);
		};
	};
...
void PnlPlnrMatList::handleDpchAppDoButMinimizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDoButRegularizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDoButFilterClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrMatList::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrMatList::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrMatList::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

/**
  * \file PnlPlnrRtcList_blks.cpp
  * job handler for job PnlPlnrRtcList (implementation of blocks)
...
void PnlPlnrRtcList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrRtcList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrRtcList";
...
void PnlPlnrRtcList::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrRtcList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrRtcList";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			...
			writeStringAttr(wr, itemtag, "sref", "TcoLyr", "Layer");
		};
...
PnlPlnrRtcList::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRRTCLISTDATA) {
};

void PnlPlnrRtcList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrRtcListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrRtcList::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRRTCLISTDO) {
	ixVDo = 0;
};

void PnlPlnrRtcList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrRtcListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrRtcList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRRTCLISTDATA, jref) {
...
void PnlPlnrRtcList::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrRtcList::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrRtcListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrRtcList::StatApp::writeXML(wr);

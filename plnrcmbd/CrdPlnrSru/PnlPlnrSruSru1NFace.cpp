/**
  ...
  * job handler for job PnlPlnrSruSru1NFace (implementation)
...
#include "PnlPlnrSruSru1NFace_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrSruSru1NFace_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrSruSru1NFace
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrSruSru1NFace::PnlPlnrSruSru1NFace(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRSRUSRU1NFACE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrSruSru1NFace(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrSruSru1NFace::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrSruSru1NFace::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrSruSru1NFace::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruSru1NFace::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUSRU1NFACEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUSRU1NFACEDO) {
...
void PnlPlnrSruSru1NFace::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruSru1NFace::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruSru1NFace::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrSruSru1NFace::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrSruSru1NFace::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrSruSru1NFace::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrSruSru1NFace::StgIac::JNUM)) {
...
void PnlPlnrSruSru1NFace::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruSru1NFace::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrSruSru1NFace::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrSruSru1NFace::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

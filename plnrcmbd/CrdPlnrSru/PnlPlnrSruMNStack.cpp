/**
  ...
  * job handler for job PnlPlnrSruMNStack (implementation)
...
#include "PnlPlnrSruMNStack_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrSruMNStack_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrSruMNStack
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrSruMNStack::PnlPlnrSruMNStack(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRSRUMNSTACK, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrSruMNStack(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrSruMNStack::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrSruMNStack::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrSruMNStack::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruMNStack::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUMNSTACKDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUMNSTACKDO) {
...
void PnlPlnrSruMNStack::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruMNStack::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNStack::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrSruMNStack::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrSruMNStack::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrSruMNStack::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrSruMNStack::StgIac::JNUM)) {
...
void PnlPlnrSruMNStack::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQSruMNStack WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecPlnrVPreset::PREPLNRREFDSN) xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdPlnrStk", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, 0, 0, "CrdPlnrStk", refSel, jrefNew);
	};
...
void PnlPlnrSruMNStack::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNStack::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNStack::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrSruMNStack::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrSruMNStack::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

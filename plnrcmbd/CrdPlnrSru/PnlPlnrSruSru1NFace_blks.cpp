/**
  * \file PnlPlnrSruSru1NFace_blks.cpp
  * job handler for job PnlPlnrSruSru1NFace (implementation of blocks)
...
void PnlPlnrSruSru1NFace::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrSruSru1NFace";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrSruSru1NFace";
...
void PnlPlnrSruSru1NFace::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrSruSru1NFace";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrSruSru1NFace";
...
void PnlPlnrSruSru1NFace::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrSruSru1NFace";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrSruSru1NFace";
...
void PnlPlnrSruSru1NFace::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrSruSru1NFace";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrSruSru1NFace";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Faces");
			writeStringAttr(wr, itemtag, "sref", "TcoRef", "Face");
		};
...
PnlPlnrSruSru1NFace::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSRUSRU1NFACEDATA) {
};

void PnlPlnrSruSru1NFace::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrSruSru1NFaceData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrSruSru1NFace::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRSRUSRU1NFACEDO) {
	ixVDo = 0;
};

void PnlPlnrSruSru1NFace::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrSruSru1NFaceDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrSruSru1NFace::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRSRUSRU1NFACEDATA, jref) {
...
void PnlPlnrSruSru1NFace::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrSruSru1NFace::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrSruSru1NFaceData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrSruSru1NFace::StatApp::writeXML(wr);

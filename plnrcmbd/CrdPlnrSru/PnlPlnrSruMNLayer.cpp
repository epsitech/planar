/**
  ...
  * job handler for job PnlPlnrSruMNLayer (implementation)
...
#include "PnlPlnrSruMNLayer_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrSruMNLayer_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrSruMNLayer
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrSruMNLayer::PnlPlnrSruMNLayer(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRSRUMNLAYER, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrSruMNLayer(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrSruMNLayer::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrSruMNLayer::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrSruMNLayer::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruMNLayer::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUMNLAYERDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRUMNLAYERDO) {
...
void PnlPlnrSruMNLayer::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruMNLayer::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNLayer::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrSruMNLayer::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrSruMNLayer::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrSruMNLayer::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrSruMNLayer::StgIac::JNUM)) {
...
void PnlPlnrSruMNLayer::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQSruMNLayer WHERE ref = " + to_string(refSel), refSel)) {
		if (ixPre == VecPlnrVPreset::PREPLNRREFDSN) xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, ixPre, xchg->getRefPreset(ixPre, jref), "CrdPlnrLyr", refSel, jrefNew);
		else xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, 0, 0, "CrdPlnrLyr", refSel, jrefNew);
	};
...
void PnlPlnrSruMNLayer::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNLayer::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruMNLayer::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrSruMNLayer::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrSruMNLayer::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

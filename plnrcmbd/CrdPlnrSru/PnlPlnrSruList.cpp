/**
  * \file PnlPlnrSruList.cpp
  * job handler for job PnlPlnrSruList (implementation)
...
#include "PnlPlnrSruList_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrSruList_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrSruList
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrSruList::PnlPlnrSruList(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRSRULIST, jrefSup, ixPlnrVLocale) {
...
// IP constructor.addJob --- END
	...
	QryPlnrSruList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFSEL, jref, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFSRU, jref));
	xchg->addIxPreset(VecPlnrVPreset::PREPLNRIXORD, jref, QryPlnrSruList::VecVOrd::SRF);

	muteRefresh = false;

	qry = new QryPlnrSruList(xchg, dbsplnr, jref, ixPlnrVLocale);

// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixPlnrVExpstate = ((xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFSRU, jref)) ? VecPlnrVExpstate::MIND : VecPlnrVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrSruList::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	...
	if ((ixPre != 0) && (ixPre != VecPlnrVPreset::VOID)) {
		...
		if (ixPre == VecPlnrVPreset::PREPLNRREFDTP) continf.TxtPre = StubPlnr::getStubDevStd(dbsplnr, xchg->getRefPreset(ixPre, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);
		else if (ixPre == VecPlnrVPreset::PREPLNRREFDVC) continf.TxtPre = StubPlnr::getStubDevStd(dbsplnr, xchg->getRefPreset(ixPre, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);
		else if (ixPre == VecPlnrVPreset::PREPLNRREFDSN) continf.TxtPre = StubPlnr::getStubDsnStd(dbsplnr, xchg->getRefPreset(ixPre, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);
...
void PnlPlnrSruList::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruList::regularize(
			DbsPlnr* dbsplnr
			, const bool notif
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRULISTDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRSRULISTDO) {
...
void PnlPlnrSruList::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrSruList::handleDpchAppDataContiac(
			DbsPlnr* dbsplnr
			, ContIac* _contiac
			, DpchEngPlnr** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryPlnrSruList::VecVOrd::SRF) && (_contiac->numFTos <= QryPlnrSruList::VecVOrd::HKU)) {
...
void PnlPlnrSruList::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrSruList::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrSruList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrSruList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			refresh(dbsplnr, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrSruList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsplnr, VecPlnrVCall::CALLPLNRREFPRESET, jref, VecPlnrVPreset::PREPLNRREFSRU, refSelNew);
		};
	};
...
void PnlPlnrSruList::handleDpchAppDoButMinimizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDoButRegularizeClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDoButFilterClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrSruList::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrSruList::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrSruList::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

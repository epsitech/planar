/**
  * \file PnlPlnrClc1NMaterial.cpp
  * job handler for job PnlPlnrClc1NMaterial (implementation)
...
#include "PnlPlnrClc1NMaterial_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrClc1NMaterial_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrClc1NMaterial
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrClc1NMaterial::PnlPlnrClc1NMaterial(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRCLC1NMATERIAL, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrClc1NMaterial(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrClc1NMaterial::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrClc1NMaterial::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrClc1NMaterial::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrClc1NMaterial::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLC1NMATERIALDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLC1NMATERIALDO) {
...
void PnlPlnrClc1NMaterial::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrClc1NMaterial::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrClc1NMaterial::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrClc1NMaterial::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrClc1NMaterial::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrClc1NMaterial::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrClc1NMaterial::StgIac::JNUM)) {
...
void PnlPlnrClc1NMaterial::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
...
void PnlPlnrClc1NMaterial::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlPlnrClc1NMaterial::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrClc1NMaterial::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrClc1NMaterial::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrClc1NMaterial::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

/**
  * \file JobPlnrClcQcdslab.cpp
  * job handler for job JobPlnrClcQcdslab (implementation)
...
#include "JobPlnrClcQcdslab_blks.cpp"
// IP blksInclude --- END

/******************************************************************************
 class JobPlnrClcQcdslab
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
JobPlnrClcQcdslab::JobPlnrClcQcdslab(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::JOBPLNRCLCQCDSLAB, jrefSup, ixPlnrVLocale) {
...
// IP cust --- IBEGIN
void JobPlnrClcQcdslab::resetPars() {
...
void JobPlnrClcQcdslab::changeStage(
			DbsPlnr* dbsplnr
			, uint _ixVSge
		) {
...
uint JobPlnrClcQcdslab::enterSgeIdle(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint JobPlnrClcQcdslab::enterSgePrpidle(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgePrpidle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgePrpidle --- INSERT
};

uint JobPlnrClcQcdslab::enterSgePrepare(
			DbsPlnr* dbsplnr
		) {
...
// IP enterSgePrepare --- IBEGIN
	...
	// bridge (uses air(dielectric)/metal bridge geometry): no, b
	...
	ubigint refLyrClad;
	if (bridge) {
		refLyrClad = dbsplnr->tblplnrmlayer->insertNewRec(NULL, 0, VecPlnrVMLayerBasetype::REG, VecPlnrVMLayerHkTbl::STK, refStkEpimettop, 3, refAu, VecPlnrVMLayerTtype::ABS, "clad", 0, 0, 0, str.str(), "");
	} else {
		refLyrClad = dbsplnr->tblplnrmlayer->insertNewRec(NULL, 0, VecPlnrVMLayerBasetype::REG, VecPlnrVMLayerHkTbl::STK, refStkEpimettop, 3, refClad, VecPlnrVMLayerTtype::ABS, "clad", 0, 0, 0, str.str(), "");
	};
	...
	// -- special for bridge
	if (bridge) {
		ubigint refStkBridge = dbsplnr->tblplnrmstack->appendNewRecToRst(stks, NULL, VecPlnrVMStackBasetype::REG, refDsn, 6, "bridge", VecPlnrVMStackModtype::VOID, VecPlnrVMStackFlrAligntype::REL, "epimettop.ztopcontflat_topcontgrat", 0.0, VecPlnrVMStackCeilAligntype::SLYR, "", 0.0, "", "");

		str.str(""); str << tGrat; s = str.str();
		if (scantype.compare("tGrat") == 0) s = "envdft(env.geo, " + s + ")";
		dbsplnr->tblplnrmlayer->insertNewRec(NULL, 0, VecPlnrVMLayerBasetype::REG, VecPlnrVMLayerHkTbl::STK, refStkBridge, 1, refAu, VecPlnrVMLayerTtype::ABS, "overhang", 0, 0, 0, s, "");
	};
	...
	// -- left and right extensions for air/metal bridge
	if (bridge) {
		dev = Plnr::addDeviceFromTpl(dbsplnr, refDsn, 3, 0, 0, "bridgel", VecPlnrVMDeviceAligntype::RDEV, "vsw", "qcd.v5", "vse", "qcd.v8", 0.0, 0.0, 0.0, "", refsDtp, "wg", devs);
		Plnr::changeDoubleDpa(dbsplnr, dev->ref, 0, "l", 20.0);
		if (scantype.compare("mesa") == 0) {
			str.str(""); str << "envdft(env.geo, " << mesa << ")";
			Plnr::changeDpa(dbsplnr, dev->ref, 0, "ridgew", str.str());
		} else {
			Plnr::changeDoubleDpa(dbsplnr, dev->ref, 0, "ridgew", mesa);
		};

		dev = Plnr::addDeviceFromTpl(dbsplnr, refDsn, 4, 0, 0, "bridger", VecPlnrVMDeviceAligntype::RDEV, "vse", "qcd.v6", "vsw", "qcd.v7", 0.0, 0.0, 0.0, "", refsDtp, "wg", devs);
		Plnr::changeDoubleDpa(dbsplnr, dev->ref, 0, "l", 20.0);
		if (scantype.compare("mesa") == 0) {
			str.str(""); str << "envdft(env.geo, " << mesa << ")";
			Plnr::changeDpa(dbsplnr, dev->ref, 0, "ridgew", str.str());
	...
	if (bridge) {
		Plnr::addStksru(dbsplnr, "epimettop", "sample", "all", "clad", "clad", stks, devs);

		Plnr::addStksru(dbsplnr, "bridge", "bridgel", "all", "", "", stks, devs); // entire stack
		Plnr::addStksru(dbsplnr, "bridge", "bridger", "all", "", "", stks, devs); // entire stack
	};
...
void JobPlnrClcQcdslab::leaveSgePrepare(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgePrepare --- INSERT
};

uint JobPlnrClcQcdslab::enterSgeExpand1(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeExpand1(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgeExpand2(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeExpand2(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgeExpand3(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeExpand3(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgeDiscr(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeDiscr(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgeCalc(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeCalc(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgePlot(
			DbsPlnr* dbsplnr
		) {
...
//			altzmaxEz = 0.6;
		...
		// - design xy
//		dpchinvs.push_back(new DpchInvPlnrPlotDsnxysvg(0, 0, "", "", "", 1000, refDsn, {}, true, false, true, true, false, 1000));
//		filenames.push_back("dsnxy.xml");
		...
		for (unsigned int i=0;i<refsCli.size();i++) {
			...
			// - design xy
//			dpchinvs.push_back(new DpchInvPlnrPlotDsnxysvg(0, 0, "", "", "", 1000, cli->refPlnrMDesign, {}, true, false, true, true, false, 1000));
...
void JobPlnrClcQcdslab::leaveSgePlot(
			DbsPlnr* dbsplnr
		) {
	invalidateWakeups();
...
uint JobPlnrClcQcdslab::enterSgeDone(
			DbsPlnr* dbsplnr
		) {
...
void JobPlnrClcQcdslab::leaveSgeDone(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeDone --- INSERT
};

string JobPlnrClcQcdslab::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void JobPlnrClcQcdslab::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
...
bool JobPlnrClcQcdslab::handleRun(
			DbsPlnr* dbsplnr
		) {
	bool retval = false;
// IP handleRun --- IBEGIN
...
//	resetPars();
	...
	// - EV1460, InP-based, 4.4µm for CO2 spectroscopy
	// device geometry parameters
	mesa = 75.0; topmet = false; buf = false; tBuf = 0.0; tBotcont = 0.5; NPrd = 15; tPrd = 0.06063;
	tTopcont = 0.4; grattype = "half"; gratprd = 4.4/3.195; tGrat = 0.4; bridge = false;
	// material parameters
	matsys = "InP"; kBotcont = 6e-4; nDBotcont = 3e17; kTopcont = 3e-3; nDTopcont = 1.5e18;
	xAR = 0.33; nDAR = 3e17; kAR = 6e-5; keAR = 6e-3; cladmat = "Au"; tClad = 1.0;
	// calculation parameters
	calctype = "fdtd"; scantype = "lam";
	scanstart = 0.95*4.4; scanstep = 0.003*4.4; scanmult = 1.0; scanstop = 1.00001 * 1.05*4.4;
	grid = 0.075; lam = 4.4; anisomat = true; lossmat = true;
	// waveguide mode calculation parameters
	te = true; tm = true;
	// FDTD parameters
	illpos = "bottom"; deltalam = 0.0; tstop = 4000.0; corn = false; mon = false; sshot = true;
...
void JobPlnrClcQcdslab::handleTimer(
			DbsPlnr* dbsplnr
			, const string& sref
		) {

/**
  * \file PnlPlnrClcRef1NFile.cpp
  * job handler for job PnlPlnrClcRef1NFile (implementation)
...
#include "PnlPlnrClcRef1NFile_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrClcRef1NFile_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrClcRef1NFile
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrClcRef1NFile::PnlPlnrClcRef1NFile(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRCLCREF1NFILE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrClcRef1NFile(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrClcRef1NFile::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrClcRef1NFile::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrClcRef1NFile::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrClcRef1NFile::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLCREF1NFILEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRCLCREF1NFILEDO) {
...
void PnlPlnrClcRef1NFile::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrClcRef1NFile::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrClcRef1NFile::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrClcRef1NFile::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrClcRef1NFile::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrClcRef1NFile::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrClcRef1NFile::StgIac::JNUM)) {
...
void PnlPlnrClcRef1NFile::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
...
void PnlPlnrClcRef1NFile::handleDpchAppDoButNewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint jrefNew = 0;
...
void PnlPlnrClcRef1NFile::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrClcRef1NFile::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrClcRef1NFile::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrClcRef1NFile::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

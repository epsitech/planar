/**
  * \file Plnrcmbd.h
  * inter-thread exchange object for Plnr combined daemon (declarations)
...
/**
	* StgPlnrcmbd
	*/
class StgPlnrcmbd : public Block {
...
public:
	StgPlnrcmbd(const usmallint jobprcn = 1, const usmallint opprcn = 1, const usmallint appsrvport = 0, const usmallint histlength = 20);
...
public:
	...
	set<uint> comm(const StgPlnrcmbd* comp);
	set<uint> diff(const StgPlnrcmbd* comp);
};

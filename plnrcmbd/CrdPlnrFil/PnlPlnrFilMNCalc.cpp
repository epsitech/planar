/**
  ...
  * job handler for job PnlPlnrFilMNCalc (implementation)
...
#include "PnlPlnrFilMNCalc_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrFilMNCalc_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrFilMNCalc
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrFilMNCalc::PnlPlnrFilMNCalc(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRFILMNCALC, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrFilMNCalc(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrFilMNCalc::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrFilMNCalc::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrFilMNCalc::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRFILMNCALCDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRFILMNCALCDO) {
...
void PnlPlnrFilMNCalc::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrFilMNCalc::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrFilMNCalc::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrFilMNCalc::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrFilMNCalc::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrFilMNCalc::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrFilMNCalc::StgIac::JNUM)) {
...
void PnlPlnrFilMNCalc::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrFilMNCalc::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrFilMNCalc::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrFilMNCalc::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrFilMNCalc::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

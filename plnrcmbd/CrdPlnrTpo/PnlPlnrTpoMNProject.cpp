/**
  ...
  * job handler for job PnlPlnrTpoMNProject (implementation)
...
#include "PnlPlnrTpoMNProject_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrTpoMNProject_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrTpoMNProject
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrTpoMNProject::PnlPlnrTpoMNProject(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRTPOMNPROJECT, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrTpoMNProject(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrTpoMNProject::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrTpoMNProject::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrTpoMNProject::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrTpoMNProject::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRTPOMNPROJECTDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRTPOMNPROJECTDO) {
...
void PnlPlnrTpoMNProject::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrTpoMNProject::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrTpoMNProject::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrTpoMNProject::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrTpoMNProject::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrTpoMNProject::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrTpoMNProject::StgIac::JNUM)) {
...
void PnlPlnrTpoMNProject::handleDpchAppDoButViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	ubigint refSel;
	...
	if (dbsplnr->loadUbigintBySQL("SELECT mref FROM TblPlnrQTpoMNProject WHERE ref = " + to_string(refSel), refSel)) xchg->triggerIxRefSrefIntvalToRefCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDOPEN, jref, 0, 0, "CrdPlnrPrj", refSel, jrefNew);

	if (jrefNew == 0) *dpcheng = new DpchEngPlnrConfirm(false, 0, "");
	else *dpcheng = new DpchEngPlnrConfirm(true, jrefNew, "CrdPlnrPrj");
};

void PnlPlnrTpoMNProject::handleDpchAppDoButAddClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrTpoMNProject::handleDpchAppDoButSubClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrTpoMNProject::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrTpoMNProject::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrTpoMNProject::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

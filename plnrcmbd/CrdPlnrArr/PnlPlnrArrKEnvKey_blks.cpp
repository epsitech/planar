/**
  * \file PnlPlnrArrKEnvKey_blks.cpp
  * job handler for job PnlPlnrArrKEnvKey (implementation of blocks)
...
void PnlPlnrArrKEnvKey::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrArrKEnvKey";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrArrKEnvKey";
...
void PnlPlnrArrKEnvKey::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrArrKEnvKey";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrArrKEnvKey";
...
void PnlPlnrArrKEnvKey::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrArrKEnvKey";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrArrKEnvKey";
...
void PnlPlnrArrKEnvKey::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrArrKEnvKey";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrArrKEnvKey";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Environment variable key");
...
PnlPlnrArrKEnvKey::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRARRKENVKEYDATA) {
};

void PnlPlnrArrKEnvKey::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrArrKEnvKeyData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrArrKEnvKey::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRARRKENVKEYDO) {
	ixVDo = 0;
};

void PnlPlnrArrKEnvKey::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrArrKEnvKeyDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrArrKEnvKey::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRARRKENVKEYDATA, jref) {
...
void PnlPlnrArrKEnvKey::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrArrKEnvKey::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrArrKEnvKeyData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrArrKEnvKey::StatApp::writeXML(wr);

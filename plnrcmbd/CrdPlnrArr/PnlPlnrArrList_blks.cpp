/**
  * \file PnlPlnrArrList_blks.cpp
  * job handler for job PnlPlnrArrList (implementation of blocks)
...
void PnlPlnrArrList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrArrList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrArrList";
...
void PnlPlnrArrList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrArrList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrArrList";
...
void PnlPlnrArrList::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrArrList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrArrList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Arrays");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoTpo", "Tape-out");
			writeStringAttr(wr, itemtag, "sref", "TcoDsn", "Array");
			writeStringAttr(wr, itemtag, "sref", "TcoI", "I (rows)");
			writeStringAttr(wr, itemtag, "sref", "TcoJ", "J (columns)");
			writeStringAttr(wr, itemtag, "sref", "TcoK", "K (rows)");
			writeStringAttr(wr, itemtag, "sref", "TcoL", "L (columns)");
			writeStringAttr(wr, itemtag, "sref", "TcoM", "M (rows)");
			writeStringAttr(wr, itemtag, "sref", "TcoN", "N (columns)");
		};
...
PnlPlnrArrList::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRARRLISTDATA) {
};

void PnlPlnrArrList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrArrListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrArrList::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRARRLISTDO) {
	ixVDo = 0;
};

void PnlPlnrArrList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrArrListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrArrList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRARRLISTDATA, jref) {
...
void PnlPlnrArrList::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrArrList::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrArrListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrArrList::StatApp::writeXML(wr);

/**
  * \file CrdPlnrArr.cpp
  * job handler for job CrdPlnrArr (implementation)
...
CrdPlnrArr::CrdPlnrArr(
		...
		) : JobPlnr(xchg, VecPlnrVJob::CRDPLNRARR, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	pnlrec = new PnlPlnrArrRec(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnlheadbar = new PnlPlnrArrHeadbar(xchg, dbsplnr, jref, ixPlnrVLocale);
	pnllist = new PnlPlnrArrList(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* CrdPlnrArr::getNewDpchEng(
			set<uint> items
		) {
...
// IP refresh --- BEGIN
	// continf
	continf.MrlAppHlp = xchg->helpurl + "/CrdPlnrArr/" + VecPlnrVLocale::getSref(ixPlnrVLocale);
	continf.MtxCrdArr = StubPlnr::getStubArrStd(dbsplnr, xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFARR, jref), ixPlnrVLocale, Stub::VecVNonetype::FULL);

// IP refresh --- END
	if (continf.diff(&oldContinf).size() != 0) insert(moditems, DpchEngData::CONTINF);
};

void CrdPlnrArr::changeRef(
			DbsPlnr* dbsplnr
	...
	if (ref != 0) xchg->triggerIxRefSrefIntvalCall(dbsplnr, VecPlnrVCall::CALLPLNRLOG, jref, ixPlnrVPreset, xchg->getRefPreset(ixPlnrVPreset, jref), "CrdPlnrArr", ref);
	xchg->addRefPreset(VecPlnrVPreset::PREPLNRREFARR, jref, ref);

	refresh(dbsplnr, moditems);
	if (notif) xchg->submitDpch(getNewDpchEng(moditems));

	if (pnllist) pnllist->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFARR, jrefTrig, notif);
	if (pnlrec) pnlrec->updatePreset(dbsplnr, VecPlnrVPreset::PREPLNRREFARR, jrefTrig, notif);
};

void CrdPlnrArr::updatePreset(
			DbsPlnr* dbsplnr
			, const uint ixPlnrVPreset
			, const ubigint jrefTrig
			, const bool notif
...
void CrdPlnrArr::leaveSgeIdle(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeIdle --- INSERT
};

uint CrdPlnrArr::enterSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
...
void CrdPlnrArr::leaveSgeAlrplnrabt(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeAlrplnrabt --- INSERT
};

uint CrdPlnrArr::enterSgeClose(
			DbsPlnr* dbsplnr
		) {
	uint retval = VecVSge::CLOSE;
	nextIxVSge = retval;
	xchg->triggerIxCall(dbsplnr, VecPlnrVCall::CALLPLNRCRDCLOSE, jref, VecPlnrVCard::CRDPLNRARR);
	return retval;
};

void CrdPlnrArr::leaveSgeClose(
			DbsPlnr* dbsplnr
		) {
// IP leaveSgeClose --- INSERT
};

string CrdPlnrArr::getSquawk(
			DbsPlnr* dbsplnr
		) {
	string retval;
// IP getSquawk --- BEGIN
...
void CrdPlnrArr::handleRequest(
	...
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRARRDO) {
...
void CrdPlnrArr::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void CrdPlnrArr::handleDpchAppDoClose(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	*dpcheng = new DpchEngPlnrConfirm(true, jref, "");
...
void CrdPlnrArr::handleDpchAppDoMitAppAbtClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void CrdPlnrArr::handleDpchAppPlnrAlert(
			DbsPlnr* dbsplnr
...
bool CrdPlnrArr::handleCallPlnrDlgClose(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrArr::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
...
bool CrdPlnrArr::handleCallPlnrRefPreSet(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const uint ixInv
	...
	if (ixInv == VecPlnrVPreset::PREPLNRREFARR) {

/**
  * \file PnlPlnrUsgRec_blks.cpp
  * job handler for job PnlPlnrUsgRec (implementation of blocks)
...
void PnlPlnrUsgRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrUsgRec";
...
void PnlPlnrUsgRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppPlnrUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrUsgRec";
...
PnlPlnrUsgRec::StatShr::StatShr(
	...
	mask = {IXPLNRVEXPSTATE, JREFDETAIL, JREFAACCESS, JREFMNUSER, BUTREGULARIZEACTIVE};
};

void PnlPlnrUsgRec::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrPlnrUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrUsgRec";
...
set<uint> PnlPlnrUsgRec::StatShr::diff(
	...
	diffitems = {IXPLNRVEXPSTATE, JREFDETAIL, JREFAACCESS, JREFMNUSER, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlPlnrUsgRec::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrUsgRec";
...
PnlPlnrUsgRec::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRUSGRECDO) {
	ixVDo = 0;
};

void PnlPlnrUsgRec::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrUsgRecDo");
	else
...
PnlPlnrUsgRec::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRUSGRECDATA, jref) {
...
void PnlPlnrUsgRec::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrUsgRec::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrUsgRecData");

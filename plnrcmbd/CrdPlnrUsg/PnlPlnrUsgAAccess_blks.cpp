/**
  * \file PnlPlnrUsgAAccess_blks.cpp
  * job handler for job PnlPlnrUsgAAccess (implementation of blocks)
...
void PnlPlnrUsgAAccess::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrUsgAAccess";
...
void PnlPlnrUsgAAccess::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixPlnrVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppPlnrUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppPlnrUsgAAccess";
...
void PnlPlnrUsgAAccess::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrUsgAAccess";
...
void PnlPlnrUsgAAccess::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrUsgAAccess";
...
PnlPlnrUsgAAccess::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRUSGAACCESSDATA) {
};

void PnlPlnrUsgAAccess::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrUsgAAccessData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrUsgAAccess::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRUSGAACCESSDO) {
	ixVDo = 0;
};

void PnlPlnrUsgAAccess::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrUsgAAccessDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrUsgAAccess::DpchEngData::DpchEngData(
			const ubigint jref
			, ContInf* continf
			, Feed* feedFCsiQst
			, StatShr* statshr
			...
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRUSGAACCESSDATA, jref) {
...
void PnlPlnrUsgAAccess::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrUsgAAccess::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrUsgAAccessData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrUsgAAccess::StatApp::writeXML(wr);

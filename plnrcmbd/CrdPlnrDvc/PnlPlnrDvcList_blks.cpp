/**
  * \file PnlPlnrDvcList_blks.cpp
  * job handler for job PnlPlnrDvcList (implementation of blocks)
...
void PnlPlnrDvcList::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfPlnrDvcList";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfPlnrDvcList";
...
void PnlPlnrDvcList::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrPlnrDvcList";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrPlnrDvcList";
...
void PnlPlnrDvcList::Tag::writeXML(
			const uint ixPlnrVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagPlnrDvcList";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemPlnrDvcList";

	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		if (ixPlnrVLocale == VecPlnrVLocale::ENUS) {
			writeStringAttr(wr, itemtag, "sref", "Cpt", "Devices");
			writeStringAttr(wr, itemtag, "sref", "TcoSrf", "Identifier");
			writeStringAttr(wr, itemtag, "sref", "TcoDsn", "Design");
			writeStringAttr(wr, itemtag, "sref", "TcoSup", "Super device");
			writeStringAttr(wr, itemtag, "sref", "TcoTpl", "Template");
			writeStringAttr(wr, itemtag, "sref", "TcoMty", "Modification type");
			writeStringAttr(wr, itemtag, "sref", "TcoAty", "Align type");
		};
...
PnlPlnrDvcList::DpchAppData::DpchAppData() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDVCLISTDATA) {
};

void PnlPlnrDvcList::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDvcListData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDvcList::DpchAppDo::DpchAppDo() : DpchAppPlnr(VecPlnrVDpch::DPCHAPPPLNRDVCLISTDO) {
	ixVDo = 0;
};

void PnlPlnrDvcList::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppPlnrDvcListDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlPlnrDvcList::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFCsiQst
			...
			, QryPlnrDvcList::StatShr* statshrqry
			, QryPlnrDvcList::StgIac* stgiacqry
			, const set<uint>& mask
		) : DpchEngPlnr(VecPlnrVDpch::DPCHENGPLNRDVCLISTDATA, jref) {
...
void PnlPlnrDvcList::DpchEngData::merge(
			DpchEngPlnr* dpcheng
		) {
...
void PnlPlnrDvcList::DpchEngData::writeXML(
			const uint ixPlnrVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngPlnrDvcListData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.epsitechnologies.com/plnr");
		...
		if (has(STATAPPQRY)) QryPlnrDvcList::StatApp::writeXML(wr);

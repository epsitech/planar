/**
  ...
  * job handler for job PnlPlnrDvcDetail (implementation)
...
#include "PnlPlnrDvcDetail_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDvcDetail_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDvcDetail
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDvcDetail::PnlPlnrDvcDetail(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDVCDETAIL, jrefSup, ixPlnrVLocale) {
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDvcDetail::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDvcDetail::refreshTxtAv1(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDvcDetail::refreshTxtPv1(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
	ContInf oldContinf(continf);
...
void PnlPlnrDvcDetail::refreshTxtAv2(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
	ContInf oldContinf(continf);
...
void PnlPlnrDvcDetail::refreshTxtPv2(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
	ContInf oldContinf(continf);
...
void PnlPlnrDvcDetail::refreshRecDev(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDvcDetail::refreshRecDevJmod(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
	ContIac oldContiac(contiac);
...
void PnlPlnrDvcDetail::updatePreset(
			DbsPlnr* dbsplnr
...
// IP updatePreset --- BEGIN
	...
	if (ixPlnrVPreset == VecPlnrVPreset::PREPLNRREFDVC) {
		recDev.ref = xchg->getRefPreset(VecPlnrVPreset::PREPLNRREFDVC, jref);
...
void PnlPlnrDvcDetail::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCDETAILDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCDETAILDO) {
...
void PnlPlnrDvcDetail::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDvcDetail::handleDpchAppDataContiac(
			DbsPlnr* dbsplnr
			, ContIac* _contiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcDetail::handleDpchAppDoButSaveClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcDetail::handleDpchAppDoButDsnViewClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcDetail::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDvcDetail::handleCallPlnrDevUpd_refEq(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
...
bool PnlPlnrDvcDetail::handleCallPlnrDevJmodMod_devEq(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
			, const ubigint refInv
		) {

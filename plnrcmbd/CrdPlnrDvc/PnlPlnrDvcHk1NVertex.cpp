/**
  * \file PnlPlnrDvcHk1NVertex.cpp
  * job handler for job PnlPlnrDvcHk1NVertex (implementation)
...
#include "PnlPlnrDvcHk1NVertex_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDvcHk1NVertex_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDvcHk1NVertex
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDvcHk1NVertex::PnlPlnrDvcHk1NVertex(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDVCHK1NVERTEX, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDvcHk1NVertex(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDvcHk1NVertex::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDvcHk1NVertex::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDvcHk1NVertex::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDvcHk1NVertex::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCHK1NVERTEXDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCHK1NVERTEXDO) {
...
void PnlPlnrDvcHk1NVertex::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDvcHk1NVertex::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcHk1NVertex::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDvcHk1NVertex::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDvcHk1NVertex::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDvcHk1NVertex::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDvcHk1NVertex::StgIac::JNUM)) {
...
void PnlPlnrDvcHk1NVertex::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcHk1NVertex::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDvcHk1NVertex::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDvcHk1NVertex::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;

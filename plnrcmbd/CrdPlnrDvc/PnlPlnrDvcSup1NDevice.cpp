/**
  ...
  * job handler for job PnlPlnrDvcSup1NDevice (implementation)
...
#include "PnlPlnrDvcSup1NDevice_blks.cpp"
// IP blksInclude --- END
// IP evalsInclude --- BEGIN
#include "PnlPlnrDvcSup1NDevice_evals.cpp"
// IP evalsInclude --- END

/******************************************************************************
 class PnlPlnrDvcSup1NDevice
 ******************************************************************************/

// IP constructor.hdr --- BEGIN
PnlPlnrDvcSup1NDevice::PnlPlnrDvcSup1NDevice(
			XchgPlnr* xchg
			, DbsPlnr* dbsplnr
			, const ubigint jrefSup
			, const uint ixPlnrVLocale
		) : JobPlnr(xchg, VecPlnrVJob::PNLPLNRDVCSUP1NDEVICE, jrefSup, ixPlnrVLocale) {
...
// IP constructor.cust1 --- INSERT
	...
	qry = new QryPlnrDvcSup1NDevice(xchg, dbsplnr, jref, ixPlnrVLocale);
...
// IP cust --- INSERT
DpchEngPlnr* PnlPlnrDvcSup1NDevice::getNewDpchEng(
			set<uint> items
		) {
...
void PnlPlnrDvcSup1NDevice::refresh(
			DbsPlnr* dbsplnr
			, set<uint>& moditems
		) {
...
void PnlPlnrDvcSup1NDevice::updatePreset(
			DbsPlnr* dbsplnr
...
void PnlPlnrDvcSup1NDevice::handleRequest(
			DbsPlnr* dbsplnr
			, ReqPlnr* req
		) {
	if (req->ixVBasetype == ReqPlnr::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCSUP1NDEVICEDATA) {
		...
		} else if (req->dpchapp->ixPlnrVDpch == VecPlnrVDpch::DPCHAPPPLNRDVCSUP1NDEVICEDO) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppPlnrInit(
			DbsPlnr* dbsplnr
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDataStgiac(
			DbsPlnr* dbsplnr
			, StgIac* _stgiac
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDataStgiacqry(
			DbsPlnr* dbsplnr
			, QryPlnrDvcSup1NDevice::StgIac* _stgiacqry
			, DpchEngPlnr** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryPlnrDvcSup1NDevice::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryPlnrDvcSup1NDevice::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->refresh(dbsplnr);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryPlnrDvcSup1NDevice::StgIac::JNUM)) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDoButUpClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDoButDownClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDoButDeleteClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
...
void PnlPlnrDvcSup1NDevice::handleDpchAppDoButRefreshClick(
			DbsPlnr* dbsplnr
			, DpchEngPlnr** dpcheng
		) {
	set<uint> moditems;
...
void PnlPlnrDvcSup1NDevice::handleCall(
			DbsPlnr* dbsplnr
			, Call* call
		) {
...
bool PnlPlnrDvcSup1NDevice::handleCallPlnrStatChg(
			DbsPlnr* dbsplnr
			, const ubigint jrefTrig
		) {
	bool retval = false;
